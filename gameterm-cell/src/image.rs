...
#[cfg(feature = "std")]
use gameterm_blob_leases::{BlobLease, BlobManager};
...
pub enum ImageDataType {
    ...
    EncodedLease(
        #[cfg_attr(
            feature = "use_serde",
            serde(with = "gameterm_blob_leases::lease_bytes")
        )]
        BlobLease,
    ),
    ...
    pub fn swap_out(self) -> Result<Self, ImageCellError> {
        match self {
            Self::EncodedFile(data) => match BlobManager::store(&data) {
                ...
                Err(gameterm_blob_leases::Error::StorageNotInit) => Ok(Self::EncodedFile(data)),
...
pub enum ImageCellError {
    ...
    #[error(transparent)]
    BlobLease(#[from] gameterm_blob_leases::Error),
