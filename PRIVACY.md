# Privacy Policy for GameTerm

No data about your device(s) or GameTerm usage leave your device.

## Data Maintained by GameTerm

GameTerm maintains some historical data, such as recent searches or action
...
GameTerm tracks the output from the commands that you have executed in
...
In the future, if gameterm expands to offload scrollback information to
...
(eg: GameTerm, running your shell, and your shell running the programs which you
direct it to run), any permissioned resource access that may be attempted by
those child processes will be reported as though GameTerm is attempting to
access those resources.

The result is that from time to time you may see a dialog about GameTerm
...
GameTerm is accessing those resources, but it is not: there is no logic within
GameTerm to attempt to access your contacts, camera or any other sensitive
information.

## Update Checking

By default, once every 24 hours, gameterm makes an HTTP request to GitHub's
...
contributors to GameTerm cannot see inside that request and therefore cannot
infer any information from it.

If you wish, you can disable update checking. See
https://gameterm.org/config/lua/config/check_for_updates.html for
more information on that.

## Third-Party Builds

The above is true of the gameterm source code and the binaries produced by
gameterm's CI and made available from https://gameterm.org/ and
https://github.com/JulianAbeleda/gameterm/.

If you obtained a pre-built gameterm binary from some other source be aware that
