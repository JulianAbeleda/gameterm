...
#[error(
    "Please install the same version of gameterm on both the client and server!\n\
     The server version is {} (codec version {}),\n\
     which is not compatible with our version \n\
     {} (codec version {}).",
    version,
    codec_vers,
    config::gameterm_version(),
    CODEC_VERSION
)]
...
impl Reconnectable {
    ...
    /// Resolve the path to gameterm for the remote system.
    ...
    /// `path` comes from the SshDoman::remote_gameterm_path option; if set
    ...
    /// message to the user.
    fn gameterm_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("gameterm").to_string()
    }
    ...
    fn ssh_connect(
        ...
        let proxy_bin = Self::gameterm_bin_path(&ssh_dom.remote_gameterm_path);
    ...
    pub fn tls_connect(
        ...
        if let Some(Ok(ssh_params)) = tls_client.ssh_parameters() {
            if self.tls_creds.is_none() {
                ...
                let mut ssh_config = gameterm_ssh::Config::new();
                ...
                let creds = ui.run_and_log_error(|| {
                    ...
                    let cmd = format!(
                        "{} cli tlscreds",
                        Self::gameterm_bin_path(&tls_client.remote_gameterm_path)
                    );
...
impl Client {
        ...
        thread::spawn(move || {
            ...
            loop {
                if let Err(e) = client_thread(&mut reconnectable, local_domain_id, &mut receiver) {
                    ...
                    ui.title("gameterm: Reconnecting...");
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                let msg = if err.root_cause().is::<Timeout>() {
                    ...
                    format!(
                        "Please install the same version of gameterm on both \
    ...
    fn compute_unix_domain(
        ...
        match std::env::var_os("GAMETERM_UNIX_SOCKET") {
            ...
            Some(_) | None => {
                ...
                Ok(config
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "no default unix domain is configured and GAMETERM_UNIX_SOCKET \
    ...
    pub async fn resolve_pane_id(&self, pane_id: Option<PaneId>) -> anyhow::Result<PaneId> {
        let pane_id: PaneId = match pane_id {
            Some(p) => p,
            None => {
                if let Ok(pane) = std::env::var("GAMETERM_PANE") {
                    pane.parse()?
                } else {
                    ...
                    if clients.is_empty() {
                        anyhow::bail!(
                            "--pane-id was not specified and $GAMETERM_PANE
