...
use std::sync::Arc;
use gameterm_escape_parser::parser::Parser;
...
pub trait DeviceControlHandler: Send + Sync {
    fn handle_device_control(&mut self, _control: gameterm_escape_parser::DeviceControlMode);
}
...
impl Terminal {
    ...
    pub fn perform_actions(&mut self, actions: Vec<gameterm_escape_parser::Action>) {
        self.state.increment_seqno();
        {
            let mut performer = Performer::new(&mut self.state);
