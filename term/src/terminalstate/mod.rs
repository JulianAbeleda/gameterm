...
use gameterm_cell::image::ImageData;
use gameterm_cell::UnicodeVersion;
use gameterm_escape_parser::csi::{
...
use gameterm_escape_parser::{OneBased, OperatingSystemCommand, CSI};
use gameterm_surface::{CursorShape, CursorVisibility, SequenceNo};
...
lazy_static::lazy_static! {
    static ref DB: Database = {
        let data = include_bytes!("../../../termwiz/data/gameterm");
...
impl TerminalState {
    ...
    /// Here in gameterm the terminalstate is isolated from other
    ...
    fn perform_csi_mode(&mut self, mode: Mode) {
        match mode {
            ...
            Mode::SetDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in gameterm's mux
            }
            Mode::ResetDecPrivateMode(DecPrivateMode::Code(
                DecPrivateModeCode::SynchronizedOutput,
            )) => {
            ...
            Mode::QueryDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in gameterm's mux; if we get here, then it isn't enabled,
    ...
    fn checksum_rectangle(&mut self, left: u32, top: u32, right: u32, bottom: u32) -> u16 {
        ...
        */
        ...
        // The concept of uninitialized cells in gameterm is not the same as that on VT520 or that
        ...
        // See: https://github.com/JulianAbeleda/gameterm/pull/4565
        if checksum == 0 {
            32u16
        } else {
            checksum
        ...
        match edit {
            ...
            Edit::EraseInLine(erase) => {
                ...
                let range = match erase {
                    ...
                    // <https://github.com/JulianAbeleda/gameterm/issues/3548>
