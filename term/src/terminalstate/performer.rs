...
impl<'a> Performer<'a> {
    ...
    fn flush_print(&mut self) {
        ...
        for g in Graphemes::new(text) {
            ...
            if print_width == 0 {
                ...
                // Relevant reading:
                // <https://github.com/JulianAbeleda/gameterm/issues/1422>
                // <https://github.com/JulianAbeleda/gameterm/issues/6637>
    ...
    /// <https://github.com/JulianAbeleda/gameterm/issues/2442>
    ...
    fn control(&mut self, control: ControlCode) {
        ...
        match control {
            ...
            // gameterm only supports UTF-8, so does not support the
    ...
    fn csi_dispatch(&mut self, csi: CSI) {
        ...
        match csi {
            ...
            CSI::Cursor(gameterm_escape_parser::csi::Cursor::Left(n)) => {
                // We treat CUB (Cursor::Left) the same as Backspace as
                // that is what xterm does.
                // <https://github.com/JulianAbeleda/gameterm/issues/1273>
                for _ in 0..n {
    ...
    fn osc_dispatch(&mut self, osc: OperatingSystemCommand) {
        ...
        match osc {
            ...
            OperatingSystemCommand::ChangeDynamicColors(first_color, colors) => {
                ...
                use gameterm_escape_parser::osc::DynamicColorNumber;
            ...
            OperatingSystemCommand::ConEmuProgress(prog) => {
                use gameterm_escape_parser::osc::Progress as TProg;
                let prog = match prog {
