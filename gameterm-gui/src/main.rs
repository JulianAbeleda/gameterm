...
#[command(
    ...
    version = config::gameterm_version()
)]
struct Opt {
    /// Skip loading gameterm.lua
    #[arg(long, short = 'n')]
    skip_config: bool,
...
async fn async_run_ssh(opts: SshCommand) -> anyhow::Result<()> {
    ...
    if opts.verbose {
        ssh_option.insert("gameterm_ssh_verbose".to_string(), "true".to_string());
    }
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("GameTerm panic", s);
        }
        default_hook(info);
    }));
}
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("GameTerm Error", &err);
    std::process::exit(1);
}
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use gameterm_font::parser::ParsedFont;
    ...
    let font_config = Rc::new(gameterm_font::FontConfiguration::new(
        Some(config.clone()),
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = gameterm_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by gameterm because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("org.wezfurlong.gameterm").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["gameterm-gui".to_string()];
    ...
    match sub {
        SubCommand::Start(start) => {
            ...
            gameterm_blob_leases::clear_storage();
            res
        }
