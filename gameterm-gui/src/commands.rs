...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let gameterm_perform_key_assignment_sel = sel!(gametermPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    gameterm_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["GameTerm", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "GameTerm" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("GameTerm {}", config::gameterm_version()),
                            Some(gameterm_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::gameterm_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit GameTerm".into(),
            doc: "Quits GameTerm".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://gameterm.org/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the gameterm documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/JulianAbeleda/gameterm/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit gameterm's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/JulianAbeleda/gameterm/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit gameterm's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        ShowTabNavigator,
        // ----------------- Help
        OpenUri("https://gameterm.org/".to_string()),
        OpenUri("https://github.com/JulianAbeleda/gameterm/discussions/".to_string()),
        OpenUri("https://github.com/JulianAbeleda/gameterm/issues/".to_string()),
        ShowDebugOverlay,
        // ----------------- Misc
