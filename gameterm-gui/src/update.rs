use crate::ICON_DATA;
use anyhow::anyhow;
use config::{configuration, gameterm_version};
...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header(
            "User-Agent",
            &format!("gameterm/gameterm-{}", gameterm_version()),
        )
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/gameterm/gameterm/releases/latest")
}

#[allow(unused)]
pub fn get_nightly_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/gameterm/gameterm/releases/tags/nightly")
}
...
pub fn load_last_release_info_and_set_banner() {
    ...
    if let Ok(data) = std::fs::read(update_file_name) {
        ...
        let current = gameterm_version();
        let force_ui = std::env::var_os("GAMETERM_ALWAYS_SHOW_UPDATE_UI").is_some();
...
fn set_banner_from_release_info(latest: &Release) {
    ...
    let url = format!("https://gameterm.org/changelog.html#{}", latest.tag_name);
...
fn update_checker() {
    ...
    loop {
        // Figure out which other gameterm-guis are running.
