...
lazy_static::lazy_static! {
    static ref WINDOW_CLASS: Mutex<String> = Mutex::new(gameterm_gui_subcommands::DEFAULT_WINDOW_CLASS.to_owned());
...
pub enum TermWindowNotif {
    ...
    GetConfigOverrides(Sender<gameterm_dynamic::Value>),
    SetConfigOverrides(gameterm_dynamic::Value),
...
pub struct TermWindow {
    ...
    pub config_overrides: gameterm_dynamic::Value,
...
impl TermWindow {
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} gameterm version: {}",
                    render_info,
...
impl TermWindow {
    pub async fn new_window(mux_window_id: MuxWindowId) -> anyhow::Result<()> {
        ...
        let myself = Self {
            ...
            window_background,
            config: config.clone(),
            config_overrides: gameterm_dynamic::Value::default(),
            palette: None,
            focused: None,
            mux_window_id,
    ...
    fn dispatch_window_event(
        ...
        match event {
            WindowEvent::Destroyed => {
                ...
                // <https://github.com/JulianAbeleda/gameterm/issues/3522>
            ...
            WindowEvent::AppearanceChanged(appearance) => {
                ...
                // <https://github.com/JulianAbeleda/gameterm/issues/2295>
                config::reload();
        ...
        match notif {
            ...
            TermWindowNotif::PerformAssignment {
                ...
                let result = || -> anyhow::Result<()> {
                    ...
                    // <https://github.com/JulianAbeleda/gameterm/issues/3209>
            ...
            TermWindowNotif::MuxNotification(n) => match n {
                ...
                MuxNotification::TabResized(_) => {
                    // Also handled by gameterm-client
                    self.update_title_post_status();
                }
...
impl TermWindow {
    ...
    fn activate_tab(&mut self, tab_idx: isize) -> anyhow::Result<()> {
        ...
        // logic in gameterm/src/main.rs. If you update this, update that!
        let max = window.len();
    ...
    fn get_semantic_prompt_zones(&mut self, pane: &Arc<dyn Pane>) -> &[StableRowIndex] {
        ...
        if cache.seqno != seqno {
            ...
            let mut zones: Vec<StableRowIndex> = zones
                .into_iter()
                .filter_map(|zone| {
                    if zone.semantic_type == gameterm_term::SemanticType::Prompt {
                        Some(zone.start_y)
                    } else {
                        None
                    }
            ...
            // <https://github.com/JulianAbeleda/gameterm/issues/1121>
            zones.dedup();
            cache.zones = zones;
            cache.seqno = seqno;
        }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            OpenUri(link) => {
                gameterm_open_url::open_url(link);
            }
