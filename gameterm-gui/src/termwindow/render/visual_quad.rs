use super::visual_stage_image::*;
use super::visual_vn_panel::*;
...
use gameterm_visual::{
    ...
    VisualRenderEntity, VisualRenderSnapshot, VisualRenderTile, VisualStagePlacement, VisualView,
};
#[cfg(test)]
use gameterm_visual::{
    VisualRenderStageDisplayable, VnOverlayDebugOverrides, VN_OVERLAY_NAMEPLATE_OPACITY,
...
mod tests {
    ...
    fn vn_panel_rects_use_shared_fullscreen_proportions() {
        ...
        assert_eq!(rects.len(), 2);
        let dialogue = rects[1];
        assert!((dialogue.min_x() - 240.0).abs() < 0.1);
        assert!((dialogue.min_y() - 90.0).abs() < 0.1);
        assert!((dialogue.size.width - 1440.0).abs() < 0.1);
