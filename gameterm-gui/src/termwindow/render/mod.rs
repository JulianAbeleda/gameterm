...
fn resolve_fg_color_attr(
    ...
    match fg {
        gameterm_term::color::ColorAttribute::Default => {
        ...
        gameterm_term::color::ColorAttribute::PaletteIndex(idx)
            if idx < 8 && config.bold_brightens_ansi_colors != BoldBrightening::No =>
        {
            // For compatibility purposes, switch to a brighter version
            // of one of the standard ANSI colors when Bold is enabled.
            // This lifts black to dark grey.
            let idx = if attrs.intensity() == gameterm_term::Intensity::Bold {
                idx + 8
            } else {
                idx
            };

            palette.resolve_fg(gameterm_term::color::ColorAttribute::PaletteIndex(idx))
        }
