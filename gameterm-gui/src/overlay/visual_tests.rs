...
use super::visual_command_dispatch::{
    dispatch_pending_action, write_story_state_file, RunCommandDispatch,
};
use super::visual_compose_dock::{SceneComposeAction, SceneComposeDock};
use super::visual_compose_result::{
    apply_compose_backend_result, compose_result_speakable_segments, fake_codex_compose_result,
    sanitize_compose_output, COMPOSE_OUTPUT_LIMIT,
};
use super::visual_dialogue_scroll::{
...
use super::visual_kiki_idle::*;
use super::visual_overlay_session::{SceneComposeDebugBackend, VisualOverlaySession};
use super::visual_render::apply_voice_debug_frame;
...
use gameterm_visual::{
    ...
    VisualRenderSnapshot, VisualResolvedSprite, VisualScene, VisualSceneLoadStatus,
    VisualSceneSource, VisualSpriteManifestStatus, VisualStage, VisualStageDisplayable,
    VisualStageLayer, VisualStagePlacement, VnDialogueScrollMetrics, VnOverlayDebugOverrides,
    VnOverlayRect, VN_OVERLAY_COMPOSER_NAMEPLATE_TEXT_INSET_ROWS,
...
use termwiz::input::{KeyCode, Modifiers, MouseButtons};
...
fn kiki_idle_animation_prefers_blink_during_blink_window() {
    let sprites = kiki_animation_sprite_manifest(KIKI_BREATH_FRAME_COUNT, KIKI_BLINK_FRAME_COUNT);
...
fn dispatch_import_story_state_restores_state_file() {
    ...
    write_story_state_file(&state_path, &serde_json::to_string_pretty(&state).unwrap()).unwrap();
    let mut scene = VisualScene::demo();
    scene.choices = vec![gameterm_visual::SceneAction {
