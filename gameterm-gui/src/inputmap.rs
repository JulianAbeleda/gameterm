...
use gameterm_dynamic::{ToDynamic, Value};
...
impl InputMap {
    ...
    pub fn new(config: &ConfigHandle) -> Self {
        ...
        if !config.disable_default_key_bindings {
            for (mods, code, action) in CommandDef::default_key_assignments(config) {
                ...
                // normalization under X11: <https://github.com/JulianAbeleda/gameterm/issues/1906>.
                ...
                //
                // See: <https://github.com/JulianAbeleda/gameterm/issues/3262>
    ...
    pub fn dump_config(&self, key_table: Option<&str>) {
        println!("local gameterm = require 'gameterm'");
        println!("local act = gameterm.action");
        println!();
        println!("return {{");
