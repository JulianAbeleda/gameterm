...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "GAMETERM_VERSION".to_string(),
            config::gameterm_version().to_string(),
        );
        env.insert(
            "GAMETERM_TARGET_TRIPLE".to_string(),
            config::gameterm_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your gameterm configuration
    #[arg(long)]
