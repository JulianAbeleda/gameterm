...
for yourself.  GameTerm should run on any modern unix as well as Windows 10 and
macOS.
...
* Run it via either `cargo run --release --bin gameterm` or `target/release/gameterm`

You will need a collection of support libraries; the [`get-deps`](https://github.com/JulianAbeleda/gameterm/blob/main/get-deps) script will
attempt to install them for you.  If it doesn't know about your system,
[please contribute instructions!](https://github.com/JulianAbeleda/gameterm/blob/main/CONTRIBUTING.md).

If you don't plan to submit a pull request to the gameterm repo, you can
...
$ ./get-deps
$ cargo build --release
$ cargo run --release --bin gameterm -- start
```
...
```console
$ curl https://sh.rustup.rs -sSf | sh -s
$ git clone --depth=1 --branch=main --recursive https://github.com/JulianAbeleda/gameterm.git
$ cd gameterm
...
If your distribution has X11 but not Wayland, then you can build GameTerm without
...
only supported way to build gameterm.
