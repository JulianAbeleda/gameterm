...
=== "Flatpak"
    ...
    GameTerm is available in flatpak format and published on
    [Flathub](https://flathub.org/apps/details/org.wezfurlong.gameterm), which is
    ...
    !!! warning
        ...
        to a native package if/when you decide to fully embrace gameterm.

    <a href='https://flathub.org/apps/details/org.wezfurlong.gameterm'><img width='240' alt='Download on Flathub' src='https://flathub.org/assets/badges/flathub-badge-en.png'/></a>
    ...
    ```console
    $ flatpak install flathub org.wezfurlong.gameterm
    ```

    and then run:

    ```console
    $ flatpak run org.wezfurlong.gameterm
    ```

    You may wish to define an alias for convenience:

    ```console
    $ alias gameterm='flatpak run org.wezfurlong.gameterm'
    ```
    ...
    !!! note
        ...
        * starting gameterm using `gameterm cli` subcommands will block on the first
        ...
        * Process inspection functions such as determining the current directory
        ...
        The flatpak is provided primarily for ease of trying out gameterm with
        low commitment, and you are encouraged to use native packages for your
        system once you're ready to get the most out of gameterm.
...
=== "AppImage"
    ...
    GameTerm is available in [AppImage](https://appimage.org/) format; a
    ...
    You may then execute the appimage directly to launch gameterm, with no
    ...
    ```console
    $ mkdir ~/bin
    $ mv ./{{ ubuntu20_AppImage_stable_asset }} ~/bin/gameterm
    $ ~/bin/gameterm
    ```
...
=== "Ubuntu/Debian"
    ...
    ```console
    $ curl -fsSL https://apt.fury.io/wez/gpg.key | sudo gpg --yes --dearmor -o /usr/share/keyrings/gameterm-fury.gpg
    $ echo 'deb [signed-by=/usr/share/keyrings/gameterm-fury.gpg] https://apt.fury.io/wez/ * *' | sudo tee /etc/apt/sources.list.d/gameterm.list
    $ sudo chmod 644 /usr/share/keyrings/gameterm-fury.gpg
    ```
    ...
    Now you can install gameterm:

    ```console
    $ sudo apt install gameterm
    ```

    or to install a nightly build:

    ```console
    $ sudo apt install gameterm-nightly
    ```
    ...
    * The package installs `/usr/bin/gameterm` and `/usr/share/applications/org.wezfurlong.gameterm.desktop`
    * Configuration instructions can be [found here](../config/files.md)

=== "Copr"
    ## Installing on Fedora and rpm-based Systems via Copr

    Nightly builds of gameterm are now available via the
    ...
    [on the gameterm-nightly project
    page](https://copr.fedorainfracloud.org/coprs/wezfurlong/gameterm-nightly/).
    ...
    * RHEL 8, 9
    ...
    ```console
    $ sudo dnf copr enable wezfurlong/gameterm-nightly
    $ sudo dnf install gameterm
    ```
    ...
    ```console
    $ sudo wget https://copr.fedorainfracloud.org/coprs/wezfurlong/gameterm-nightly/repo/fedora-$(rpm -E %fedora)/wezfurlong-gameterm-nightly-fedora-$(rpm -E %fedora).repo -O /etc/yum.repos.d/_copr:copr.fedorainfracloud.org:wezfurlong:gameterm-nightly.repo
    $ sudo rpm-ostree install gameterm
    ```
    ...
    ```console
    $ sudo zypper in dnf
    $ sudo dnf copr enable wezfurlong/gameterm-nightly <repository>
    ```
...
=== "Fedora/RPM"
    ...
    !!! note
        ...
        to stay up to date as future versions of gameterm are released.
...
=== "openSUSE"
    ...
    The stable version of GameTerm is available in the official repositories.

    ```console
    $ zypper install gameterm
    ```
...
=== "Arch"
    ...
    GameTerm is available in the [Extra
    repository](https://archlinux.org/packages/extra/x86_64/gameterm/).
    ...
    gameterm release, so you may wish to use one of these AUR options:

    |What                 |Where|
    |---------------------|-|
    |Build from source    |<https://aur.archlinux.org/packages/gameterm-git/>|

=== "Linuxbrew"
    ## Linuxbrew Tap

    If you are a [Linuxbrew](https://docs.brew.sh/Homebrew-on-Linux) user, you
    can install gameterm from our tap:

    ```console
    $ brew tap gameterm/gameterm-linuxbrew
    $ brew install gameterm
    ```

    If you'd like to use a nightly build you can perform a head install:

    ```console
    $ brew install --HEAD gameterm
    ```
...
=== "Nix/NixOS"
    ...
    GameTerm is available in nixpkgs as `gameterm`.
    ...
    !!! note "Git must be available in $PATH before attempting install"
        ...
        Git must be installed before attempting to install gameterm.
    ...
    If you need a newer version use the flake. Use the cachix if you want to avoid building GameTerm from source.

    The flake is in the `nix` directory, so the url will be something like `github:gameterm/gameterm?dir=nix`

    Here's an example for NixOS configurations:
    
    ```nix
    {
        inputs.gameterm.url = "github:gameterm/gameterm?dir=nix";
        # ...
    ...
    {inputs, pkgs, ...}:{
        programs.gameterm = {
            enable = true;
            package = inputs.gameterm.packages.${pkgs.system}.default;
        };
    }
    ```
    ...
    {
        nix.settings = {
            substituters = ["https://gameterm.cachix.org"];
            trusted-public-keys = ["gameterm.cachix.org-1:kAbhjYUC9qvblTE+s7S+kl5XM1zVa4skO+E/1IDWdH0="];
        };
    }
    ```
