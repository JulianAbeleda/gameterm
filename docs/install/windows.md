...
64-bit Windows 10.0.17763 or later is required to run GameTerm; running on
earlier versions of Windows is not possible, as GameTerm requires [Pseudo
...
GameTerm is available in a setup.exe style installer; the installer is produced
with Inno Setup and will install gameterm to your program files directory and
...
GameTerm is also available in a simple zip file that can be extracted and
run from anywhere, including a flash drive for a portable/relocatable
installation.

1. Download <a href="{{ windows_zip_stable }}">Release</a>
2. Extract the zipfile and double-click `gameterm.exe` to run the UI
...
Once you have `winget`, you can install gameterm like so:

```console
$ winget install wez.gameterm
```

and to later upgrade it:

```console
$ winget upgrade wez.gameterm
```
...
$ scoop install gameterm
```

### For `Chocolatey` users

If you prefer to use [Chocolatey](https://chocolatey.org) to manage software,
gameterm is available from the Community Repository.  It can be installed like
so:

```console
$ choco install gameterm -y
```
