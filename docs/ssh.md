gameterm uses an embedded ssh library to provide an integrated SSH client.  The
client can be used to make ad-hoc SSH connections to remote hosts
by invoking the client like this:

```console
$ gameterm ssh wez@my.server
```

(checkout `gameterm ssh -h` for more options).

When invoked in this way, gameterm may prompt you for SSH authentication
...
alternative configuration that connects to a remote gameterm instance
...
gameterm is now able to parse `~/.ssh/config` and `/etc/ssh/ssh_config`
...
a non-zero value will cause gameterm to send an `IGNORE` packet on that interval.
...
`gameterm ssh` CLI allows overriding config settings via the command line.  This
