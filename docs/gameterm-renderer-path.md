...
The render-facing API from `gameterm-visual` should be a snapshot, not another file loader. It should expose scene size, visible tile/entity records, sprite ids, selection flags, and a generation number for cache invalidation.
...
1. Add a render snapshot API to `gameterm-visual`.
2. Thread an optional visual snapshot into `RenderScreenLineParams`.
3. Add visual generation to `LineQuadCacheKey`.
4. Add a small `populate_visual_sprite_quad` beside `populate_image_quad`.
5. Emit only visible tile/entity quads first.
