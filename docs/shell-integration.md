gameterm supports integrating with the shell through the following means:
...
You can find some [examples for various shells in the gameterm
repo](https://github.com/JulianAbeleda/gameterm/tree/main/assets/shell-integration).

To use this file to setup shell integration in gameterm with Bash or Zsh, you can
...
Installing the gameterm shell integration will define the following user vars
by default:

* `GAMETERM_PROG` - the command line being executed by the shell
* `GAMETERM_USER` - holds the output from `id -un`; the current user name
* `GAMETERM_HOST` - holds the output from `hostname`; the hostname that the shell is running on
* `GAMETERM_IN_TMUX` - holds `1` if the shell is running inside tmux, `0` otherwise
...
The shell integration provides a shell function named `__gameterm_set_user_var` which can be
...
the prompt environment in your `.gameterm.lua`; this example configures
...
configure gameterm to launch clink by setting the `default_prog` configuration
in your `.gameterm.lua`; for example, if you have extracted clink to `c:\clink`
