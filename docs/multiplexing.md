...
The out-of-the-box experience with `gameterm` allows you to multiplex local tabs
...
Multiplexing in `gameterm` is based around the concept of *multiplexing domains*;
a domain is a distinct set of windows and tabs.  When gameterm starts up it
...
Once connected to a domain, `gameterm` can attach its windows and tabs to the
...
*gameterm also supports [regular ad-hoc ssh connections](ssh.md).
This section of the docs refers to running a gameterm daemon on the remote end
of a multiplexing session that uses ssh as a channel*

A connection to a remote gameterm multiplexer made via an ssh connection is
referred to as an *SSH domain*.  **A compatible version of gameterm must be
...
the gameterm multiplexer daemon on the remote host and connect to
...
```console
$ gameterm connect SSHMUX:my.server
# or to spawn into a new tab in an existing gameterm gui instance:
$ gameterm cli spawn --domain-name SSHMUX:my.server
```

To customize this functionality, see the example for [gameterm.default_ssh_domains()](config/lua/gameterm/default_ssh_domains.md)
...
-- This causes `gameterm` to act as though it was started as
-- `gameterm connect unix` by default, connecting to the unix
-- domain on startup.
...
```console
$ gameterm connect unix
```

Note that in earlier versions of gameterm, a `connect_automatically` domain
...
config.unix_domains = {
  {
    name = 'unix',
    proxy_command = { 'nc', '-U', '/Users/wez/.local/share/gameterm/sock' },
  },
}
```
...
between the gameterm client and the server exceeds the specified threshold, the
...
user. This option only applies when `multiplexing = "GameTerm"`.
...
Inside your WSL instance, configure `.gameterm.lua` with this snippet:
...
config.unix_domains = {
  {
    ...
    -- container.
    socket_path = '/mnt/c/Users/USERNAME/.local/share/gameterm/sock',
...
config.unix_domains = {
  {
    name = 'wsl',
    serve_command = { 'wsl', 'gameterm-mux-server', '--daemonize' },
  },
}
config.default_gui_startup_args = { 'connect', 'wsl' }
```

Now when you start gameterm you'll be presented with a WSL tab.

You can also omit `default_gui_startup_args` and use:

```console
$ gameterm connect wsl
```
...
Starting with version `20200202-180558-2489abf9`, gameterm can bootstrap a TLS
session by performing an initial connection via SSH to start the gameterm
...
SSH authentication.  Once the connection has been initiated, gameterm
