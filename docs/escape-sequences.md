...
>
> If you notice that something is inaccurate or missing, please do [file an issue](https://github.com/JulianAbeleda/gameterm/issues/new/choose)
> so that it can be resolved!

## Output/Escape Sequences

GameTerm considers the output from the terminal to be a UTF-8 encoded stream of
...
GameTerm will attempt to apply your [font shaping](config/font-shaping.md) configuration
...
| ^G |0x07|BEL |Bell       |Logs `Ding! (this is the bell)` to stderr of the GameTerm process. See [#3](https://github.com/JulianAbeleda/gameterm/issues/3)|
...
As mentioned above, GameTerm only supports UTF-8 encoding.  C1 control codes
...
GameTerm classifies these sequences into a number of functional families which
...
parameters be separated by colons (`:`), but for compatibility reasons GameTerm
...
`GameTerm` and most (all?) other terminal emulators:
...
This is a gameterm extension: gameterm considers color mode `6` as RGBA,
...
GameTerm supports [Synchronized Rendering](https://gist.github.com/christianparpart/d8a62cc1ab659194337d73e399004036).
...
|DCS \[PARAMS\] q \[DATA\] ST | Sixel Graphic Data | Decodes [Sixel graphic data](https://vt100.net/docs/vt3xx-gp/chapter14.html) and apply the image to the terminal model. Support is preliminary and incomplete; see [this issue](https://github.com/JulianAbeleda/gameterm/issues/217) for status. |
|DCS 1000 q | tmux control mode | Bridges tmux into the GameTerm multiplexer.  Currently incomplete, see [this issue](https://github.com/JulianAbeleda/gameterm/issues/336) for status. |
...
are canonically delimited by the `ST` (String Terminator) sequence, but GameTerm
