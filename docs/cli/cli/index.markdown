...
The *cli* subcommand interacts with a running gameterm GUI or multiplexer
...
gameterm uses the following logic to decide which one it should connect to.

* If the `--prefer-mux` flag is passed, then the `gameterm.lua` config file is
  consulted to determine the first *unix domain* defined by the config.
* If the `$GAMETERM_UNIX_SOCKET` environment variable is set, use that location
...
* If the `$GAMETERM_PANE` environment variable is set, it will be used
...
See also: [gameterm cli list](list.md)
