# `gameterm cli split-pane`

*Run `gameterm cli split-pane --help` to see more help*
...
to `gameterm cli split-pane` so that any parameters you may wish to pass to the
program are not confused with parameters to `gameterm cli split-pane`.  This example
launches bash as a login shell in a new pane at the bottom:

```
$ gameterm cli split-pane -- bash -l
3
```
...
```
$ gameterm cli split-pane --left --percent 30
4
```
...
```console
{% include "../../examples/cmd-synopsis-gameterm-cli-split-pane--help.txt" %}
```
