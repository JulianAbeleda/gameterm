GameTerm provides a searchable scrollback buffer with a configurable maximum
...
You can control whether GameTerm displays a scrollbar via your configuration
file:
...
When the search overlay is active the behavior of gameterm changes:
...
to obtain the defaults and extend them. In earlier versions of gameterm there
...
may be more recent than your version of gameterm) is shown below.

You can see the configuration in your version of gameterm by running
`gameterm show-keys --lua --key-table search_mode`.
...
config.keys = {
  -- search for things that look like git hashes
  {
    key = 'H',
    mods = 'SHIFT|CTRL',
    action = gameterm.action.Search { Regex = '[a-f0-9]{6,}' },
  },
}
```
