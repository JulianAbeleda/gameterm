...
*Note: you can run `gameterm show-keys` to show the effective key and mouse assignments*.
...
NOTE: In the action column, `act` is an alias to `gameterm.action` (to avoid repetition):
