...
GameTerm ships with over 700 color schemes available from
...
[gameterm.gui.get_appearance()](lua/gameterm.gui/get_appearance.md)
...
you need to use [gameterm.color.get_default_colors()](lua/gameterm.color/get_default_colors.md) and explicitly merge them.
...
### Defining a Color Scheme in your `.gameterm.lua`
...
Color schemes names that you define in your `gameterm.lua` take precedence
...
See also [gameterm.get_builtin_color_schemes()](lua/gameterm/get_builtin_color_schemes.md) for
...
the available color schemes for an example](https://github.com/JulianAbeleda/gameterm/tree/main/config/src/scheme_data.rs).

It is recommended that you place your custom scheme in a directory
named `$HOME/.config/gameterm/colors` if you're on a POSIX system.

On a Windows system, `gameterm` will search for schemes in a directory
named `colors` that is in the same directory as the `gameterm.exe`.

If you wish to place your color scheme files in some other location, then you
will need to instruct gameterm where to look for your scheme files; the
...
```
  ...
  <video width="80%" controls src="../screenshots/gameterm-dynamic-colors.mp4" loop></video>
...
config.window_frame = {
  ...
  font = gameterm.font { family = 'Roboto', weight = 'Bold' },
...
![Screenshot](../screenshots/gameterm-vday-screenshot.png)

{{since('20201031-154415-9614e117')}}

You can attach an image to the background of the gameterm window:
...
to the directory containing your `gameterm.lua` config file.
...
If your Operating System provides Compositing support then GameTerm is able to
