`gameterm` allows assigning action(s) to specific key events, and comes
...
* *Physical Key* - a way to identify a key based on its hardware-dependent location. `gameterm` can refer to keys based on code they would emit if configured to use an ANSI US English keyboard layout (even if that layout is not currently active), or based on its raw scan code.
...
And then some gameterm concepts:

* *Key Assignment* - an action assigned to a matching key and modifier combination.
* *Key Table* - a grouping of key assignments. For each window, `gameterm` maintains a stack of table activations, allowing for rich modal keyboard input customization


## Keyboard Processing Flow

This schematic depicts the processing flow for keyboard events in `gameterm`:
...
a concept.  GameTerm tries to behave reasonably by default, but also give you
...
gameterm will respect the composition effects of AltGr produced by the system.
...
GameTerm is now able to perform dead-key expansion when `use_ime = false`.  Dead
...
GameTerm has support for using the operating system Input Method Editor (IME) on
...
a dead key in gameterm will "hold" the dead key until the next character is
...
You can tell GameTerm to disable dead keys by setting this in your configuration
file:
...
`gameterm` to prevent it from doing that, short of disabling the IME.

### Defining Assignments for key combinations that may be composed

When a key combination produces a composed key result, gameterm will look up
