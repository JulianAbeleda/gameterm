...
`keys` section in your `~/.gameterm.lua` config file.  For example, you can
...
config.keys = {
  ...
  -- be potentially recognized and handled by the tab
  {
    key = 'm',
    mods = 'CMD',
    action = gameterm.action.DisableDefaultAssignment,
  },
}
```
 ...
 * `LEADER` - a special modal modifier state managed by `gameterm`. See [Leader Key](#leader-key) for more information.
...
If you omit an explicit prefix, gameterm will assume `phys:` and use the
...
In some cases, `gameterm` may not know how to represent a key event in either
...
config.keys = {
  {
    key = '|',
    mods = 'LEADER|SHIFT',
    action = gameterm.action.SplitHorizontal { domain = 'CurrentPaneDomain' },
  },
  -- Send "CTRL-A" to the terminal when pressing CTRL-A, CTRL-A
  {
    key = 'a',
    mods = 'LEADER|CTRL',
    action = gameterm.action.SendKey { key = 'a', mods = 'CTRL' },
  },
}
```
...
config.keys = {
  ...
  },
  {
    key = '-',
    mods = 'LEADER',
    action = gameterm.action.SplitVertical { domain = 'CurrentPaneDomain' },
  },
}
```
