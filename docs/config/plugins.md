...
<!-- See also https://github.com/wez/gameterm/commit/e4ae8a844d8feaa43e1de34c5cc8b4f07ce525dd -->
...
!!! Tip
    ...
    Michael Brusegard maintains a [list of plugins](https://github.com/michaelbrusegard/awesome-gameterm)
...
local a_plugin = gameterm.plugin.require 'https://github.com/owner/repo'
...
Run the command [`gameterm.plugin.update_all()`](lua/gameterm.plugin/update_all.md) to update all local plugins.
...
When a plugin is first referenced, [`gameterm.plugin.require()`](lua/gameterm.plugin/require.md) will clone the repo if it doesn't already
exist and store it in the runtime directory under `plugins/NAME` where
`NAME` is derived from the repo URL.

You can discover locations of the various plugins with [`gameterm.plugin.list()`](lua/gameterm.plugin/list.md).
...
5. Add the plugin using a local file url e.g.
   ```lua
   local a_plugin = gameterm.plugin.require "file:///home/user/projects/myPlugin"
   ```

!!! Info
    When changes are made to the local project, [`gameterm.plugin.update_all()`](lua/gameterm.plugin/update_all.md) must be run
...
`gameterm.plugin.list()`. This function returns an array of triplets. e.g.

```
[
    {
        "component": "filesCssZssZssZsUserssZsdevelopersZsprojectssZsmysDsPlugin",
        "plugin_dir": "/Users/alec/Library/Application Support/gameterm/plugins/filesCssZssZssZsUserssZsalecsZsprojectssZsbarsDsgameterm",
...
function findPluginPackagePath(myProject)
  ...
  for _, v in ipairs(gameterm.plugin.list()) do
