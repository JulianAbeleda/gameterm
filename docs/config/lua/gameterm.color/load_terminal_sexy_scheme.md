# `gameterm.color.load_terminal_sexy_scheme(file_name)`

{{since('20220807-113146-c2fee766')}}

Loads a json file exported from [terminal.sexy](https://terminal.sexy/)
and returns it as a gameterm color scheme.

Note that gameterm ships with all of the pre-defined terminal.sexy color
schemes, so this function is primarily useful if you want to design a color
scheme using terminal.sexy and then import it to gameterm.
...
```
> colors, metadata = gameterm.color.load_terminal_sexy_scheme("/path/to/file.json")
> print(colors)
