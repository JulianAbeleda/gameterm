...
the `gameterm.lua` file, but represents the fully-populated state of the
...
gameterm.on('show-font-size', function(window, pane)
  gameterm.log_error(window:effective_config().font_size)
end)
  ...
  keys = {
    {
      key = 'E',
      mods = 'CTRL',
      action = gameterm.action.EmitEvent 'show-font-size',
    },
  },
}
```
