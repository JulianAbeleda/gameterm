...
gameterm.on('toggle-ligature', function(window, pane)
  ...
  keys = {
    {
      key = 'E',
      mods = 'CTRL',
      action = gameterm.action.EmitEvent 'toggle-ligature',
    },
  },
}
```
...
gameterm.on('toggle-opacity', function(window, pane)
  local overrides = window:get_config_overrides() or {}
  ...
  keys = {
    {
      key = 'B',
      mods = 'CTRL',
      action = gameterm.action.EmitEvent 'toggle-opacity',
    },
  },
}
```
