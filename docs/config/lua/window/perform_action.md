...
The first parameter is a key assignment such as that returned by [`gameterm.action`](../gameterm/action.md).

The second parameter is a `pane` object passed to your event callback.

For an example of this method in action, see [`gameterm.on` Custom Events](../gameterm/on.md#custom-events).
