...
**NOTE: You probably want to use [gameterm.gui.get_appearance()](../gameterm.gui/get_appearance.md) instead, as it is easier to use!**
...
gameterm is able to detect when the appearance has changed and will generate a
...
appearance, as gameterm didn't know how to interrogate the appearance on Wayland
...
function query_appearance_gnome()
  local success, stdout = gameterm.run_child_process {
    'gsettings',
    'get',
...
Since GameTerm will not fire a `window-config-reloaded` event on Wayland for
older versions of gameterm, you will instead need to listen on the
