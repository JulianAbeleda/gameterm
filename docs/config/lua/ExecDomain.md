...
gameterm to explicitly spawn a command that runs `docker exec`, you would also
...
You must use the `gameterm.exec_domain` function to define a domain. It accepts
the following parameters:

```
gameterm.exec_domain(NAME, FIXUP [, LABEL])
```
...
```lua
gameterm.exec_domain('myname', function(cmd)
  return cmd
end)
```
...
what gameterm will execute in order to satisfy the user's request to
spawn a new program.
...
-- gameterm.exec_domain()
gameterm.exec_domains(domain_name, fixup_func, function(domain_name)
  return domain_name
end)
```
...
[gameterm.format()](gameterm/format.md) to manage that.
...
config.exec_domains = {
  ...
  -- killed.
  gameterm.exec_domain('scoped', function(cmd)
    -- The "cmd" parameter is a SpawnCommand object.
    -- You can log it to see what's inside:
    gameterm.log_info(cmd)

    -- Synthesize a human understandable scope name that is
    -- (reasonably) unique. GAMETERM_PANE is the pane id that
    -- will be used for the newly spawned pane.
    -- GAMETERM_UNIX_SOCKET is associated with the gameterm
    -- process id.
    local env = cmd.set_environment_variables
    local ident = 'gameterm-pane-'
      .. env.GAMETERM_PANE
      .. '-on-'
      .. basename(env.GAMETERM_UNIX_SOCKET)
    ...
    local wrapped = {
      ...
      '--user',
      '--scope',
      '--description=Shell started by gameterm',
      '--same-dir',
      '--collect',
      '--unit=' .. ident,
    }
...
-- spawned by gameterm will have its own scope
...
function docker_list()
  local docker_list = {}
  local success, stdout, stderr = gameterm.run_child_process {
    'docker',
    'container',
    'ls',
    '--format',
  ...
  for _, line in ipairs(gameterm.split_by_newlines(stdout)) do
