...
live resize operation.  `gameterm` will coalesce the stream of multiple events
...
gameterm.on('window-resized', function(window, pane)
  recompute_padding(window)
end)

gameterm.on('window-config-reloaded', function(window)
  recompute_padding(window)
end)
```
