...
encodes the default, built-in action that gameterm will take.  It may be `nil`
in the case where gameterm would not take any action.

You may take any action you wish in this event handler.

If you return `false` then you will prevent gameterm from carrying out its
default action.

Otherwise, gameterm will proceed to perform that action once your event
handler returns.
...
gameterm.on(
  ...
  function(window, pane, button, default_action)
    -- just log the default action and allow gameterm to perform it
    gameterm.log_info('new-tab', window, pane, button, default_action)
  end
)
```
...
gameterm.on(
  ...
  function(window, pane, button, default_action)
    ...
    -- and tell gameterm that we handled the event so that it doesn't
