...
[gameterm.run_child_process](../gameterm/run_child_process.md)) are not possible
...
-- or `gameterm cli set-tab-title`, but falls back to the
...
* a table holding `FormatItem`s as used in the [gameterm.format](../gameterm/format.md) function.  This allows formatting style and color information for individual elements within the tab.
...
to [tab_max_width](../config/tab_max_width.md).  GameTerm will then compute
...
`gameterm.on("format-tab-title", ...)` calls.
...
local SOLID_LEFT_ARROW = gameterm.nerdfonts.pl_right_hard_divider

-- The filled in variant of the > symbol
local SOLID_RIGHT_ARROW = gameterm.nerdfonts.pl_left_hard_divider
...
gameterm.on(
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    title = gameterm.truncate_right(title, max_width - 2)
