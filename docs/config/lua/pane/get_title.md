...
Returns the title of the pane.  This will typically be `gameterm` by default but
...
If the title text is `gameterm` and the pane is a local pane, then gameterm will
attempt to resolve the executable path of the foreground process that is
associated with the pane and will use that instead of `gameterm`.
