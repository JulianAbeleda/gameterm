...
also recognized by gameterm; this example sets the `foo` user variable
to the value `bar`:
...
__gameterm_set_user_var "foo" "bar"
```

you're then able to access this in your gameterm config:

```lua
gameterm.log_info('foo var is ' .. pane:get_user_vars().foo)
```
