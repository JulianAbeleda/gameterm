...
* `4`: set paused state, pr is optional. *Not supported by GameTerm*

When the progress OSC is processed, the state is captured, and if it has
changed, GameTerm will trigger an update of the tab bar, which will in turn
cause events such as
[format-tab-title](../window-events/format-tab-title.md) to trigger.

The progress information is not directly used by gameterm, but you can access it
...
local PCT_GLYPHS = {
  gameterm.nerdfonts.md_circle_slice_1,
  gameterm.nerdfonts.md_circle_slice_2,
  gameterm.nerdfonts.md_circle_slice_3,
  gameterm.nerdfonts.md_circle_slice_4,
  gameterm.nerdfonts.md_circle_slice_5,
  gameterm.nerdfonts.md_circle_slice_6,
  gameterm.nerdfonts.md_circle_slice_7,
  gameterm.nerdfonts.md_circle_slice_8,
}
  ...
  function(tab, tabs, panes, config, hover, max_width)
    ...
    if progress ~= 'None' then
      ...
      elseif progress == 'Indeterminate' then
        ...
        status = gameterm.serde.json_encode(progress)
      end
...
return gameterm.config_builder()
```
