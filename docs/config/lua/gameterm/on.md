...
tags:
 ...
 - event
---
# `gameterm.on(event_name, callback)`

{{since('20201031-154415-9614e117')}}

This function follows the html/javascript naming for defining event handlers.

`gameterm.on` causes your specified `callback` to be called when `event_name`
is emitted.  Events can be emitted by gameterm itself, or through code/configuration
that you specify.

`gameterm.on` can register multiple callbacks for the same event; internally
...
See [gameterm.action_callback](./action_callback.md) for a helper to define a custom action callback.
...
You may register handlers for arbitrary events for which gameterm itself
has no special knowledge.  It is recommended that you avoid event names
that are likely to be used future versions of gameterm in order to avoid
unexpected behavior if/when those names might be used in future.

The [gameterm.emit](emit.md) function and the [EmitEvent](../keyassignment/EmitEvent.md) key assignment can be used to
emit events.
...
gameterm.on('trigger-vim-with-scrollback', function(window, pane)
