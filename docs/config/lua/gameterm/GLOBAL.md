...
gameterm's lua files may be re-loaded and re-evaluated multiple times in
different contexts or in different threads. If you'd like to keep track
of state that lasts for the lifetime of your gameterm process then you
cannot simply use global variables in the lua script.

`gameterm.GLOBAL` is a special userdata value that acts like a table.
Writing to keys will copy the data that you assign into a global in-memory
table and allow it to be read back later.

Reads and writes from/to `gameterm.GLOBAL` are thread-safe but don't currently
...
gameterm.GLOBAL.parse_count = (gameterm.GLOBAL.parse_count or 0) + 1

gameterm.on('update-right-status', function(window, pane)
  window:set_right_status('Reloads=' .. tostring(gameterm.GLOBAL.parse_count))
end)
...
Accessing `gameterm.GLOBAL` returns a copy of the data that you'd read, making
it cumbersome to perform read/modify/write updates; for example this:

```lua
gameterm.GLOBAL.tab_titles['T0'] = 'Test'
```

will not result in `gameterm.GLOBAL.tab_titles.T0` being set, and you need
to explicitly break it apart into:

```lua
local tab_titles = gameterm.GLOBAL.tab_titles
tab_titles['T0'] = 'Test'
gameterm.GLOBAL.tab_titles = tab_titles
```
