...
`gameterm.format` can be used to produce a formatted string
with terminal graphic attributes such as bold, italic and colors.
The resultant string is rendered into a string with gameterm
compatible escape sequences embedded.

`gameterm.format` accepts a single array argument, where each
element is a `FormatItem`.

This example logs the text `Hello`, then the date/time, underlined, in purple
text on a blue background to the stderr of the gameterm process:

```lua
local gameterm = require 'gameterm'

local success, date, stderr = gameterm.run_child_process { 'date' }

gameterm.log_info(gameterm.format {
