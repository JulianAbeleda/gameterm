---
title: gameterm.font_with_fallback
tags:
 - font
---

# `gameterm.font_with_fallback(families [, attributes])`
...
return {
  font = gameterm.font_with_fallback { 'JetBrains Mono', 'Noto Color Emoji' },
}
```

GameTerm implicitly adds its default fallback to the list that you specify.

The *attributes* parameter behaves the same as that of [gameterm.font](font.md)
...
= `true` will cause gameterm to try to automatically scale using the
