---
title: gameterm.config_builder
tags:
 - utility
---

# gameterm.config_builder()
...
local config = {}
if gameterm.config_builder then
  config = gameterm.config_builder()
end
...
When evaluated by earlier versions of gameterm, this config will produce the
...
```
11:44:11.668  WARN   gameterm_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
```

When using the config builder, the warning message is improved:

```
11:45:23.774  WARN   gameterm_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
...
The consequence of an error is that gameterm will show a configuration error
