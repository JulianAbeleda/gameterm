...
to remain the same across different versions of gameterm.
...
conversions of the parameters passed to [gameterm.log_info](log_info.md).

```lua
local gameterm = require 'gameterm'
assert(gameterm.to_string { 1, 2 } == [=[[
    1,
    2,
]]=])
assert(gameterm.to_string { a = 1, b = 2 } == [[{
    "a": 1,
    "b": 2,
}]])
```
