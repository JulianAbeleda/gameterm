...
# `gameterm.font(family [, attributes])`
...
return {
  font = gameterm.font 'JetBrains Mono',
}
```
...
oblique) for non-bitmap fonts, gameterm can only select and use fonts that you
...
return {
  font = gameterm.font('JetBrains Mono', { weight = 'Bold' }),
}
```

When resolving fonts from [font_dirs](../config/font_dirs.md), gameterm follows CSS Fonts
...
[gameterm.font_with_fallback](font_with_fallback.md) when you want to specify precise
