---
title: gameterm.get_builtin_color_schemes
tags:
 - color
 - scheme
 - theme
...
# `gameterm.get_builtin_color_schemes()`
...
couple of entries just from your `gameterm.lua` configuration file.

This example shows how to make gameterm pick a random color scheme for
...
local schemes = {}
for name, scheme in pairs(gameterm.get_builtin_color_schemes()) do
  table.insert(schemes, name)
end

gameterm.on('window-config-reloaded', function(window, pane)
...
local scheme = gameterm.get_builtin_color_schemes()['Gruvbox Light']
...
local function dark_schemes()
  local schemes = gameterm.get_builtin_color_schemes()
  local dark = {}
  for name, scheme in pairs(schemes) do
    -- parse into a color object
    local bg = gameterm.color.parse(scheme.background)
...
[gameterm.color.get_builtin_schemes()](../gameterm.color/get_builtin_schemes.md)
but can still be called as `gameterm.get_builtin_color_schemes()`. See that page
for more examples.
