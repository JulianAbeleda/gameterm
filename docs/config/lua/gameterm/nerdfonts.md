...
status lines and titles in your gameterm configuration.

GameTerm includes `Nerd Font Symbols Font` as a default font fallback
which means that these special symbols are available even without 
requiring you to use a patched font.

This example shows how to lookup `gameterm.nerdfonts.fa_clock_o`, the Font
...
gameterm.on('update-right-status', function(window, pane)
  ...
  window:set_right_status(gameterm.format {
    { Text = gameterm.nerdfonts.fa_clock_o .. ' ' .. date },
  })
end)
```
