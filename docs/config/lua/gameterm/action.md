...
`gameterm.action` is a special enum constructor type that makes it bit
...
Indexing `gameterm.action` with a valid
...
```
gameterm.action.QuickSelectArgs
```
  ...
  keys = {
    {
      key = ' ',
      mods = 'CTRL|SHIFT',
      action = gameterm.action.QuickSelectArgs,
    },
  },
}
```
