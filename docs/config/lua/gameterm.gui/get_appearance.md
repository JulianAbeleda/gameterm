...
gameterm is able to detect when the appearance has changed and will reload the
...
-- gameterm.gui is not available to the mux server, so take care to
-- do something reasonable when this config is evaluated by the mux
function get_appearance()
  if gameterm.gui then
    return gameterm.gui.get_appearance()
  end
  return 'Dark'
end
