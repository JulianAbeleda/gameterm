...
config.ssh_domains = {
  {
    ...
    -- remote_gameterm_path = "/home/yourusername/bin/gameterm"
  },
}
```
...
* `"GameTerm"` - this is the default; use gameterm's multiplexing client.
  Having gameterm installed on the server is required to use this mode.
* `"None"` - don't use any multiplexing. The connection is an ssh connection
  using the same mechanism as is used by `gameterm ssh`; losing connectivity
...
gameterm to assume that the remote host uses a specific shell command language
dialect, which in turn allows gameterm to respect the
