# `gameterm.time.call_after(interval_seconds, function)`
...
gameterm.time.call_after(60, function()
  gameterm.reload_configuration()
end)

local amount =
  math.ceil((tonumber(gameterm.time.now():format '%M') / 60) * 255)
