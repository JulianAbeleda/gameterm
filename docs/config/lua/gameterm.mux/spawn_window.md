...
```lua
local tab, pane, window = gameterm.mux.spawn_window {}
```
...
```lua
gameterm.mux.spawn_window { args = { 'top' } }
```
...
```lua
gameterm.mux.spawn_window { cwd = '/tmp' }
```
...
```lua
gameterm.mux.spawn_window { set_environment_variables = { FOO = 'BAR' } }
```
...
```lua
gameterm.mux.spawn_window { domain = { DomainName = 'my.name' } }
```
...
```lua
gameterm.mux.spawn_window { width = 60, height = 30 }
```
...
```lua
gameterm.mux.spawn_window { workspace = { 'coding' } }
```
...
gameterm.mux.spawn_window {
  position = {
    ...
    -- * {Named="HDMI-1"} - uses a screen by name. See gameterm.gui.screens()
    -- origin = "ScreenCoordinateSystem"
  },
}
```

See also [gameterm.gui.screens()](../gameterm.gui/screens.md)
