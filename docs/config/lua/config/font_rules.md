...
attributes, gameterm uses `font_rules` to decide how to render that text.

By default, unstyled text will use the font specified by the [font](font.md)
configuration, and gameterm will use that as a base, and then automatically
...
```lua
config.font = gameterm.font_with_fallback 'Operator Mono SSm Lig Medium'
config.font_rules = {
...
```lua
config.font = gameterm.font { family = 'FiraCode' }
...
You can run `gameterm ls-fonts` to summarize the font rules and the fonts that
match them:
