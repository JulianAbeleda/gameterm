...
When set to `true` (the default), GameTerm will compute its own idea of what the glyphs
...
|[Git Branch Symbols](https://github.com/JulianAbeleda/gameterm/issues/6328)|Custom branch drawing symbols for rendering DAGs such as Git branch structure|{{since('nightly', inline=True)}}|
