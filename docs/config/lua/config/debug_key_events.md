...
level log message on the stderr stream from gameterm.  **You will typically need
to launch `gameterm` directly from another terminal to see this logging**.
...
```
 2021-02-20T17:04:28.149Z INFO  gameterm_gui::gui::termwindow   > key_event 
   KeyEvent { key: Char('l'), modifiers: NONE, raw_key: None,
   raw_modifiers: NONE, raw_code: Some(46), repeat_count: 1, key_is_down: true }
 2021-02-20T17:04:28.605Z INFO  gameterm_gui::gui::termwindow   > key_event
...
* `key_is_down` indicates whether the key is being pressed or released. This
  ...
  only triggered on key press events in gameterm.
