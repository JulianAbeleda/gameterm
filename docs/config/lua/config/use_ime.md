...
|X11       |20211204-082213-a66c61ee9|[XIM](https://en.wikipedia.org/wiki/X_Input_Method) based. Your system needs to have a running input method engine (such as ibus or fcitx) that support the XIM protocol in order for gameterm to use it.|
...
Changing `use_ime` usually requires re-launching GameTerm to take full effect.
...
option is set appropriately before gameterm is launched!  For
