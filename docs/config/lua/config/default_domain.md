...
When starting the GUI (not using the `serial` or `connect` subcommands), by default gameterm will set the built-in `"local"` domain as the default multiplexing domain.
...
the default domain to instead use a particular WSL distribution so that gameterm
...
that gameterm can then use [shell integration](../../../shell-integration.md) to
...
then gameterm will by default create a `WslDomain` with the name `"WSL:Ubuntu-18.04"`
...
then when gameterm starts up, it will open with a shell running inside that Ubuntu
