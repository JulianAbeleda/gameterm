...
If you want to get the most application support out of gameterm, then you may
wish to install a copy of the `gameterm` TERM definition:

```console
$ tempfile=$(mktemp) \
  && curl -o $tempfile https://raw.githubusercontent.com/gameterm/gameterm/main/termwiz/data/gameterm.terminfo \
  && tic -x -o ~/.terminfo $tempfile \
  && rm $tempfile
```

You can then set `term = "gameterm"` in your `.gameterm.lua` config file.
...
you are using has a relatively outdated ncurses installation, the `gameterm`
terminfo will also enable italics and true color support.

If you are using WSL, gameterm will automatically populate `WSLENV` to properly set TERM, COLORTERM, TERM_PROGRAM and TERM_PROGRAM_VERSION in the environment when crossing between win32 and WSL environments.
...
The following snippet works if you installed `gameterm.terminfo` with nix into your user profile. Update the path to `TERMINFO_DIRS` to match the location on your system.
