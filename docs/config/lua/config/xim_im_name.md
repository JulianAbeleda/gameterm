...
Explicitly set the name of the IME server to which gameterm will connect
via the XIM protocol when using X11 and [use_ime](use_ime.md) is `true`.

By default, this option is not set which means that gameterm will consider
...
will cause gameterm to connect to fcitx regardless of the value of `XMODIFIERS`.
