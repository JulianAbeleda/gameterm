...
When set to `true` (the default), gameterm will configure the `SSH_AUTH_SOCK`
...
by running `gameterm cli list-clients` and inspecting the `SSH_AUTH_SOCK`
column.

The symlink is updated within (at the time of writing this documentation) 100ms
of the active Mux client changing.

You can set `mux_enable_ssh_agent = false` to prevent gameterm from assigning
