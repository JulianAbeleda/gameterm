...
When launching the GUI using either `gameterm` or `gameterm-gui` (with no
subcommand explicitly specified), gameterm will use the value of
`default_gui_startup_args` to pick a default mode for running the GUI.

The default for this config is `{"start"}` which makes `gameterm` with no
additional subcommand arguments equivalent to `gameterm start`.

If you know that you always want to use gameterm's ssh client to login to a
...
which will cause `gameterm` with no additional subcommand arguments to be
equivalent to running `gameterm ssh some-host`.
