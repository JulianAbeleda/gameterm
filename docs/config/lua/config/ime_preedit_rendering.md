...
GameTerm supports the following IME preedit rendering.

* `"Builtin"` - (Default) IME preedit is rendered by GameTerm itself
...
* Changing `ime_preedit_rendering` usually requires re-launching GameTerm to take full effect.
* In macOS, `ime_preedit_rendering` has effected nothing yet.
  IME preedit is always rendered by GameTerm itself.
