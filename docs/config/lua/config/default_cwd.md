...
[gameterm.home_dir](../gameterm/home_dir.md) to explicitly refer to your home
directory.

If `gameterm start --cwd /some/path` is used to specify the current working
...
On macOS and Linux, `gameterm` can attempt to resolve the process group leader
...
On Windows, there isn't a process group leader concept, but `gameterm` will
