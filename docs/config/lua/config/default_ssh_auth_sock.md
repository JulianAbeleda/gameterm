...
Setting this value will cause gameterm to replace the the value of the
`SSH_AUTH_SOCK` environment when it first starts up, and to use this value for
the auth socket registered with the multiplexer server (visible via `gameterm
cli list-clients`).
...
The following gameterm configuration snippet shows how to detect when gnome
...
then
  local onep_auth =
    string.format('%s/.1password/agent.sock', gameterm.home_dir)
  ...
  -- would be 0
  if #gameterm.glob(onep_auth) == 1 then
