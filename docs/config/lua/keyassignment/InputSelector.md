...
* `choices` - a lua table consisting of the potential choices. Each entry
  ...
  with [gameterm.format](../gameterm/format.md) to produce styled text.
* `action` - and event callback registered via `gameterm.action_callback`.  The
...
config.keys = {
  {
    ...
    action = act.InputSelector {
      action = gameterm.action_callback(function(window, pane, id, label)
        if not id and not label then
          gameterm.log_info 'cancelled'
        else
          gameterm.log_info('you selected ', id, label)
          pane:send_text(id)
        end
      end),
      title = 'I am title',
...
config.keys = {
  {
    key = 'R',
    mods = 'CTRL|SHIFT',
    action = gameterm.action_callback(function(window, pane)
...
config.keys = {
  {
    ...
    action = gameterm.action_callback(function(window, pane)
      ...
      window:perform_action(
        act.InputSelector {
          action = gameterm.action_callback(
            function(inner_window, inner_pane, id, label)
              if not id and not label then
                gameterm.log_info 'cancelled'
              else
                gameterm.log_info('id = ' .. id)
                gameterm.log_info('label = ' .. label)
