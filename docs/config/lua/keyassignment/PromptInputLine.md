...
* `description` - the text to show at the top of the display area. You may
  embed escape sequences and/or use [gameterm.format](../gameterm/format.md).
...
* `prompt` - the text to show as the prompt. You may embed escape sequences
  and/or use [gameterm.format](../gameterm/format.md).  Defaults to: `"> "`. {{since('nightly', inline=True)}}
...
config.keys = {
  {
    ...
    action = act.PromptInputLine {
      ...
      action = gameterm.action_callback(function(window, pane, line)
...
This example also shows the use of `gameterm.format` to emit colored text.
