...
* `message` - the text to show for confirmation. You may embed
  escape sequences and/or use [gameterm.format](../gameterm/format.md).
  Defaults to: `"🛑 Really continue?"`.
* `action` - event callback registered via `gameterm.action_callback`.  The
  ...
  user selects `Yes`.
* `cancel` - event callback registered via `gameterm.action_callback`.  The
  callback's function signature is `(window, pane)` where `window` and
  `pane` are the [Window](../window/index.md) and [Pane](../pane/index.md).
...
config.keys = {
  {
    ...
    action = act.Confirmation {
      ...
      cancel = gameterm.action_callback(function(window, pane)
        gameterm.log_error 'user declined'
      end),
    },
  },
}
