...
selected domain.  For example, when you use `gameterm connect DOMAIN` or
`gameterm start --domain DOMAIN` to start the GUI, the `gui-attached` event will
...
Note that the `gui-startup` event does not fire when invoking `gameterm connect
DOMAIN` or `gameterm start --domain DOMAIN --attach`.
...
gameterm.on('gui-attached', function(domain)
