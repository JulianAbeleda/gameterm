...
when running the `gameterm start` subcommand.

It is triggered before any default program is started.

If no explicit program was passed to `gameterm start`, and if the
...
This event does not fire for `gameterm connect` invocations.

{{since('20220807-113146-c2fee766')}}

The event receives an optional [SpawnCommand](../SpawnCommand.md) argument that
corresponds to any arguments that may have been passed via `gameterm start`.
In earlier releases if you implemented this event, you would essentially
prevent `gameterm start -- something` from spawning `something`.
...
local mux = gameterm.mux
local config = {}

gameterm.on('gui-startup', function(cmd)
...
gameterm.on('gui-startup', function(cmd)
  -- allow `gameterm start -- something` to affect what we spawn
  ...
  local project_dir = gameterm.home_dir .. '/gameterm'
...
* [gameterm.mux](../gameterm.mux/index.md)
