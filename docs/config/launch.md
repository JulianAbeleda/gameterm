...
=== "On Posix Systems"
    ...
    ignored_** in order for gameterm to continue to be functional without
    restarting after the user changes their shell.

    gameterm will set the `$SHELL` environment variable to the shell that it
    ...
    Older versions of gameterm (circa 2022 and earlier) used slightly
...
=== "On Windows Systems"
    ...
    1. The value of the `%COMSPEC%` environment variable is used if it is set.
       **It is not recommended to change COMSPEC**, keep reading this
       page of the documentation to learn how to make gameterm
...
If you'd like `gameterm` to run a different program than the shell as
...
for example, open an editor in gameterm you can use the `start` subcommand
to launch it.  This example opens up a new terminal window running vim
to edit your gameterm configuration:

```console
$ gameterm start -- vim ~/.gameterm.lua
```

## Specifying the current working directory

If you'd like `gameterm` to start running a program in a specific working
...
The behavior is to take the environment of the `gameterm` process
...
if gameterm.target_triple == 'x86_64-pc-windows-msvc' then
  ...
  for _, vsvers in
    ipairs(
      gameterm.glob('Microsoft Visual Studio/20*', 'C:/Program Files (x86)')
    )
  do
