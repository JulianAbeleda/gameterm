...
The default encoding scheme used by gameterm is xterm compatible, but there are
...
When gameterm receives the sequence `CSI >4;Nm`, where `N` is `0`, `1` or `2`,
...
gameterm will honor the application requests to modify the keyboard encoding.

## Windows

On Windows, [allow_win32_input_mode](lua/config/allow_win32_input_mode.md)
defaults to `true` which causes gameterm to listen for an escape sequence
