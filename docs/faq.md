...
GameTerm will automatically set `LANG` appropriately.
...
and GameTerm will automatically set `LANG` appropriately.
...
emoji, or perhaps some kanji) then gameterm will try to locate a fallback
...
that you need in your `.gameterm.lua`:

```lua
local gameterm = require 'gameterm'

return {
  font = gameterm.font_with_fallback {
    'My Preferred Font',
...
were to emit that same emoji from a script, gameterm would render it correctly.
...
`gameterm` supports [advanced font shaping](config/font-shaping.md), which,
...
experiencing this if, e.g., `!=` becomes rendered as `≠` in `gameterm`.
...
The first thing to note is that `gameterm` will always and only output `UTF-8`
...
section of the docs describes how gameterm processes
...
being set appropriately.  `gameterm` sets this to `xterm-256color` by default,
because gameterm aims to be compatible with with the settings defined by that
...
that gameterm isn't sending the correct input then please [open an
issue](https://github.com/JulianAbeleda/gameterm/issues) and include the `xxd` hexdump,
...
Starting in version 20210314-114017-04b7cedd, GameTerm has support for colored
...
You may wish to try these steps to install a copy of a `gameterm` terminfo file;
this will compile a copy of the terminfo and install it into your `~/.terminfo`
directory:

```bash
tempfile=$(mktemp) \
  && curl -o $tempfile https://raw.githubusercontent.com/gameterm/gameterm/master/termwiz/data/gameterm.terminfo \
...
This is not an issue in GameTerm; the same issue manifests in any terminal
...
* Determine the XCursor theme:
  1. is `xcursor_theme` set in the gameterm configuration?
...
4. If the file exists, then gameterm will try to load it

If no XCursor was found, gameterm will fall back to using the default X11 cursor
...
below, and then moving the mouse over the gameterm window:

```
; GAMETERM_LOG=window::os::x11::cursor=trace gameterm
...
## I'm on macOS and gameterm cannot find things in my PATH

On macOS, gameterm is typically launched directly by the Finder process and inherits
...
However, if you want gameterm to directly spawn some other utility that isn't in that
basic PATH, gameterm will report that it cannot find it.

Probably the easiest to maintain solution is to change something like:

```lua
gameterm.action.SpawnCommandInNewWindow {
  args = { 'nvim', gameterm.config_file },
}
```
...
gameterm.action.SpawnCommandInNewWindow {
  args = {
    os.getenv 'SHELL',
    '-c',
    'nvim ' .. gameterm.shell_quote_arg(gameterm.config_file),
  },
}
```
...
gameterm.action.SpawnCommandInNewWindow {
  args = {
    gameterm.home_dir .. '/.local/bob/nvim-bin/nvim',
...
config.set_environment_variables = {
  ...
  PATH = gameterm.home_dir .. '/.local/bob/nvim-bin:' .. os.getenv 'PATH',
}
```
 ...
 * [gameterm.config_file](config/lua/gameterm/config_file.md)
 * [gameterm.shell_quote_arg](config/lua/gameterm/shell_quote_arg.md)
...
By default, gameterm enables ligature support in the font that you have selected.
