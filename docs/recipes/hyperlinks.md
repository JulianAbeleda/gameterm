...
- Other hyperlinks like URLs remain unchanged and follow GameTerm's default behavior
...
gameterm.on('open-uri', function(window, pane, uri)
  ...
  if uri:find '^file:' == 1 and not pane:is_alt_screen_active() then
    ...
    local url = gameterm.url.parse(uri)
    if is_shell(pane:get_foreground_process_name()) then
      ...
      local success, stdout, _ = gameterm.run_child_process {
        'file',
        '--brief',
        '--mime-type',
        url.file_path,
      ...
      if success then
        if stdout:find 'directory' then
          pane:send_text(
            gameterm.shell_join_args { 'cd', url.file_path } .. '\r'
          )
          pane:send_text(gameterm.shell_join_args {
            'ls',
            '-a',
            '-p',
        ...
        if stdout:find 'text' then
          if url.fragment then
            ...
            pane:send_text(
              gameterm.shell_join_args { editor, url.file_path } .. '\r'
            )
          end
          return false
        end
...
- When a shell is not detected (see the `is_shell` function), GameTerm falls back to a long shell command, which may clutter the prompt slightly. This happens because GameTerm cannot directly determine a file's MIME type when not connected to a local shell.
