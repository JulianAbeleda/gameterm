...
to manage separate groups of related tmux windows (which map to tabs in gameterm).

gameterm doesn't have an exact match to tmux sessions, but does have a similar
...
The gameterm GUI is focused on the active workspace, which means that it will
...
When switching the active workspace, gameterm will swap the contents of the
