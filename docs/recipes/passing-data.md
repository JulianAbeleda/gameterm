...
While gameterm provides a few functions that can help to peek into locally
...
# This function is included in the gameterm shell integration script, but
...
function _run_prog() {
    ...
    __gameterm_set_user_var "PROG" "$1"

    # arrange to clear it when it is done
    trap '__gameterm_set_user_var PROG ""' EXIT
...
Then on the gameterm side, this information can be used when formatting the tab titles:

```lua
local gameterm = require 'gameterm'

gameterm.on('format-tab-title', function(tab)
...
If you install the [gameterm shell integration](../shell-integration.md) you
...
your gameterm config, and will work across multiplexer connections and even
...
gameterm, like many other terminals, will interpret Operating System Command
...
prior to executing a command. gameterm doesn't currently set this up
automatically. Note that gameterm will attempt to determine the foreground
...
Emitting OSC 7 will tell gameterm to use a specific URI for the current working
...
You may also use `gameterm set-working-directory` for this if you have `gameterm`
available.
...
is a local pane, gameterm can attempt to determine the working directory of the
foreground process that is associated with the pane.

Installing the [gameterm shell integration](../shell-integration.md) will
arrange for bash/zsh to set OSC 7 for you.

## Local Process State

gameterm provides some functions that can attempt to extract information about
...
* [gameterm.procinfo.get_info_for_pid()](../config/lua/gameterm.procinfo/get_info_for_pid.md) -
...
[gameterm.procinfo](../config/lua/gameterm.procinfo/index.md) module.
...
your gameterm configuration, but they are limited to local processes only and
