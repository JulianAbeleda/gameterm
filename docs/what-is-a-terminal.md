...
GameTerm is a Terminal Emulator, but what actually is that, and what is a PTY,
...
flowchart 
    ...
    PTYM  <-- input/output -->  TE["Terminal Emulator\n(e.g. gameterm)"]:::gameterm
    classDef gameterm stroke:#00F,stroke-width:2px
    end
```
...
When running on Windows with ConPTY, an additional helper program (for gameterm,
...
but what it means for gameterm users is that they may wish to bypass ConPTY in
some cases by using `gameterm ssh` to directly communicate with a "real" unix
