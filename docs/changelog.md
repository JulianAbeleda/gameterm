...
* Migrated serial support to the `serial2` rust crate. This opens the door
  ...
  #6411 #6460
* macOS: The gameterm terminfo file is now compiled and bundled in the
  application bundle. Thanks to @ddeville! #6538
* `gameterm record` now has a `-o outputfile` option. Thanks to @Tyarel8! #6626
...
* macOS: toast notifications now use UNUserNotificationCenter. This requires
  that GameTerm.app be code-signed, which is the case for official binaries.
...
* `gameterm imgcat --hold` now avoids local echo and accepts pressing `Escape`,
  `CTRL-C` and `CTRL-D` as various ways of exiting hold mode. Thanks to
  @mgpinf! #6801

#### New
* [gameterm.serde](config/lua/gameterm.serde/index.md) module for serialization
  and deserialization of JSON, TOML and YAML. Thanks to @expnn! #4969
* `gameterm ssh` now supports agent forwarding. Thanks to @Riatre! #5345
...
* gameterm-ssh now supports `ProxyUseFdPass`. Thanks to @loops! #6103 #6093
...
* macOS: wacky initial window size with external monitors or certain font
  sizes. #4966 #4250
* macOS: dragging non-filename data over gameterm could cause it to crash. #4771
...
* `gameterm cli activate-pane` now respects `unzoom_on_switch_pane`. Thanks to
  @quantonganh! #5306 #5305
* gameterm-ssh now correctly handles two-phase processing of `%h` tokens. Thanks
...
* Wayland: Fixed potential panic on startup when monitors have changed are
  in the process of hot plugging when gameterm starts. Thanks to @loops! #6084
...
* x11: transient errors in obtaining/setting the selection could cause
  gameterm to exit. Thanks to @loops! #6135 #5482 #6128
...
* `gameterm cli split-pane --move-pane-id` could kill panes. Thanks to @scauligi!
  #6028 #6029
...
* `gameterm -e` will now wait for the spawned program to terminate before
...
* `gameterm start --new-tab` and `gameterm connect --new-tab` to request a new
...
* `gameterm set-working-directory` and `gameterm imgcat` didn't correctly
...
* The return type of
  ...
  has changed to the new [Url](config/lua/gameterm.url/Url.md) object, which
...
* Nightly `.deb` packages are now named `gameterm-nightly` rather than `gameterm`,
  and are set to conflict with `gameterm`.
...
#### New
* [gameterm imgcat](cli/imgcat.md) now has `--position`, `--no-move-cursor` and
  `--hold` options. #3716
* [gameterm set-working-directory](cli/set-working-directory.md) will now wrap
...
* [gameterm imgcat](cli/imgcat.md) will now wrap the image transfer OSC escape
...
* [gameterm imgcat](cli/imgcat.md) will compensate for tmux and conpty, which
...
* [gameterm imgcat](cli/imgcat.md) will now resample very large images in
...
* [gameterm cli zoom-pane](cli/cli/zoom-pane.md). Thanks to @quantonganh! #4160
...
* X11: fd leak on each call to
  [gameterm.gui.enumerate_gpus](config/lua/gameterm.gui/enumerate_gpus.md). #3612
...
* Wayland: gameterm wouldn't start on Plasma 6 or newer versions of sway. Thanks
  to @hexchain! #3996 #4322.
* font-config: when resolving a fallback font for a text cluster like `U+3065,U+2686`
  where no single font contains both glyphs, gameterm would fail to show a glyph
...
* [gameterm cli adjust-pane-size](cli/cli/adjust-pane-size.md). Thanks to @mrjones2014! #3471 #3491
...
* X11/Wayland: CTRL-key presses for non-US latin keymaps regressed due to
  ...
  layouts](https://github.com/JulianAbeleda/gameterm/issues/2845). #3610
...
* Windows: closing a window while the debug overlay was active could
  leave a lingering gameterm-gui.exe running. #3522
...
* imgcat broken when using `gameterm ssh`. #3624
...
* Windows: access denied error when using `gameterm.plugin.require`. Thanks to @nyxkrage! #3767
...
* ClearToEndOfLine performed in the last column of the display, when it is
  ...
  #3548
* GAMETERM_UNIX_SOCKET didn't get set when using `gameterm start -- something`. #3679
...
* `gameterm ssh` connection errors were not shown; the window would close
...
* Multiplexer: closing a window that has any mux client panes will now detach
  ...
  With a definitive detach, the window will close and gameterm will terminate if
...
* [gameterm replay](cli/replay.md) new options `--explain-only`, `--cat`. #3446
...
* `ssh_domains` now auto-populate from your `~/.ssh/config` file. You can use
  [gameterm.default_ssh_domains()](config/lua/gameterm/default_ssh_domains.md) to
...
* Lua: [gui_window:active_tab()](config/lua/window/active_tab.md),
  ...
  from a window/tab.
* [gameterm cli set-tab-title](cli/cli/set-tab-title.md) and
  [gameterm cli set-window-title](cli/cli/set-window-title.md). #522 #1598
* [gameterm cli rename-workspace](cli/cli/rename-workspace.md). #2787
* [gameterm.mux.rename_workspace](config/lua/gameterm.mux/rename_workspace.md). #2787
* [gameterm cli get-pane-direction](cli/cli/get-pane-direction.md)
...
* [gameterm.has_action()](config/lua/gameterm/has_action.md) makes it easier to
  author a gameterm config that works across different gameterm versions. #3454
...
* Windows: "error converting Lua string to &str" on systems with an ACP that
  was not compatible with UTF-8 when gameterm was placed in a directory with
  a non UTF-8, non-ASCII name. #3390
* mux: `--config` related command line options passed to `gameterm-mux-server` were
...
* `gameterm connect --workspace WS DOM` didn't use the provided workspace when
...
* `gameterm.config_builder` now works with `table.insert(config.keys, {..})`
* mux: `gameterm cli activate-pane-direction` inconsistent behavior with remote mux. #3387 #3448
...
* [gameterm cli kill-pane](cli/cli/kill-pane.md),
  [gameterm cli activate-pane](cli/cli/activate-pane.md) and
  [gameterm cli activate-tab](cli/cli/activate-tab.md) #3352 #886
* macOS:
...
* `gameterm.GLOBALS.foo` now supports `__len` and `__pairs` metamethods, as well
  as being passed to `gameterm.json_encode`
* `gameterm --skip-config` resulted in an `unexpected argument` error, while
  `gameterm -n` still worked as intended. #3325
...
* ConPTY: logical line wrapping falsely joining long runs of output from classic
  ...
  #3278 #3177
* `gameterm cli activate-pane-direction` didn't cause the gui to repaint, making
...
* Unblock all signals when spawning programs, to avoid strangess when the
  parent of the gameterm process spawns gameterm with an unusual signal mask.
...
* `gameterm cli activate-pane-direction` would not cause the focus to change
...
* mux: `gameterm cli move-pane-to-new-tab` didn't resync new window structure
  and would appear to have had no effect until you detached and re-attached. #3219
* mux: `gameterm cli move-pane-to-new-tab` didn't forward the structural change
...
* Copy Mode now supports using `CTRL-u` and `CTRL-d` to move by half a page at
  ...
  [#2662](https://github.com/JulianAbeleda/gameterm/pull/2662)
* macOS: allow association with `.command`, `.sh`, `.zsh`, `.bash`, `.fish` and
  ...
  terminal. [#2871](https://github.com/JulianAbeleda/gameterm/issues/2871)
  [#2741](https://github.com/JulianAbeleda/gameterm/issues/2741)
* macOS: initial cut at macOS native menu bar
  [#1485](https://github.com/JulianAbeleda/gameterm/issues/1485)
* mux: exposed [MuxDomain](config/lua/MuxDomain/index.md) to lua, along with
  [gameterm.mux.get_domain()](config/lua/gameterm.mux/get_domain.md),
  [gameterm.mux.all_domains()](config/lua/gameterm.mux/all_domains.md) and
  [gameterm.mux.set_default_domain()](config/lua/gameterm.mux/set_default_domain.md).
* [hide_mouse_cursor_when_typing](config/lua/config/hide_mouse_cursor_when_typing.md)
  ...
  [#2946](https://github.com/JulianAbeleda/gameterm/pull/2946)
...
* `CTRL-SHIFT-P` now activates the new [command
  ...
  [#1485](https://github.com/JulianAbeleda/gameterm/issues/1485)
* `gameterm ssh` now supports `%l` and `%L` tokens in config files.
  [#3176](https://github.com/JulianAbeleda/gameterm/issues/3176)
* [hyperlink_rules](config/lua/config/hyperlink_rules.md) now support
  specifying which capture group should be highlighted.
* [gameterm.default_hyperlink_rules](config/lua/gameterm/default_hyperlink_rules.md)
  function makes it easier to extend the default set of hyperlink rules.

#### Fixed
* X11: hanging or killing the IME could hang gameterm
  [#2819](https://github.com/JulianAbeleda/gameterm/issues/2819)
* `gameterm ssh` now respects the `AddressFamily` option when connecting
  [#2893](https://github.com/JulianAbeleda/gameterm/issues/2893)
* Windows: panic when minimizing a window when `front_end='WebGpu'`
  [#2881](https://github.com/JulianAbeleda/gameterm/issues/2881)
* X11: `gameterm.screens().active` is now populated based on the currently
  focused window, rather than just copying the `main` screen data. Thanks to
  [@NBonaparte](https://github.com/NBonaparte)!
  [#2928](https://github.com/JulianAbeleda/gameterm/pull/2928)
* Resizing windows when overriding the dpi in the config would not respect
  ...
  [#2914](https://github.com/JulianAbeleda/gameterm/issues/2914)
  [#2978](https://github.com/JulianAbeleda/gameterm/pull/2978)
* Wayland: mouse cursor didn't auto-hide when typing. Thanks to
  [@jmbaur](https://github.com/jmbaur)!
  [#2977](https://github.com/JulianAbeleda/gameterm/pull/2977)
* mux: `default_workspace` was not always respected when spawning
  [#2981](https://github.com/JulianAbeleda/gameterm/issues/2981)
* [window:active_key_table()](config/lua/window/active_key_table.md) now
  ...
  [#2986](https://github.com/JulianAbeleda/gameterm/discussions/2986)
...
* lag when making heavy use of foreground process information in tab titles.
  [#2991](https://github.com/JulianAbeleda/gameterm/issues/2991)
* X11: always update selection owner even if our window already owns it
  [#2926](https://github.com/JulianAbeleda/gameterm/issues/2926)
...
* Windows: ALT key was not recognized when processing mouse events.
  ...
  [#3053](https://github.com/JulianAbeleda/gameterm/pull/3053)
* Copy Mode:
...
* Images are now buffered to temporary files and decoded in background
  ...
  [#3263](https://github.com/JulianAbeleda/gameterm/issues/3263)
* Improved compatibility with the Kitty Image Protocol
  [#2716](https://github.com/JulianAbeleda/gameterm/issues/2716)
* [gameterm.time.call_after](config/lua/gameterm.time/call_after.md) would not
  work when used in an event callback.
  [#3026](https://github.com/JulianAbeleda/gameterm/issues/3026)
* Hover state not correctly indicated for retro tab bar when showing at
  the bottom of the window
  [#3113](https://github.com/JulianAbeleda/gameterm/issues/3113)

#### Changed
* `CTRL-SHIFT-P` now activates the new [command
  palette](config/lua/keyassignment/ActivateCommandPalette.md), instead of
  `PaneSelect` [#1485](https://github.com/JulianAbeleda/gameterm/issues/1485)
...
* `gameterm -e` is now an alias for `gameterm start`. Thanks to
  [@Abdiramen](https://github.com/Abdiramen)!
  [#2889](https://github.com/JulianAbeleda/gameterm/pull/2889)
  [#2782](https://github.com/JulianAbeleda/gameterm/issues/2782)
* [bold_brightens_ansi_colors](config/lua/config/bold_brightens_ansi_colors.md)
  ...
  font. [#2932](https://github.com/JulianAbeleda/gameterm/issues/2932)
...
* Referencing `gameterm.GLOBAL` now returns references rather than copies, making
  it less cumbersome to code read/modify/write with global state
* `gameterm start` now accepts `--domain` and `--attach` options. `gameterm
  connect DOMAIN` is now implemented internally as `gameterm start --domain
  DOMAIN --attach`.
* X11: spurious pointer focus events no longer influence terminal focus events.
  ...
  [#2959](https://github.com/JulianAbeleda/gameterm/pull/2959)
* Copy Mode: Added <kbd>Home</kbd> and <kbd>End</kbd> default assignments.
  Thanks to [@cunha](https://github.com/cunha)!
  [#2762](https://github.com/JulianAbeleda/gameterm/pull/2762)
* [gameterm.mux.spawn_window](config/lua/gameterm.mux/spawn_window.md) and
  ...
  [#2976](https://github.com/JulianAbeleda/gameterm/issues/2976)
...
* WSL Domains returned from
  [gameterm.default_wsl_domains](config/lua/gameterm/default_wsl_domains.md) now
  set `default_cwd="~"` [#2826](https://github.com/JulianAbeleda/gameterm/issues/2826)
* gameterm now also searches `$XDG_CONFIG_DIRS` as well as `$XDG_CONFIG_HOME`
  when searching for config files. Thanks to [@jmbaur](https://github.com/jmbaur)!
  [#3146](https://github.com/JulianAbeleda/gameterm/pull/3146)
* [gameterm.time.call_after](config/lua/gameterm.time/call_after.md) now accepts
  fractional numbers of seconds.
  [#3287](https://github.com/JulianAbeleda/gameterm/issues/3287)
...
#### New
* [gameterm.gui.default_key_tables](config/lua/gameterm.gui/default_key_tables.md)
  and [gameterm.gui.default_keys](config/lua/gameterm.gui/default_keys.md) for
...
* [normalize_output_to_unicode_nfc](config/lua/config/normalize_output_to_unicode_nfc.md)
  ...
  the terminal model.  [#2482](https://github.com/JulianAbeleda/gameterm/issues/2482)
* [cursor_thickness](config/lua/config/cursor_thickness.md),
  ...
  to fine tune appearance. [#2505](https://github.com/JulianAbeleda/gameterm/issues/2505)
  [#2326](https://github.com/JulianAbeleda/gameterm/issues/2326)
* Support for `modifyOtherKeys` keyboard encoding
  [#2527](https://github.com/JulianAbeleda/gameterm/issues/2527)
* Superscript and subscript text attributes via SGR 73 and SGR 74
* [gameterm cli activate-pane-direction](cli/cli/activate-pane-direction.md)
  command. Thanks to [@abusch](https://github.com/abusch)!
  [#2526](https://github.com/JulianAbeleda/gameterm/pull/2526)
* [window:is_focused()](config/lua/window/is_focused.md) method for testing
  whether a GUI window has focus.
  [#2537](https://github.com/JulianAbeleda/gameterm/discussions/2537)
...
* [show_tabs_in_tab_bar](config/lua/config/show_tabs_in_tab_bar.md) and
  ...
  [#2082](https://github.com/JulianAbeleda/gameterm/issues/2082)
...
* Wayland: key repeat gets stuck after pressing two keys in quick succession.
  ...
  [#2492](https://github.com/JulianAbeleda/gameterm/pull/2492)
  [#2452](https://github.com/JulianAbeleda/gameterm/issues/2452)
* If the underline attribute was active and CRLF scrolled a new line into the
  ...
  [#2489](https://github.com/JulianAbeleda/gameterm/issues/2489)
* Correctly invalidate the display when using
  `gameterm.action.ClearScrollback("ScrollbackAndViewport")`
  [#2498](https://github.com/JulianAbeleda/gameterm/issues/2498)
* Hyperlinks didn't underline on hover
  [#2496](https://github.com/JulianAbeleda/gameterm/issues/2496)
* base16 color schemes cursor fg/bg were the same. We now also set the indexed
  ...
  [#2491](https://github.com/JulianAbeleda/gameterm/pull/2492)
* Panic when processing a sixel with inconsistent width information
  [#2500](https://github.com/JulianAbeleda/gameterm/issues/2500)
...
* Panic when trying to activate the search overlay when the launcher menu is
  active [#2529](https://github.com/JulianAbeleda/gameterm/issues/2529)
* Overlays did not see config overrides set via `window:set_config_overrides`
  [#2544](https://github.com/JulianAbeleda/gameterm/issues/2544)
* Closing a window while tab had a zoomed pane would leave the other panes
  untouched and gameterm would linger in the background
  [#2548](https://github.com/JulianAbeleda/gameterm/issues/2548)
* CharSelect panic when pressing enter when no matches were found
  [#2580](https://github.com/JulianAbeleda/gameterm/issues/2580)
* Panic when setting `initial_rows` or `initial_cols` to `0`
  [#2593](https://github.com/JulianAbeleda/gameterm/issues/2593)
* X11: Crash on systems using DRI2 based Intel graphics
  [#2559](https://github.com/JulianAbeleda/gameterm/issues/2559)
* Missing validation of conflicting domain names
  [#2618](https://github.com/JulianAbeleda/gameterm/issues/2618)
* Creating tabs in a multiplexing domain could fail after previously closing
  ...
  [#2614](https://github.com/JulianAbeleda/gameterm/issues/2614)
* CharSelect now uppercases hex digit input for better compatibility with
  ...
  [#2581](https://github.com/JulianAbeleda/gameterm/issues/2581)
* Multiple active multiplexer client domain connections could result
  in showing duplicate tabs in a window
  [#2616](https://github.com/JulianAbeleda/gameterm/issues/2616)
* Incorrect line width when applying hyperlink rules to a wrapped line
  containing double-wide cells.
  [#2568](https://github.com/JulianAbeleda/gameterm/issues/2568)
* Incorrect shaping for U+28 U+FF9F
  [#2572](https://github.com/JulianAbeleda/gameterm/issues/2572)
* Panic when hitting enter in launcher menu when no fuzzy results match
  [#2629](https://github.com/JulianAbeleda/gameterm/issues/2629)
* Default `CTRL-SHIFT-<NUM>` assignments didn't work on Windows and X11
  systems when `key_map_preference = "Mapped"`
  [#2623](https://github.com/JulianAbeleda/gameterm/issues/2623)
* Panic when calling `window:set_workspace` when the default domain is a
  multiplexer domain.
  [#2638](https://github.com/JulianAbeleda/gameterm/issues/2638)
* nvim's `title` and `titlestring` options don't work when `TERM=gameterm`.
  [#2112](https://github.com/JulianAbeleda/gameterm/issues/2112)
* Horizontal wheel scrolling generated incorrect mouse events
  [#2649](https://github.com/JulianAbeleda/gameterm/issues/2649)
* Cursor color changes did not always render
  [#2708](https://github.com/JulianAbeleda/gameterm/issues/2708)
* Unable to set cursor on Wayland/X11
  [#2687](https://github.com/JulianAbeleda/gameterm/issues/2687)
  [#2743](https://github.com/JulianAbeleda/gameterm/issues/2743)
* Default `MoveTabRelative` assignments were incorrectly set to
  ...
  [#2705](https://github.com/JulianAbeleda/gameterm/issues/2705)
* Dragging by retro tab bar left or right status area would jump around erratically.
  [#2758](https://github.com/JulianAbeleda/gameterm/issues/2758)
* Fixed background `Cover` algorithm. Thanks to
  [@xiaopengli89](https://github.com/xiaopengli89)!
  [#2636](https://github.com/JulianAbeleda/gameterm/pull/2636)
* `gameterm start --cwd .` didn't use the cwd of the spawned process when the
  gameterm gui was already running. Thanks to
  [@exactly-one-kas](https://github.com/exactly-one-kas)!
  [#2661](https://github.com/JulianAbeleda/gameterm/pull/2661)
* IME composition text and cursor color incorrectly applied to all panes rather
  than just the active pane.
  [#2569](https://github.com/JulianAbeleda/gameterm/issues/2569)

#### Changed
* Removed Last Resort fallback font
* X11: use `_NET_WM_MOVERESIZE` to drag by tab bar, when supported by the WM
  [#2530](https://github.com/JulianAbeleda/gameterm/issues/2530)
...
* macOS: CTRL-modified keys are now routed to the IME
  [#2435](https://github.com/JulianAbeleda/gameterm/pull/2435)
...
* Added dummy `-e` command line option to support programs that assume that all
  ...
  [#2670](https://github.com/JulianAbeleda/gameterm/pull/2670)
  [#2622](https://github.com/JulianAbeleda/gameterm/issues/2622)
  [#2271](https://github.com/JulianAbeleda/gameterm/issues/2271)
* Windows: installer no longer prevents installing the x64 binary on arm64 systems.
  ...
  [#2746](https://github.com/JulianAbeleda/gameterm/pull/2746)
  [#2667](https://github.com/JulianAbeleda/gameterm/issues/2667)
...
* [switch_to_last_active_tab_when_closing_tab](config/lua/config/switch_to_last_active_tab_when_closing_tab.md)
  ...
  [#2487](https://github.com/JulianAbeleda/gameterm/issues/2487)
#### Changed
* fontconfig: when locating a fallback font for a given codepoint, allow
  ...
  [#2468](https://github.com/JulianAbeleda/gameterm/discussions/2468)
* `os.getenv` now knows how to resolve environment variables that would normally
  ...
  or those set through the registry on Windows. [#2481](https://github.com/JulianAbeleda/gameterm/discussions/2481)
* Searching is now incremental and shows progress. [#1209](https://github.com/JulianAbeleda/gameterm/issues/1209)

#### Fixed
* Hangul in NFD incorrectly shaped [#2482](https://github.com/JulianAbeleda/gameterm/issues/2482)
* Visual artifacts when resizing splits [#2483](https://github.com/JulianAbeleda/gameterm/issues/2483)

### 20220904-064125-9a6cee2b

* Fix build on architectures where `c_char` is `u8` instead of `i8`. Thanks to [@liushuyu](https://github.com/liushuyu)! [#2480](https://github.com/JulianAbeleda/gameterm/pull/2480)
...
* [window_frame](config/lua/config/window_frame.md) now supports setting border size and color [#2417](https://github.com/JulianAbeleda/gameterm/issues/2417)
* [CopyMode](copymode.md) now supports selecting and move by semantic zones. [#2346](https://github.com/JulianAbeleda/gameterm/issues/2346)
* [max_fps](config/lua/config/max_fps.md) option to limit maximum frame rate [#2419](https://github.com/JulianAbeleda/gameterm/discussions/2419)
...
* Cursor now changes to a lock glyph to indicate when local echo is disabled for password entry. Detection is limited to local unix processes and cannot work with tmux. Use `detect_password_input=false` to disable this. [#2460](https://github.com/JulianAbeleda/gameterm/issues/2460)
...
* [ActivatePaneDirection](config/lua/keyassignment/ActivatePaneDirection.md) now uses recency to resolve ambiguous moves [#2374](https://github.com/JulianAbeleda/gameterm/issues/2374)
...
* Cache XDG Portal Appearance values. Thanks to [@vimposter](https://github.com/vimpostor)! [#2402](https://github.com/JulianAbeleda/gameterm/pull/2402)
* Compensate for TUI programs that flicker due to unsynchronized output by adding up to 3ms additional latency after each read to coalesce their screen outputs into a single frame. You can set this delay via a new `mux_output_parser_coalesce_delay_ms` option. [#2443](https://github.com/JulianAbeleda/gameterm/issues/2443)
* win32: Updated openconsole/conpty to v1.14.2281.0

#### Fixed

* macOS: crash on startup if `$SHELL` points to something that isn't executable. [#2378](https://github.com/JulianAbeleda/gameterm/issues/2378)
* tab titles truncated too short [#2379](https://github.com/JulianAbeleda/gameterm/issues/2379)
* `bypass_mouse_reporting_modifiers` stopped working (regression around new mouse binding logic) [#2389](https://github.com/JulianAbeleda/gameterm/issues/2389)
* Entering IME-composed text would have no effect in `gameterm ssh` [#2434](https://github.com/JulianAbeleda/gameterm/issues/2434)
* `gui-startup` event now also works with `gameterm ssh`
* `x` and `+` buttons in the fancy tab bar are now always square [#2399](https://github.com/JulianAbeleda/gameterm/issues/2399)
* middle clicking a tab to close it will now confirm closing using the same rules as [CloseCurrentTab](config/lua/keyassignment/CloseCurrentTab.md) [#2350](https://github.com/JulianAbeleda/gameterm/issues/2350)
* Emitting the tmux-style `ESC k TITLE ST` sequence via ConPTY breaks output for the pane [#2442](https://github.com/JulianAbeleda/gameterm/issues/2442)
* Avoid using full path canonicalization for `--cwd` options [#2449](https://github.com/JulianAbeleda/gameterm/issues/2449)
* Scroll to the bottom on mouse input when mouse reporting is enabled [#2447](https://github.com/JulianAbeleda/gameterm/issues/2447)
* ssh: correctly expand `%h` ssh_config tokens [#2448](https://github.com/JulianAbeleda/gameterm/issues/2448)
* ssh: `CloseCurrentPane` wouldn't release all resources associated with the pane and could lead to a `too many open files` error for a long running `gameterm ssh` session. [#2466](https://github.com/JulianAbeleda/gameterm/issues/2466)
* mouse cursor is now reset to arrow when the mouse leaves the window [#2471](https://github.com/JulianAbeleda/gameterm/issues/2471)

### 20220807-113146-c2fee766

#### New
* [ActivateKeyTable](config/lua/keyassignment/ActivateKeyTable.md) now supports `until_unknown=true` to implicitly pop the table when a key not defined by that table is pressed. [#2178](https://github.com/JulianAbeleda/gameterm/issues/2178)
...
* [window:get_selection_escapes_for_pane](config/lua/window/get_selection_escapes_for_pane.md) method for getting the current selection including escape sequences. [#2223](https://github.com/JulianAbeleda/gameterm/issues/2223)
* [window:current_event](config/lua/window/current_event.md) method for getting the current event. [#2296](https://github.com/JulianAbeleda/gameterm/pull/2296)
* [gameterm.color](config/lua/gameterm.color/index.md) module for working with colors and importing color schemes.
* [gameterm.gui](config/lua/gameterm.gui/index.md) module and [mux_window:gui_window](config/lua/mux-window/gui_window.md) method.
* [gameterm.gui.screens()](config/lua/gameterm.gui/screens.md) function for getting information about the available screens/monitors/displays
* [gameterm.gui.get_appearance()](config/lua/gameterm.gui/get_appearance.md) function for a simpler way to get system dark mode state
* [gameterm.procinfo](config/lua/gameterm.procinfo/index.md) module for querying local process information.
* [gameterm.time](config/lua/gameterm.time/index.md) module for working with time, including methods for determining sun rise/set.
* You may now use [gameterm.format](config/lua/gameterm/format.md) (or otherwise use strings with escape sequences) in the labels of the [Launcher Menu](config/launch.md#the-launcher-menu).
* You may now specify `assume_emoji_presentation = true` (or `false`) in [gameterm.font()](config/lua/gameterm/font.md) and [gameterm.font_with_fallback()](config/lua/gameterm/font_with_fallback.md)
* Wayland: `zwp_text_input_v3` is now supported, which enables IME to work in gameterm if your compositor also implements this protocol.
* [gameterm.json_parse()](config/lua/gameterm/json_parse.md) and [gameterm.json_encode()](config/lua/gameterm/json_encode.md) functions for working with JSON.
...
* [pane:is_alt_screen_active()](config/lua/pane/is_alt_screen_active.md) for testing whether the alt screen is active. Thanks to [@Funami580](https://github.com/Funami580)! [#2234](https://github.com/JulianAbeleda/gameterm/issues/2234)
* X11/Wayland: [XDG desktop portal](https://flatpak.github.io/xdg-desktop-portal/) is now used to determine whether dark mode is in use [#2258](https://github.com/JulianAbeleda/gameterm/issues/2258)
* [SetPaneZoomState](config/lua/keyassignment/SetPaneZoomState.md) key assignment and [MuxTab:set_zoomed()](config/lua/MuxTab/set_zoomed.md) for explicitly setting the zoom state of a pane. [#2284](https://github.com/JulianAbeleda/gameterm/discussions/2284)
* [mouse_bindings](config/mouse.md) can now handle scroll events. Thanks to [@Funami580](https://github.com/Funami580)! [#2173](https://github.com/JulianAbeleda/gameterm/issues/2173) [#2296](https://github.com/JulianAbeleda/gameterm/pull/2296)
* [mouse_bindings](config/mouse.md) may now also be defined based on whether the alt-screen is active and/or whether the application in the pane has enabled mouse reporting. [#581](https://github.com/JulianAbeleda/gameterm/issues/581)
* `gameterm.action.CopyMode('ClearSelectionMode')` allows clearing the selection mode without leaving [Copy Mode](copymode.md). Thanks to [@aznhe21](https://github.com/aznhe21)! [#2352](https://github.com/JulianAbeleda/gameterm/pull/2352)
* [window:set_left_status](config/lua/window/set_left_status.md) for setting status to the left of the tabs in the tab bar [#1561](https://github.com/JulianAbeleda/gameterm/issues/1561)
...
* If `timeout_milliseconds` is specified in
  ...
  activation. [#1129](https://github.com/JulianAbeleda/gameterm/issues/1129)
* The lua examples in the docs are now syntax checked and formatted via
  ...
  [#2273](https://github.com/JulianAbeleda/gameterm/issues/2273)
  [#2253](https://github.com/JulianAbeleda/gameterm/issues/2253)
...
* Quickselect: now defaults to searching 1000 lines above and below the current viewport, making it faster and the labels shorter for users with a larger scrollback. A new `scope_lines` parameter to [QuickSelectArgs](config/lua/keyassignment/QuickSelectArgs.md) allows controlling the search region explicitly. Thanks to [@yyogo](https://github.com/yyogo) for the initial PR! [#1317](https://github.com/JulianAbeleda/gameterm/pull/1317)
* OSC 10, 11 and 12 (Set Default Text Background, Default Text Foreground Color, and Text Cursor Color) now support setting the alpha component [#2313](https://github.com/JulianAbeleda/gameterm/issues/2313), and added [CSI 38:6](escape-sequences.md#csi-386-foreground-color-rgba), `CSI 48:6` and `CSI 58:6` extensions to allow setting full color RGB with Alpha channel for spans of text.
* Copy Mode: setting the same selection mode a second time will now toggle off that mode and clear the selection, preserving the current position [#2246](https://github.com/JulianAbeleda/gameterm/discussions/2246)
* Copy Mode: new default vim-style `y` "yank" key assignment will copy the selection and close copy mode

#### Fixed
* [ActivateKeyTable](config/lua/keyassignment/ActivateKeyTable.md)'s `replace_current` field was not actually optional. Made it optional. [#2179](https://github.com/JulianAbeleda/gameterm/issues/2179)
* `winget` causes toast notification spam [#2185](https://github.com/JulianAbeleda/gameterm/issues/2185)
* `gameterm connect sshdomain` could hang on startup if password authentication was required [#2194](https://github.com/JulianAbeleda/gameterm/issues/2194)
* `colors.indexed` would error out with `Cannot convert String to u8`. [#2197](https://github.com/JulianAbeleda/gameterm/issues/2197)
* X11: closing a window when multiple were open could result in an X protocol error that closed all windows [#2198](https://github.com/JulianAbeleda/gameterm/issues/2198)
* Config will now automatically reload after error. Previously, you would need to manually reload the config using [ReloadConfiguration](config/lua/keyassignment/ReloadConfiguration.md). [#1174](https://github.com/JulianAbeleda/gameterm/issues/1174)
* Config will now automatically reload for changes made to `require`d lua files. Previously, only the main config file and any files that you explicitly passed to [add_to_config_reload_watch_list](config/lua/gameterm/add_to_config_reload_watch_list.md) would trigger a reload.
* macOS: numeric keypad enter generated CTRL-C instead of enter. Regression of [#739](https://github.com/JulianAbeleda/gameterm/issues/739). [#2204](https://github.com/JulianAbeleda/gameterm/issues/2204)
* Wayland: inconsistent pasting. Thanks to [@Funami580](https://github.com/Funami580)! [#2225](https://github.com/JulianAbeleda/gameterm/issues/2225) [#2226](https://github.com/JulianAbeleda/gameterm/pulls/2226)
* win32 input mode: fixed encoding of backspace and delete keys. Thanks to [@kreudom](https://github.com/kreudom)! [#2233](https://github.com/JulianAbeleda/gameterm/pull/2233)
* Tab bar could glitch and show incorrect contents when adjusting for monitor or changed font scaling [#2208](https://github.com/JulianAbeleda/gameterm/issues/2208)
* Wayland: transparent gap under tab bar when window is transparent, split and using per-pane color schemes [#1620](https://github.com/JulianAbeleda/gameterm/issues/1620)
* Tab bar could show a gap to the right when resizing
* Padding could show window background rather than pane background around split panes at certain window sizes [#2210](https://github.com/JulianAbeleda/gameterm/issues/2210)
...
* Unix: Clicking a URL when no browser is open could cause gameterm to hang until the newly opened browser is closed. [#2245](https://github.com/JulianAbeleda/gameterm/issues/2245)
* Quickselect: now selects the bottom-most match rather than the top-most match. [#2250](https://github.com/JulianAbeleda/gameterm/issues/2250)
* Mux: `gameterm.mux.set_active_workspace` didn't update the current window to match the newly activated workspace. [#2248](https://github.com/JulianAbeleda/gameterm/issues/2248)
* Overlays such as debug and launcher menu now handle resize better
* Shift-F1 through F4 generated different encoding than xterm [#2263](https://github.com/JulianAbeleda/gameterm/issues/2263)
* X11/Wayland: apps that extract the `Exec` field from gameterm.desktop (such as thunar, Dolphin and others) can now simply concatenate the command line they want to invoke, and it will spawn in the their current working directory. Thanks to [@Anomalocaridid](https://github.com/Anomalocaridid)! [#2271](https://github.com/JulianAbeleda/gameterm/pull/2271) [#2103](https://github.com/JulianAbeleda/gameterm/issues/2103)
* [gui-startup](config/lua/gui-events/gui-startup.md) now passes a [SpawnCommand](config/lua/SpawnCommand.md) parameter representing the `gameterm start` command arguments.
* Tab `x` button is no longer obscured by tab title text for long tab titles [#2269](https://github.com/JulianAbeleda/gameterm/issues/2269)
* Cursor position could end up in the wrong place when rewrapping lines and the cursor was on the rewrap boundary [#2162](https://github.com/JulianAbeleda/gameterm/issues/2162)
* Two or more panes closing at the same time could result in their containing tab hanging and being stuck with "no pane" for a title [#2304](https://github.com/JulianAbeleda/gameterm/issues/2304)
* Visual Bell now fills out to the adjacent window edges rather than being constrained by the padding. [#2364](https://github.com/JulianAbeleda/gameterm/issues/2364)
...
* New [gameterm.mux](config/lua/gameterm.mux/index.md) module, [gui-startup](config/lua/gui-events/gui-startup.md) and [mux-startup](config/lua/mux-events/mux-startup.md) events for spawning programs into your preferred arrangement when gameterm starts. [#674](https://github.com/JulianAbeleda/gameterm/issues/674)
* ssh client now supports `BindAddress`. Thanks to [@gpanders](https://github.com/gpanders)! [#1875](https://github.com/JulianAbeleda/gameterm/pull/1875)
* [PaneInformation.domain_name](config/lua/PaneInformation.md) and [pane:get_domain_name()](config/lua/pane/get_domain_name.md) which return the name of the domain with which a pane is associated. [#1881](https://github.com/JulianAbeleda/gameterm/issues/1881)
* You may now use `CTRL-n` and `CTRL-p` (in addition to the up/down arrow and vi motion keys) to change the selected row in the Launcher.  Thanks to [@Junnplus](https://github.com/Junnplus)! [#1880](https://github.com/JulianAbeleda/gameterm/pull/1880)
* Attaching multiplexer domains now attaches the first window as a tab in the active window, rather than opening a new window. [#1874](https://github.com/JulianAbeleda/gameterm/issues/1874)
...
* X11: gameterm now sets `_NET_WM_NAME` in addition to `WM_NAME` for clients that don't know how to fallback
* [treat_east_asian_ambiguous_width_as_wide](config/lua/config/treat_east_asian_ambiguous_width_as_wide.md) for control over how ambiguous width characters are resolved. [#1888](https://github.com/JulianAbeleda/gameterm/issues/1888)
* [clean_exit_codes](config/lua/config/clean_exit_codes.md) config to fine tune [exit_behavior](config/lua/config/exit_behavior.md) [#1889](https://github.com/JulianAbeleda/gameterm/issues/1889)
* [ClearSelection](config/lua/keyassignment/ClearSelection.md) key assignment [#1900](https://github.com/JulianAbeleda/gameterm/issues/1900)
* `gameterm cli list --format json` and `gameterm cli list-clients --format json` allow retrieving data in json format. Thanks to [@ratmice](https://github.com/ratmice)! [#1911](https://github.com/JulianAbeleda/gameterm/pull/1911)
* macOS, Windows, Wayland: you may now drag and drop files from other programs and have their paths paste into the terminal. The new [quote_dropped_files](config/lua/config/quote_dropped_files.md) option controls how the file names are quoted. Thanks to [@junnplus](https://github.com/junnplus), [@datasone](https://github.com/datasone) and [@Funami580](https://github.com/Funami580)! [#1868](https://github.com/JulianAbeleda/gameterm/pull/1868) [#1953](https://github.com/JulianAbeleda/gameterm/pull/1953) [#2148](https://github.com/JulianAbeleda/gameterm/pull/2148)
* The mouse scroll wheel now cycles between tabs when hovering over the tab. Thanks to [@junnplus](https://github.com/junnplus)! [#1726](https://github.com/JulianAbeleda/gameterm/issues/1726)
* Holding down `ALT` while dragging the left button will select a rectangular block. It is also possible to use `ALT+SHIFT` to select a rectangular block. [ExtendSelectionToMouseCursor](config/lua/keyassignment/ExtendSelectionToMouseCursor.md) and [SelectTextAtMouseCursor](config/lua/keyassignment/SelectTextAtMouseCursor.md) now accept `"Block"` as a selection mode. Thanks to [@Funami580](https://github.com/Funami580) for helping! [#1361](https://github.com/JulianAbeleda/gameterm/issues/1361)
* In Copy Mode, `CTRL-v` will enable rectangular block selection mode. [#1656](https://github.com/JulianAbeleda/gameterm/issues/1656)
* In Copy Mode, `SHIFT-v` will enable line selection mode. Thanks to [@bew](https://github.com/bew)! [#2086](https://github.com/JulianAbeleda/gameterm/pull/2086)
* In Copy Mode, `o` and `O` can be used to move the cursor to the other end of the selection, as in vim. Thanks to [@bew](https://github.com/bew)! [#2150](https://github.com/JulianAbeleda/gameterm/pull/2150)
* Copy Mode: key assignments are [now configurable](copymode.md#configurable-key-assignments) [#993](https://github.com/JulianAbeleda/gameterm/issues/993)
* Search Mode: key assignments are [now configurable](scrollback.md#configurable-search-mode-key-assignments) [#993](https://github.com/JulianAbeleda/gameterm/issues/993)
...
* Copy Mode and Search Mode can be toggled and remember search results and cursor positioning, making it easier to locate and select text without using the mouse [#1592](https://github.com/JulianAbeleda/gameterm/issues/1592)
* In the Launcher Menu, you may now use `CTRL-G` to cancel/exit the launcher [#1977](https://github.com/JulianAbeleda/gameterm/issues/1977)
* [cell_width](config/lua/config/cell_width.md) option to adjust the horizontal spacing when the available font stretches are insufficient. [#1979](https://github.com/JulianAbeleda/gameterm/issues/1979)
* [min_scroll_bar_height](config/lua/config/min_scroll_bar_height.md) to control the minimum size of the scroll bar thumb [#1936](https://github.com/JulianAbeleda/gameterm/issues/1936)
* [RotatePanes](config/lua/keyassignment/RotatePanes.md) key assignment for re-arranging the panes in a tab
* [SplitPane](config/lua/keyassignment/SplitPane.md) key assignment that allows specifying the size and location of the split, as well as top-level (full width/height) splits. `gameterm cli split-pane --help` shows equivalent options you can use from the cli. [#578](https://github.com/JulianAbeleda/gameterm/issues/578)
* [ime_preedit_rendering](config/lua/config/ime_preedit_rendering.md) option to choose whether to use the builtin or the system IME preedit rendering mode. Thanks to [@kumattau](https://github.com/kumattau)! [#2006](https://github.com/JulianAbeleda/gameterm/pull/2006)
* [gameterm.strftime_utc](config/lua/gameterm/strftime_utc.md) for manipulating times in UTC rather than the local timezone
* `gameterm cli send-text --no-paste` option to send text to a pain without wrapping it as a bracketed paste
* [PaneSelect](config/lua/keyassignment/PaneSelect.md) key assignment to activate the pane selection UI to activate or swap the selected pane. [#1842](https://github.com/JulianAbeleda/gameterm/issues/1842) [#1975](https://github.com/JulianAbeleda/gameterm/issues/1975)
* [window_background_gradient](config/lua/config/window_background_gradient.md) now also supports `Linear` gradients with an angle of your choice. Thanks to [@erf](https://github.com/erf)! [#2038](https://github.com/JulianAbeleda/gameterm/pull/2038)
* RPM and DEB packages now install zsh and bash `gameterm` CLI completions
...
* [gameterm.GLOBAL](config/lua/gameterm/GLOBAL.md) for persisting lua data across config reloads
* `gameterm show-keys` command to show key and mouse binding assignments [#2134](https://github.com/JulianAbeleda/gameterm/issues/2134)

#### Updated
* Bundled harfbuzz to 4.3.0

#### Changed
* Debian packages now register gameterm as an alternative for `x-terminal-emulator`. Thanks to [@xpufx](https://github.com/xpufx)! [#1883](https://github.com/JulianAbeleda/gameterm/pull/1883)
* Windows: gameterm will now read the default environment variables from the `HKLM\System\CurrentControlSet\Control\Session Manager\Environment` and `HKCU\Environment` and apply those to the base environment prior to applying `set_environment_variables`. [#1848](https://github.com/JulianAbeleda/gameterm/issues/1848)
* [Key Table](config/key-tables.md) lookups will now keep searching the activation stack until a matching assignment is found, allowing for layered key tables. [#993](https://github.com/JulianAbeleda/gameterm/issues/993)
* Search mode's search term is now remembered per-tab between activations of search mode. [#1912](https://github.com/JulianAbeleda/gameterm/issues/1912)
...
* Quickselect now supports multi-line anchors such as `^` and `$`.  [#2008](https://github.com/JulianAbeleda/gameterm/issues/2008)
* Overriding config using the cli `--config` option will now error out and prevent starting up if unknown config options are specified, or if the value evaluates to `nil`. Unknown options continue to generate warnings (rather than errors) when observed in the config file so that you're not "locked out" of gameterm if you make a typo in the config file.
* Windows: [allow_win32_input_mode](config/lua/config/allow_win32_input_mode.md) now defaults to `true` and enables using [win32-input-mode](https://github.com/microsoft/terminal/blob/main/doc/specs/%234999%20-%20Improved%20keyboard%20handling%20in%20Conpty.md) to send high-fidelity keyboard input to ConPTY. This means that win32 console applications, such as [FAR Manager](https://github.com/FarGroup/FarManager) that use the low level `INPUT_RECORD` API will now receive key-up events as well as events for modifier-only key presses. [#1509](https://github.com/JulianAbeleda/gameterm/issues/1509) [#2009](https://github.com/JulianAbeleda/gameterm/issues/2009) [#2098](https://github.com/JulianAbeleda/gameterm/issues/2098) [#1904](https://github.com/JulianAbeleda/gameterm/issues/1904)
* Wayland: [enable_wayland](config/lua/config/enable_wayland.md) now defaults to `true`. [#2104](https://github.com/JulianAbeleda/gameterm/issues/2104)
* [exit_behavior](config/lua/config/exit_behavior.md) now defaults to `"Close"`. [#2105](https://github.com/JulianAbeleda/gameterm/issues/2105)
* Improved [gameterm.action](config/lua/gameterm/action.md) syntax for slightly more ergnomic and understandable key assignments. [#1150](https://github.com/JulianAbeleda/gameterm/issues/1150)

#### Fixed
* Flush after replying to `XTGETTCAP`, `DECRQM`, `XTVERSION`, `DA2`, `DA3` [#2060](https://github.com/JulianAbeleda/gameterm/issues/2060) [#1850](https://github.com/JulianAbeleda/gameterm/issues/1850) [#1950](https://github.com/JulianAbeleda/gameterm/issues/1950)
* macOS: `CMD-.` was treated as `CTRL-ESC` [#1867](https://github.com/JulianAbeleda/gameterm/issues/1867)
* macOS: `CTRL-Backslash` on German layouts was incorrect [#1891](https://github.com/JulianAbeleda/gameterm/issues/1891)
* `nf-mdi-contacts` nerdfont symbol treated as zero-width [#1864](https://github.com/JulianAbeleda/gameterm/issues/1864)
* X11/Wayland: `CTRL-i`, `CTRL-j`, `CTRL-m` misinterpreted as `CTRL-Tab`, `CTRL-Enter`, `CTRL-Return` [#1851](https://github.com/JulianAbeleda/gameterm/issues/1851)
* Scrollbar stopped working after a lot of output scrolled outside of the scrollback limit.  Thanks to [@davidrios](https://github.com/davidrios)! [#1866](https://github.com/JulianAbeleda/gameterm/pull/1866)
* Windows: uncommitted IME composition could get stuck when switching input methods. [#1922](https://github.com/JulianAbeleda/gameterm/issues/1922)
* OSC sequences, such as those that change the window or tab title, that accept a single string parameter will now join multiple parameters together. This allows window and tab titles to contain a semicolon. Thanks to [@kumattau](https://github.com/kumattau)! [#1939](https://github.com/JulianAbeleda/gameterm/pull/1939)
* Unable to use `ALT` as a modifier for the `leader` key. [#1958](https://github.com/JulianAbeleda/gameterm/issues/1958)
* IME Candidate window position was incorrect. Thanks to [@kumattau](https://github.com/kumattau) and [@aznhe21](https://github.com/aznhe21)! [#1976](https://github.com/JulianAbeleda/gameterm/pull/1976) [#2001](https://github.com/JulianAbeleda/gameterm/pull/2001) [#2022](https://github.com/JulianAbeleda/gameterm/pull/2022)
* Prevent panic for some classes of invalid input, found while fuzzing. Thanks to [@5225225](https://github.com/5225225)! [#1990](https://github.com/JulianAbeleda/gameterm/pull/1990) [#1986](https://github.com/JulianAbeleda/gameterm/pull/1986)
* Detaching an ssh multiplexer domain sometimes killed the associated panes! [#1993](https://github.com/JulianAbeleda/gameterm/issues/1993)
* `DecreaseFontSize` wasn't quite the inverse of `IncreaseFontSize`. Thanks to [@Funami580](https://github.com/Funami580)! [#1997](https://github.com/JulianAbeleda/gameterm/pull/1997)
* Wayland: unable to paste text that was copied before starting the initial gameterm window. Thanks to [@Funami580](https://github.com/Funami580)! [#1994](https://github.com/JulianAbeleda/gameterm/pull/1994) [#1385](https://github.com/JulianAbeleda/gameterm/issues/1385)
* Unicode NFD text could incorrectly render with repeated glyphs [#2032](https://github.com/JulianAbeleda/gameterm/issues/2032)
* Windows: spawning new panes/tabs wouldn't automatically use the working directory of the current pane when OSC 7 was not being used [#2036](https://github.com/JulianAbeleda/gameterm/issues/2036)
* Wayland: panic when display scaling is enabled. [#1727](https://github.com/JulianAbeleda/gameterm/issues/1727)
* `Dark+` color scheme background color [#2013](https://github.com/JulianAbeleda/gameterm/pull/2013)
* Synthesized bold didn't kick in for automatically computed `font_rules`. [#2074](https://github.com/JulianAbeleda/gameterm/issues/2074)
* Added [freetype_pcf_long_family_names](config/lua/config/freetype_pcf_long_family_names.md) option to workaround PCF font naming issues on certain Linux distributions. [#2100](https://github.com/JulianAbeleda/gameterm/issues/2100)
* X11/Wayland: gameterm.desktop now specifies `StartupWMClass`. Thanks to [@uncomfyhalomacro](https://github.com/uncomfyhalomacro)! [#2052](https://github.com/JulianAbeleda/gameterm/issues/2052) [#2125](https://github.com/JulianAbeleda/gameterm/pull/2125)
* `sudo -i` in a pane would cause subsequent pane/tab creation to fail until the cwd was changed to an accessible directory [#2120](https://github.com/JulianAbeleda/gameterm/issues/2120)
* X11: Fixed an issue where some events could get lost around resize events, especially prevalent when using the NVIDIA proprietary drivers. Thanks to [@pjones123](https://github.com/pjones123) and [@yuttie](https://githug.com/yuttie) for working through this! [#1992](https://github.com/JulianAbeleda/gameterm/issues/1992) [#2063](https://github.com/JulianAbeleda/gameterm/issues/2063) [#2111](https://github.com/JulianAbeleda/gameterm/pull/2111) [#1628](https://github.com/JulianAbeleda/gameterm/issues/1628)
* macOS: `SHIFT-Tab` and `CTRL-SHIFT-Tab` produced incorrect results [#1902](https://github.com/JulianAbeleda/gameterm/issues/1902)
* X11: Fixed an issue where copy and paste between two gameterm windows could produce stale results. [#2110](https://github.com/JulianAbeleda/gameterm/issues/2110)
* Mouse selection spanning multiple lines always included the first column even when the mouse was to the left of the first column. Thanks to [@Funami580](https://github.com/Funami580)! [#2106](https://github.com/JulianAbeleda/gameterm/pull/2106)
* Fonts: Codepoints for eg: symbol glyphs that were not explicitly listed in your font fallback list may not be resolved when used in styled (eg: bold) text. [#1913](https://github.com/JulianAbeleda/gameterm/issues/1913) [#2158](https://github.com/JulianAbeleda/gameterm/issues/2158)

### 20220408-101518-b908e2dd

#### New
* [Key Tables](config/key-tables.md) feature for powerful modal key assignments
* `gameterm start --position x,y`, `gameterm start --position displayname:30%,30%` option to control starting window position on all systems except for Wayland. See `gameterm start --help` for more info. [#1794](https://github.com/JulianAbeleda/gameterm/issues/1794)
#### Changed
...
* Disabled ligatures for `"Monaco"` and `"Menlo"` fonts, as those have `"fi"` ligatures that match even for words such as `find`. [#1786](https://github.com/JulianAbeleda/gameterm/issues/1786) [#1736](https://github.com/JulianAbeleda/gameterm/issues/1736)
...
* On macOS, non-native fullscreen mode now attempts to avoid the notch on systems that have one. [#1737](https://github.com/JulianAbeleda/gameterm/issues/1737)
* Sixel parsing performance has been improved
* You may now [specify a scaling factor per fallback font](config/lua/gameterm/font_with_fallback.md#manual-fallback-scaling), which is useful when your preferred CJK font renders smaller than your Roman primary font, for example.
...
* Wayland: touchpad scroll is now more responsive/precise. Thanks to [@davidrios](https://github.com/davidrios)! [#1800](https://github.com/JulianAbeleda/gameterm/pull/1800) [#1840](https://github.com/JulianAbeleda/gameterm/pull/1840)
* Kitty image protocol: now also supports shared memory data transmission. Thanks to [@tantei3](https://github.com/tantei3)! [#1810](https://github.com/JulianAbeleda/gameterm/pull/1810)
* Secondary DA response bumped up to persuade vim to set `ttymouse=sgr` by default. [#1825](https://github.com/JulianAbeleda/gameterm/issues/1825)

#### Fixed
* Incorrect csi-u encoding with non-ascii characters. [#1746](https://github.com/JulianAbeleda/gameterm/issues/1746)
* X11 `_NET_WM_ICON` had red/blue channels swapped [#1754](https://github.com/JulianAbeleda/gameterm/issues/1754)
* ls-fonts output didn't quote the `style` field [#1762](https://github.com/JulianAbeleda/gameterm/issues/1762)
* `window_decorations = "RESIZE"` on Windows prevented minimize/maximize and aerosnap, double click to maximize, and had an ugly top border. Many thanks to [@davidrios](https://github.com/davidrios)! [#1674](https://github.com/JulianAbeleda/gameterm/issues/1674) [#1675](https://github.com/JulianAbeleda/gameterm/pull/1675) [#1771](https://github.com/JulianAbeleda/gameterm/pull/1771)
* On Windows, explorer shortcut icons with the maximized setting would fall out of maximized state on startup. Thanks to [@davidrios](https://github.com/davidrios)! [#1502](https://github.com/JulianAbeleda/gameterm/issues/1502)
* `LANG` environment variable was not always set on macOS, leading to mojibake when entering CJK.  [#1761](https://github.com/JulianAbeleda/gameterm/issues/1761) [#1765](https://github.com/JulianAbeleda/gameterm/issues/1765)
* Fonts with only non-unicode names (eg: only using a Chinese multibyte string encoding) were treated as having names like `?????` and were not accessible. [#1761](https://github.com/JulianAbeleda/gameterm/issues/1761)
* Hover state of leftmost retro style tab was overly sticky when the mouse moved out of the tab. [#1764](https://github.com/JulianAbeleda/gameterm/issues/1764)
* On macOS, the font size could incorrectly double or halve after waking from sleep or moving the window to/from an external monitor. [#1566](https://github.com/JulianAbeleda/gameterm/issues/1566) [#1745](https://github.com/JulianAbeleda/gameterm/issues/1745)
* On Windows, touchpad scrolling was janky. Many thanks to [@davidrios](https://github.coim/davidrios)! [#1773](https://github.com/JulianAbeleda/gameterm/pull/1773) [#1725](https://github.com/JulianAbeleda/gameterm/pull/1725) [#949](https://github.com/JulianAbeleda/gameterm/pull/949)
* X11: workaround i3-gaps not sending initial CONFIGURE_NOTIFY or FOCUS events, leading to weird initial window size and broken focus status. [#1710](https://github.com/JulianAbeleda/gameterm/issues/1710) [#1757](https://github.com/JulianAbeleda/gameterm/issues/1757)
* Hyperlink rules with more captures than replacements could panic gameterm when text matched. [#1780](https://github.com/JulianAbeleda/gameterm/issues/1780)
* Malformed XTGETTCAP response. [#1781](https://github.com/JulianAbeleda/gameterm/issues/1781)
* Multiplexer performance with images was unusable for all but tiny images. [#1237](https://github.com/JulianAbeleda/gameterm/issues/1237)
* `CloseCurrentPane{confirm=false}` would leave behind a phantom tab/pane when used with the multiplexer. [#1277](https://github.com/JulianAbeleda/gameterm/issues/1277)
* `CloseCurrentPane{confirm=true}` artifacts when used with the multiplexer. [#783](https://github.com/JulianAbeleda/gameterm/issues/783)
* Scrollbar thumb could jump around/move out of bounds. Thanks to [@davidrios](https://github.com/davidrios)! [#1525](https://github.com/JulianAbeleda/gameterm/issues/1525)
* OSC 52 could stop working for tabs/panes spawned into the GUI via the CLI. [#1790](https://github.com/JulianAbeleda/gameterm/issues/1790)
* Workaround for fonts with broken horizontal advance metrics [#1787](https://github.com/JulianAbeleda/gameterm/issues/1787)
* Improved mouse based selection. Thanks to [@davidrios](https://github.com/davidrios)! [#1805](https://github.com/JulianAbeleda/gameterm/issues/1805) [#1199](https://github.com/JulianAbeleda/gameterm/issues/1199) [#1386](https://github.com/JulianAbeleda/gameterm/issues/1386) [#354](https://github.com/JulianAbeleda/gameterm/issues/354)
* X11 `KP_End` wasn't recognized [#1804](https://github.com/JulianAbeleda/gameterm/issues/1804)
* fontconfig matches now also treat `"charcell"` spacing as monospace. [#1820](https://github.com/JulianAbeleda/gameterm/issues/1820)
* Multiplexer render update laggy, especially when using multiple windows. [#1814](https://github.com/JulianAbeleda/gameterm/issues/1814) [#1841](https://github.com/JulianAbeleda/gameterm/issues/1841)

### 20220319-142410-0fcdea07

#### New

* [window:composition_status()](config/lua/window/composition_status.md) and [window:leader_is_active()](config/lua/window/leader_is_active.md) methods that can help populate [window:set_right_status()](config/lua/window/set_right_status.md) [#686](https://github.com/JulianAbeleda/gameterm/issues/686)
...
* Support for SGR-Pixels mouse reporting. Thanks to [Autumn Lamonte](https://gitlab.com/autumnmeowmeow)! [#1457](https://github.com/JulianAbeleda/gameterm/issues/1457)
* [ActivatePaneByIndex](config/lua/keyassignment/ActivatePaneByIndex.md) key assignment action. [#1517](https://github.com/JulianAbeleda/gameterm/issues/1517)
* Windows: gameterm may now use [win32-input-mode](https://github.com/microsoft/terminal/blob/main/doc/specs/%234999%20-%20Improved%20keyboard%20handling%20in%20Conpty.md) to send high-fidelity keyboard input to ConPTY. This means that win32 console applications, such as [FAR Manager](https://github.com/FarGroup/FarManager) that use the low level `INPUT_RECORD` API will now receive key-up events as well as events for modifier-only key presses. Use `allow_win32_input_mode=true` to enable this. [#318](https://github.com/JulianAbeleda/gameterm/issues/318) [#1509](https://github.com/JulianAbeleda/gameterm/issues/1509) [#1510](https://github.com/JulianAbeleda/gameterm/issues/1510)
* Windows: [default_domain](config/lua/config/default_domain.md), [wsl_domains](config/lua/config/wsl_domains.md) options and [gameterm.default_wsl_domains()](config/lua/gameterm/default_wsl_domains.md) provide more flexibility for WSL users. The effect of `add_wsl_distributions_to_launch_menu=false` was replaced by `wsl_domains={}`.
* `Symbols Nerd Font Mono` is now bundled with GameTerm and is included as a default fallback font. This means that you may use any of the glyphs available in the [Nerd Fonts](https://github.com/ryanoasis/nerd-fonts) collection with any font without patching fonts and without explicitly adding that font to your fallback list. Pomicons have an unclear license for distribution and are excluded from this bundled font, however, you may manually install the font with those icons from the Nerd Font site itself and it will take precedence over the bundled font.  This font replaces the older `PowerlineExtraSymbols` font.  [#1521](https://github.com/JulianAbeleda/gameterm/issues/1521).
* [gameterm.nerdfonts](config/lua/gameterm/nerdfonts.md) as a convenient way to resolve Nerd Fonts glyphs by name in your config file
...
* Workspaces. Follow work in progress on [#1531](https://github.com/JulianAbeleda/gameterm/issues/1531) and [#1322](https://github.com/JulianAbeleda/gameterm/discussions/1322)! [window:active_workspace()](config/lua/window/active_workspace.md), [default_workspace](config/lua/config/default_workspace.md), [SwitchWorkspaceRelative](config/lua/keyassignment/SwitchWorkspaceRelative.md), [SwitchToWorkspace](config/lua/keyassignment/SwitchToWorkspace.md)
* `gameterm cli send-text "hello"` allows sending text, as though pasted, to a pane. See `gameterm cli send-text --help` for more information. [#888](https://github.com/JulianAbeleda/gameterm/issues/888)
* `local_echo_threshold_ms` option to adjust the predictive local echo timing for [SshDomain](config/lua/SshDomain.md), [TlsDomainClient](config/lua/TlsDomainClient.md) and [unix domains](multiplexing.md). Thanks to [@qperret](https://github.com/qperret)! [#1518](https://github.com/JulianAbeleda/gameterm/pull/1518)
* It is now possible to set `selection_fg` and `selection_bg` to be fully or partially transparent. [Read more](config/appearance.md). [#1615](https://github.com/JulianAbeleda/gameterm/issues/1615)
* Experimental (and incomplete!) support for Bidi/RTL can be enabled through the config. [Follow along in the tracking issue](https://github.com/JulianAbeleda/gameterm/issues/784)
* Primary selection is now supported on Wayland systems that implement [primary-selection-unstable-v1](https://wayland.app/protocols/primary-selection-unstable-v1) or the older Gtk primary selection protocol. Thanks to [@lunaryorn](https://github.com/lunaryorn)! [#1423](https://github.com/JulianAbeleda/gameterm/issues/1423)
* [pane:has_unseen_output()](config/lua/pane/has_unseen_output.md) and [PaneInformation.has_unseen_output](config/lua/PaneInformation.md) allow coloring or marking up tabs based on unseen output. [#796](https://github.com/JulianAbeleda/gameterm/discussions/796)
* Context menu extension for Nautilus. Thanks to [@lunaryorn](https://github.com/lunaryorn)! [#1092](https://github.com/JulianAbeleda/gameterm/issues/1092)
* [gameterm.enumerate_ssh_hosts()](config/lua/gameterm/enumerate_ssh_hosts.md) function that can be used to auto-generate ssh domain configuration

#### Changed

* **Key Assignments now use Physical Key locations by default!!** [Read more](config/keys.md#physical-vs-mapped-key-assignments) [#1483](https://github.com/JulianAbeleda/gameterm/issues/1483) [#601](https://github.com/JulianAbeleda/gameterm/issues/601) [#1080](https://github.com/JulianAbeleda/gameterm/issues/1080) [#1391](https://github.com/JulianAbeleda/gameterm/issues/1391)
* Key assignments now match prior to any dead-key or IME composition [#877](https://github.com/JulianAbeleda/gameterm/issues/877)
* Removed the `ALT-[NUMBER]` default key assignments as they are not good for non-US layouts. [#1542](https://github.com/JulianAbeleda/gameterm/issues/1542)
* `gameterm cli`, when run outside of a gameterm pane, now prefers to connect to the main GUI instance rather than background mux server. Use `gameterm cli --prefer-mux` to ignore the GUI instance and talk only to the mux server. See `gameterm cli --help` for additional information.
* [ScrollByPage](config/lua/keyassignment/ScrollByPage.md) now accepts fractional numbers like `0.5` to scroll by half a page at time. Thanks to [@hahuang65](https://github.com/hahuang65)! [#1534](https://github.com/JulianAbeleda/gameterm/pull/1534)
...
* [canonicalize_pasted_newlines](config/lua/config/canonicalize_pasted_newlines.md) default has changed to be more compatible for `nano` users, and now provides more control over the text format that is pasted. [#1575](https://github.com/JulianAbeleda/gameterm/issues/1575)
...
* macOS: `use_ime=true` no longer prevents key repeat from working with some keys [#1131](https://github.com/JulianAbeleda/gameterm/issues/1131)
* Bundled harfbuzz to 4.0.1

#### Fixed

* Regression that broke fontconfig aliases such as `"monospace"` [#1250](https://github.com/JulianAbeleda/gameterm/pull/1250)
* Windows/X11/Wayland: CTRL+C in non-latin keyboard layouts wouldn't send CTRL+C [#678](https://github.com/JulianAbeleda/gameterm/issues/678)
* The new tab button in the fancy tab didn't respect `new_tab_hover` colors [#1498](https://github.com/JulianAbeleda/gameterm/issues/1498)
* Font baseline alignment when mixing symbols/emoji with the main text [#1499](https://github.com/JulianAbeleda/gameterm/issues/1499)
* Glitchy window resize [#1491](https://github.com/JulianAbeleda/gameterm/issues/1491)
* Ligatured glyphs no longer turn partially black when cursoring through them [#478](https://github.com/JulianAbeleda/gameterm/issues/478)
* Kitty Image Protocol: didn't respect `c` and `r` parameters to scale images
* Cursor location on the primary screen wasn't updated correctly if the window was resized while the alternate screen was active [#1512](https://github.com/JulianAbeleda/gameterm/issues/1512)
* Windows: latency issue with AltSnap and other window-managery things [#1013](https://github.com/JulianAbeleda/gameterm/issues/1013) [#1398](https://github.com/JulianAbeleda/gameterm/issues/1398) [#1075](https://github.com/JulianAbeleda/gameterm/issues/1075) [#1099](https://github.com/JulianAbeleda/gameterm/issues/1099)
* Multiplexer sessions now propagate user vars [#1528](https://github.com/JulianAbeleda/gameterm/issues/1528)
* Config reloads on the multiplexer server didn't cause the palette to update on the client [#1526](https://github.com/JulianAbeleda/gameterm/issues/1528)
* [ScrollToPrompt](config/lua/keyassignment/ScrollToPrompt.md) could get confused when there were multiple prompts on the same line [#1121](https://github.com/JulianAbeleda/gameterm/issues/1121)
* Hangul text in NFD was not always correctly composed when shaping fonts. [#1573](https://github.com/JulianAbeleda/gameterm/issues/1573)
* Avoid OOM when processing sixels with huge repeat counts [#1610](https://github.com/JulianAbeleda/gameterm/issues/1610)
* Set the sticky bit on socket and pid files created in `XDG_RUNTIME_DIR` to avoid removal by tmpwatch [#1601](https://github.com/JulianAbeleda/gameterm/issues/1601)
* Shaping combining sequences like `e U+20d7` could "lose" the vector symbol if the font produced an entry with no `x_advance`. [#1617](https://github.com/JulianAbeleda/gameterm/issues/1617)
* Setting the cursor color via escape sequences now take precedence over `force_reverse_video_cursor`. [#1625](https://github.com/JulianAbeleda/gameterm/issues/1625)
* Fixed Detection of DECSDM support via DECRQM/DECRPM, Correct sixel image placement when DECSDM is set and VT340 default sixel colors. Thanks to [Autumn](https://github.com/autumnmeowmeow)! [#1577](https://github.com/JulianAbeleda/gameterm/pull/1577)
* Fixed missing whitespace from intermediate lines when copying a wrapped logical line [#1635](https://github.com/JulianAbeleda/gameterm/issues/1635)
* Unable to match `Iosevka Term` when multiple iosevka ttc files were installed on macOS [#1630](https://github.com/JulianAbeleda/gameterm/issues/1630)
* Incorrect umask for panes spawned via the multiplexer server [#1633](https://github.com/JulianAbeleda/gameterm/issues/1633)
* Fall back from `top_left_arrow` to `left_ptr` when loading XCursor themes [#1655](https://github.com/JulianAbeleda/gameterm/issues/1655)
* Fixed lingering hover state in titlebar when the mouse pointer left the window. Thanks to [@davidrios](https://github.com/davidrios)! [#1434](https://github.com/JulianAbeleda/gameterm/issues/1434)
* We now respect the difference between `Italic` and `Oblique` font styles when matching fonts. You may explicitly specify `style="Oblique"` rather than using `italic=true` for fonts that offer both italic and oblique variants. [#1646](https://github.com/JulianAbeleda/gameterm/issues/1646)
* Hang when clicking a URL would launch the browser for the first time on unix systems [#1721](https://github.com/JulianAbeleda/gameterm/issues/1721)
* Wayland input handling gets broken after suspend/resume. Thanks to [@LawnGnome](https://github.com/LawnGnome)! [#1497](https://github.com/JulianAbeleda/gameterm/issues/1497)
...
* [ScrollToTop](config/lua/keyassignment/ScrollToTop.md) and [ScrollToBottom](config/lua/keyassignment/ScrollToBottom.md) key assignments [#1360](https://github.com/JulianAbeleda/gameterm/issues/1360)
* [SSH Domains](config/lua/SshDomain.md) now support specifying `ssh_config` overrides. [#1149](https://github.com/JulianAbeleda/gameterm/issues/1149)
* [default_gui_startup_args](config/lua/config/default_gui_startup_args.md) allows defaulting to starting the ssh client (for example). [#1030](https://github.com/JulianAbeleda/gameterm/issues/1030)
* [mux-is-process-stateful](config/lua/mux-events/mux-is-process-stateful.md) event for finer control over prompting when closing panes. [#1412](https://github.com/JulianAbeleda/gameterm/issues/1412)
* [harfbuzz_features](config/font-shaping.md), [freetype_load_target](config/lua/config/freetype_load_target.md), [freetype_render_target](config/lua/config/freetype_render_target.md) and [freetype_load_flags](config/lua/config/freetype_load_flags.md) can now be overridden on a per-font basis as described in [gameterm.font](config/lua/gameterm/font.md) and [gameterm.font_with_fallback](config/lua/gameterm/font_with_fallback.md).
* [ActivateTabRelativeNoWrap](config/lua/keyassignment/ActivateTabRelativeNoWrap.md) key assignment [#1414](https://github.com/JulianAbeleda/gameterm/issues/1414)
* [QuickSelectArgs](config/lua/keyassignment/QuickSelectArgs.md) key assignment [#846](https://github.com/JulianAbeleda/gameterm/issues/846) [#1362](https://github.com/JulianAbeleda/gameterm/issues/1362)
* [gameterm.open_with](config/lua/gameterm/open_with.md) function for opening URLs/documents with the default or a specific application [#1362](https://github.com/JulianAbeleda/gameterm/issues/1362)
* [pane:get_foreground_process_name()](config/lua/pane/get_foreground_process_name.md) method, [PaneInformation](config/lua/PaneInformation.md) now has `foreground_process_name` and `current_working_dir` fields, and [pane:get_current_working_dir](config/lua/pane/get_current_working_dir.md) is now supported on Windows for local processes, even without using OSC 7. [#1421](https://github.com/JulianAbeleda/gameterm/discussions/1421) [#915](https://github.com/JulianAbeleda/gameterm/issues/915) [#876](https://github.com/JulianAbeleda/gameterm/issues/876)
* [ActivatePaneDirection](config/lua/keyassignment/ActivatePaneDirection.md) now also supports `"Next"` and `"Prev"` to cycle through panes [#976](https://github.com/JulianAbeleda/gameterm/issues/976)
* [pane:get_logical_lines_as_text](config/lua/pane/get_logical_lines_as_text.md) to retrieve unwrapped logical lines from a pane [#1468](https://github.com/JulianAbeleda/gameterm/issues/1468)
* [gameterm.get_builtin_color_schemes()](config/lua/gameterm/get_builtin_color_schemes.md) function to eg: pick a random scheme per window, or otherwise reason about schemes. See [the docs](config/lua/gameterm/get_builtin_color_schemes.md) for examples!
...
* quickselect: we now de-duplicate labels for results with the same textual content. [#1271](https://github.com/JulianAbeleda/gameterm/issues/1271)
* The default `CompleteSelectionOrOpenLinkAtMouseCursor` left button release assignment now also accepts SHIFT being held in order to make SHIFT-click `ExtendSelectionToMouseCursor` feel more ergonomic if the mouse button is released before the SHIFT key. [#1204](https://github.com/JulianAbeleda/gameterm/issues/1204)
* Unicode BIDI and other zero-width graphemes are now filtered out from the terminal model. It's not ideal in the sense that that information is essentially lost when copying to the clipboard, but it makes the presentation correct. [#1422](https://github.com/JulianAbeleda/gameterm/issues/1422)
...
* Bundled NotoColorEmoji to 2.034 (with Unicode 14 support) Thanks to [@4cm4k1](https://github.com/4cm4k1)! [#1440](https://github.com/JulianAbeleda/gameterm/pull/1440)
* macos: removing the titlebar from `window_decorations` now preserves rounded window corners [#1034](https://github.com/JulianAbeleda/gameterm/issues/1034)
* Colors can now be specified in the HSL colorspace using syntax like `"hsl:235 100 50"` [#1436](https://github.com/JulianAbeleda/gameterm/issues/1436)
* Line/Bar cursors in [force_reverse_video_cursor](config/lua/config/force_reverse_video_cursor.md) mode now use the text foreground color rather than the cursor border color. [#1076](https://github.com/JulianAbeleda/gameterm/issues/1076)
* Improved logo appearance. Thanks to [@ghifarit53](https://github.com/ghifarit53)! [#1454](https://github.com/JulianAbeleda/gameterm/pull/1454)
* You can now pass [SendKey](config/lua/keyassignment/SendKey.md) to [gameterm.action](config/lua/gameterm/action.md) and make your `keys` config look more consistent
* Mouse wheel events are now routed to the hovered pane, rather than sent to the focused pane [#798](https://github.com/JulianAbeleda/gameterm/issues/798)

#### Fixed

* DECSTR (terminal soft reset) now turns off DECLRMM (left and right margin mode). Thanks to [@ninjalj](https://github.com/ninjalj)! [#1376](https://github.com/JulianAbeleda/gameterm/pull/1376)
* Improved conformance of CUP, HVP, SLRM, STBM escape sequences by support empty first parameter. Thanks to [@ninjalj](https://github.com/ninjalj)! [#1377](https://github.com/JulianAbeleda/gameterm/pull/1377)
* tab bar didn't correctly handle double-wide cells and could truncate at edges when using `format-tab-title` [#1371](https://github.com/JulianAbeleda/gameterm/issues/1371)
* `gameterm cli --no-auto-start` was not respected
* Pixel geometry configured on the PTY in new windows could be incorrect on HiDPI displays until the window was resized [#1387](https://github.com/JulianAbeleda/gameterm/issues/1387)
* Image attachment geometry for imgcat and sixels could stretch the image across the rounded up number of cells that contained the image. [#1300](https://github.com/JulianAbeleda/gameterm/issues/1300), [#1270](https://github.com/JulianAbeleda/gameterm/issues/1270)
* Closing a split pane created inside a `gameterm ssh` session wouldn't actually close the pane [#1197](https://github.com/JulianAbeleda/gameterm/issues/1197)
* Clicking when unfocused could lead to unwanted text selection [#1140](https://github.com/JulianAbeleda/gameterm/issues/1140) [#1310](https://github.com/JulianAbeleda/gameterm/issues/1310)
* Changing font scaling on Windows no longer causes the initial terminal rows/cols to be under-sized [#1381](https://github.com/JulianAbeleda/gameterm/issues/1381)
* New version update notifications are now more coordinated between multiple gameterm GUI instances, and update related configuration now respects configuration reloading. [#1402](https://github.com/JulianAbeleda/gameterm/issues/1402)
...
* Closing a window will no longer recursively terminate contained multiplexer client panes; the window will instead be restored when you next connect to that multiplexer server. Killing/closing individual tabs/panes *will* kill the panes; this change only affects closing the window. [#848](https://github.com/JulianAbeleda/gameterm/issues/848) [#917](https://github.com/JulianAbeleda/gameterm/issues/917) [#1224](https://github.com/JulianAbeleda/gameterm/issues/1224)
* Colors were too intense due to over gamma correction [#1025](https://github.com/JulianAbeleda/gameterm/issues/1025)
* Mesa and EGL colors were too dim due to under gamma correction [#1373](https://github.com/JulianAbeleda/gameterm/issues/1373)
* `gameterm ssh` no longer tries to use `default_prog` or `default_cwd` when spawning additional panes on the remote host [#1456](https://github.com/JulianAbeleda/gameterm/issues/1456)
* Launcher menu WSL items now launch correctly on non-US versions of Windows [#1462](https://github.com/JulianAbeleda/gameterm/issues/1462)
* Korean text in NFD form is now correctly sized and rendered [#1474](https://github.com/JulianAbeleda/gameterm/issues/1474)
* macOS: `use_ime=true` conflicted with `LEADER` key assignments [#1409](https://github.com/JulianAbeleda/gameterm/issues/1409)
* macOS: certain keys (eg: `F8` and `F9`) did nothing when `use_ime=true`. [#975](https://github.com/JulianAbeleda/gameterm/issues/975)
* Splitting a tab would cause the window to lose its transparency [#1459](https://github.com/JulianAbeleda/gameterm/issues/1459)

### 20211205-192649-672c1cc1

#### Fixed

* Windows: `gameterm <something>` would fail silently when spawning `gameterm-gui` under the covers. Regression introduced by [#1278](https://github.com/JulianAbeleda/gameterm/issues/1278). Workaround is to directly spawn `gameterm-gui`.
* Windows: the PTY handles were ignored in favor of the redirected stdio handles of the parent of the gameterm mux process [#1358](https://github.com/JulianAbeleda/gameterm/issues/1358)
* Windows: Failure to spawn `gameterm` when launching an ssh mux domain session no longer waits forever
* "Update available" message kept showing even though already running the latest version [#1365](https://github.com/JulianAbeleda/gameterm/issues/1365)

### 20211204-082213-a66c61ee9

#### New

* X11 now supports IME. It currently defaults to disabled, but you can set `use_ime = true` in your config to enable it (you need to restart gameterm for this to take effect). Many thanks to [@H-M-H](https://github.com/H-M-H) for bringing xcb-imdkit to Rust and implementing this in gameterm! [#250](https://github.com/JulianAbeleda/gameterm/issues/250) [#1043](https://github.com/JulianAbeleda/gameterm/pull/1043)
* it is now possible to define colors in the range 16-255 in `colors` and color scheme definitions. Thanks to [@potamides](https://github.com/potamides)! [#841](https://github.com/JulianAbeleda/gameterm/issues/841) [#1056](https://github.com/JulianAbeleda/gameterm/pull/1056)
...
* [visual_bell](config/lua/config/visual_bell.md) and [audible_bell](config/lua/config/audible_bell.md) configuration options, as well as a [bell](config/lua/window-events/bell.md) event allows you to trigger lua code when the bell is rung. [#3](https://github.com/JulianAbeleda/gameterm/issues/3)
* [gameterm.action_callback](config/lua/gameterm/action_callback.md) function to make it easier to use custom events. Thanks to [@bew](https://github.com/bew)! [#1151](https://github.com/JulianAbeleda/gameterm/pull/1151)
* `gameterm connect` now also supports the `--class` parameter to override the window class
...
* BSDish systems now support [toast notifications](https://github.com/JulianAbeleda/gameterm/issues/489)
* [gameterm.background_child_process](config/lua/gameterm/background_child_process.md) function to spawn a process without waiting.
* [mux_env_remove](config/lua/config/mux_env_remove.md) setting to control which environment variables should be cleared prior to spawning processes in the multiplexer server [#1225](https://github.com/JulianAbeleda/gameterm/issues/1225)
* [canonicalize_pasted_newlines](config/lua/config/canonicalize_pasted_newlines.md) option to help Windows users manage newlines in pastes [#1213](https://github.com/JulianAbeleda/gameterm/issues/1213)
* SSH client now uses `libssh` by default. [ssh_backend](config/lua/config/ssh_backend.md) can be used to change this.
* [unzoom_on_switch_pane](config/lua/config/unzoom_on_switch_pane.md) option. Thanks to [@yyogo](https://github.com/yyogo) [#1301](https://github.com/JulianAbeleda/gameterm/issues/1301)
...
* macOS: binaries produced by gameterm's CI are now codesigned, which resolves some spurious permission dialogs that affected some users [#482](https://github.com/JulianAbeleda/gameterm/issues/482)

#### Changed

* new default key assignments: CTRL+PageUp and CTRL+Tab activate next tab, CTRL+PageDown and CTRL+SHIFT+Tab activate previous tab. ALT+{1..8} directly select the first through 8th tabs. Thanks to [@friederbluemle](https://github.com/friederbluemle)! [#1132](https://github.com/JulianAbeleda/gameterm/pull/1132)
* X11: we now allow matching visuals with >= 8 bits per rgb value. Previously, we only matched exactly 8 bits. This improve compatibility with systems that have the COMPOSITE extension disabled. Thanks to [@shizeeg](https://github.com/shizeeg)! [#1083](https://github.com/JulianAbeleda/gameterm/pull/1083)
* The incomplete `Allsorts` shaper was removed.
* Windows: `gameterm-gui.exe` now only grabs the parent process' console handle when spawned from `gameterm.exe`, which prevents some frustrating interactions when launching `gameterm-gui.exe` via `start` in cmd/powershell. [#1278](https://github.com/JulianAbeleda/gameterm/issues/1278)
* AppImage: take care to remove APPIMAGE related environment when spawning child processes. Thanks to [@srevinsaju](https://github.com/srevinsaju)! [#1338](https://github.com/JulianAbeleda/gameterm/pull/1338)

#### Updated and Improved

* bundled harfbuzz updated to version 3.0.0, bundled freetype updated to 2.11
* window close confirmations now accept both uppercase and lowercase Y/N key presses. Thanks to [@SpyrosRoum](https://github.com/SpyrosRoum)! [#1119](https://github.com/JulianAbeleda/gameterm/pull/1119)
* multi-click-streaks are now interrupted by the cursor moving to a different cell. Thanks to [@valpackett](https://github.com/valpackett)! [#1126](https://github.com/JulianAbeleda/gameterm/issues/1126)
* `.deb` packages now `Provides: x-terminal-emulator`. [#1139](https://github.com/JulianAbeleda/gameterm/issues/1139)
* [use_cap_height_to_scale_fallback_fonts](config/lua/config/use_cap_height_to_scale_fallback_fonts.md) now computes *cap-height* based on the rasterized glyph bitmap which means that the data is accurate in more cases, including for bitmap fonts.  Scaling is now also applied across varying text styles; previously it only applied to a font within an `gameterm.font_with_fallback` font list.
* Can now match fontconfig aliases, such as `monospace`, on systems that use fontconfig. Thanks to [@valpackett](https://github.com/valpackett)! [#1149](https://github.com/JulianAbeleda/gameterm/issues/1149)
* Powerline semicircle glyphs now look much better. Thanks to [@bew](https://github.com/bew) and [@sdrik](https://github.com/sdrik)! [#1311](https://github.com/JulianAbeleda/gameterm/issues/1311)
* Splits now look better, especially when using escape sequences to change the default background color [#1256](https://github.com/JulianAbeleda/gameterm/issues/1256)

#### Fixed

* `gameterm cli spawn` would use the initial terminal size for a new tab, rather than using the current tab size [#920](https://github.com/JulianAbeleda/gameterm/issues/920)
* `text_background_opacity` opacity was not respected
* spawning commands via the mux didn't respect the `PATH` configured in `set_environment_variables`. [#1029](https://github.com/JulianAbeleda/gameterm/issues/1029)
* cursor could have a transparent "hole" through the window with certain cursor styles
* Consolas font + random input could cause a divide-by-zero when computing glyph metrics [#1042](https://github.com/JulianAbeleda/gameterm/issues/1042)
...
* X11: laggy input after selecting text. [#1027](https://github.com/JulianAbeleda/gameterm/issues/1027)
* macOS: `send_composed_key_when_left_alt_is_pressed` and `send_composed_key_when_right_alt_is_pressed` are now respected when `use_ime=true`. Thanks to [@jakelinnzy](https://github.com/jakelinnzy)! [#1096](https://github.com/JulianAbeleda/gameterm/pull/1096)
* X11: jittery resize with some window managers [#1051](https://github.com/JulianAbeleda/gameterm/issues/1051)
* X11: [window:get_appearance](config/lua/window/get_appearance.md) now actually returns Dark when the theme is dark. [#1098](https://github.com/JulianAbeleda/gameterm/issues/1098)
* ALT + Arrow, PageUp/PageDown, Ins, Del, Home, End incorrectly sent ESC prefixed key sequences. [#892](https://github.com/JulianAbeleda/gameterm/issues/892)
* Crash due to Out of Memory condition when the iTerm2 protocol was used to send excessively large PNG files [#1031](https://github.com/JulianAbeleda/gameterm/issues/1031)
* `DCH` (delete char) sequence would remove cells and replace them with default-blank cells instead of blank-cells-with-current-bg-color. [#789](https://github.com/JulianAbeleda/gameterm/issues/789)
* invisible I-beam or underline cursor when `force_reverse_video_cursor = true` [#1076](https://github.com/JulianAbeleda/gameterm/issues/1076)
* `SU` (scroll up) sequence would fill with default-blank cells instead of blank-cells-with-current-bg-color. [#1102](https://github.com/JulianAbeleda/gameterm/issues/1102)
* X11: computed but did not use the correct DPI for HiDPI screens [#947](https://github.com/JulianAbeleda/gameterm/issues/947)
...
* Wayland: incorrect initial surface size for HiDPI screens. Thanks to [@valpackett](https://github.com/valpackett)! [#1111](https://github.com/JulianAbeleda/gameterm/issues/1111) [#1112](https://github.com/JulianAbeleda/gameterm/pull/1112)
* invisible cursor in CopyMode when using kakoune [#1113](https://github.com/JulianAbeleda/gameterm/issues/1113)
* Wayland: `bypass_mouse_reporting_modifiers` didn't work. Thanks to [@valpackett](https://github.com/valpackett)! [#1122](https://github.com/JulianAbeleda/gameterm/issues/1122)
* new tabs could have the wrong number of rows and columns if a tiling WM resizes the window before OpenGL has been setup. [#1074](https://github.com/JulianAbeleda/gameterm/issues/1074)
* Wayland: dragging the window using the tab bar now works. Thanks to [@valpackett](https://github.com/valpackett)! [#1127](https://github.com/JulianAbeleda/gameterm/issues/1127)
* error matching a font when that font is in a .ttc that contains multiple font families. [#1137](https://github.com/JulianAbeleda/gameterm/issues/1137)
* Wayland: panic with most recent wlroots. Thanks to [@valpackett](https://github.com/valpackett)! [#1144](https://github.com/JulianAbeleda/gameterm/issues/1144)
* incorrect spacing for IDEOGRAPHIC SPACE. [#1161](https://github.com/JulianAbeleda/gameterm/issues/1161)
* italic fonts weren't always recognized as being italic, resulting in italic variants being used instead of the non-italic variants in some cases! [#1162](https://github.com/JulianAbeleda/gameterm/issues/1162)
* Ask freetype for cell metrics in bitmap-only fonts, rather than simply taking the bitmap width. [#1165](https://github.com/JulianAbeleda/gameterm/issues/1165)
* gameterm can now match bitmap fonts that are spread across multiple font files [#1189](https://github.com/JulianAbeleda/gameterm/issues/1189)
* ssh config parser incorrectly split `Host` patterns with commas instead of whitespace [#1196](https://github.com/JulianAbeleda/gameterm/issues/1196)
* search now auto-updates when the pane content changes [#1205](https://github.com/JulianAbeleda/gameterm/issues/1205)
* fonts with emoji presentation are shifted to better align with the primary font baseline [#1203](https://github.com/JulianAbeleda/gameterm/issues/1203)
* the whole tab was closed when only the zoomed pane exited. [#1235](https://github.com/JulianAbeleda/gameterm/issues/1235)
* multiplexer: wrong `GAMETERM_UNIX_SOCKET` environment passed to children when using unix domain sockets and `connect_automatically=true` [#1222](https://github.com/JulianAbeleda/gameterm/issues/1222)
* multiplexer: spawning remote tabs didn't correctly record local tab mapping, resulting in phantom additional tabs showing in the client. [#1222](https://github.com/JulianAbeleda/gameterm/issues/1222)
* `gameterm ls-fonts --text "✘"` didn't account for the system fallback list. [#849](https://github.com/JulianAbeleda/gameterm/issues/849)
* macOS: The `Menlo` font is now implicitly included in the system fallback list, as it is the only font that contains U+2718 ✘
* `gameterm cli spawn --cwd ..` and `gameterm cli split-pane --cwd ..` now resolve relative paths [#1243](https://github.com/JulianAbeleda/gameterm/issues/1243)
* Incorrect DECRPTUI response to DA3. Thanks to [@ninjalj](https://github.com/ninjalj)! [#1330](https://github.com/JulianAbeleda/gameterm/pull/1330)
* Reloading config now loads newly defined multiplexer domains, however, existing domains are not updated. [#1279](https://github.com/JulianAbeleda/gameterm/issues/1279)
...
* x11: support for [VoidSymbol](config/keys.md#voidsymbol) in key assignments. Thanks to [@digitallyserviced](https://github.com/digitallyserviced)! [#759](https://github.com/JulianAbeleda/gameterm/pull/759)
* Fixed: UTF8-encoded-C1 control codes were not always recognized as control codes, and could result in a panic when later attempting to update the line. [#768](https://github.com/JulianAbeleda/gameterm/issues/768)
* Fixed: `gameterm cli split-pane` didn't use the current working dir of the source pane. [#766](https://github.com/JulianAbeleda/gameterm/issues/766)
* Fixed: double-click-drag selection could panic when crossing line boundaries [#762](https://github.com/JulianAbeleda/gameterm/issues/762)
* Fixed: wrong scaling for ligatures in Recursive Mono font [#699](https://github.com/JulianAbeleda/gameterm/issues/699)
* Fixed: incorrect Sixel HLS hue handling [#775](https://github.com/JulianAbeleda/gameterm/issues/775)
* Fixed: we now recognize the `CSI 48:2:0:214:255m` form of specifying true color text attributes [#785](https://github.com/JulianAbeleda/gameterm/issues/785)
* Fixed: split separators didn't respect `tab_bar_at_bottom=true` and were rendered in the wrong place [#797](https://github.com/JulianAbeleda/gameterm/issues/797)
* Improved: messaging around [exit_behavior](https://gameterm.org/config/lua/config/exit_behavior.md)
* Fixed: errors loading custom color schemes are now logged to the error log [#794](https://github.com/JulianAbeleda/gameterm/issues/794)
* Fixed: OSC 7 (current working directory) now works with paths that contain spaces and other special characters. Thanks to [@Arvedui](https://github.com/Arvedui)! [#799](https://github.com/JulianAbeleda/gameterm/pull/799)
...
* New: bold/dim and/or italics are now synthesized for fonts when the matching font is not actually italic or doesn't match the requested weight. [#815](https://github.com/JulianAbeleda/gameterm/issues/815)
...
* Fixed: Windows: first character after `gameterm ssh` keyboard authention was swallowed [#771](https://github.com/JulianAbeleda/gameterm/issues/771)
* Fixed: Windows: detect window resizes while authenticating for `gameterm ssh` [#696](https://github.com/JulianAbeleda/gameterm/issues/696)
* Fixed: OSC 52 clipboard escape didn't work in the initial pane spawned in the multiplexer server [#764](https://github.com/JulianAbeleda/gameterm/issues/764)
* Fixed: splitting panes in multiplexer could fail after a network reconnect [#781](https://github.com/JulianAbeleda/gameterm/issues/781)
* Fixed: multiplexer now propagates toast notifications and color palette to client [#489](https://github.com/JulianAbeleda/gameterm/issues/489) [#748](https://github.com/JulianAbeleda/gameterm/issues/748)
* Fixed: neovim interprets drags as double clicks [#823](https://github.com/JulianAbeleda/gameterm/discussions/823)
* New: `CTRL+SHIFT+L` is assigned to [ShowDebugOverlay](config/lua/keyassignment/ShowDebugOverlay.md) [#641](https://github.com/JulianAbeleda/gameterm/issues/641)
* Improved: antialiasing for undercurl. Thanks to [@ModProg](https://github.com/ModProg)! [#838](https://github.com/JulianAbeleda/gameterm/pull/838)
* Fixed: `gameterm start --cwd c:/` didn't run `default_prog`. Thanks to [@exactly-one-kas](https://github.com/exactly-one-kas)! [#851](https://github.com/JulianAbeleda/gameterm/pull/851)
* Improved: [skip_close_confirmation_for_processes_named](config/lua/config/skip_close_confirmation_for_processes_named.md) now includes common windows shell processes `cmd.exe`, `pwsh.exe` and `powershell.exe`. [#843](https://github.com/JulianAbeleda/gameterm/issues/843)
* Fixed: don't keep the window alive after running `something & disown ; exit` [#839](https://github.com/JulianAbeleda/gameterm/issues/839)
...
* Changed: `COLORTERM=truecolor` is now set in the environment. [#875](https://github.com/JulianAbeleda/gameterm/issues/875)
* New: `gameterm cli spawn --new-window` flag for creating a new window via the CLI [#887](https://github.com/JulianAbeleda/gameterm/issues/887)
* Fixed: closing last pane in a tab via `CloseCurrentPane` could cause the window to close [#890](https://github.com/JulianAbeleda/gameterm/issues/890)
* Improved: `gameterm ls-fonts --list-system` shows all available fonts, `gameterm ls-fonts --text "hello"` explains which fonts are used for each glyph in the supplied text
* Fixed: mouse cursor is now Arrow rather than I-beam when the application in the terminal has enabled mouse reporting [#859](https://github.com/JulianAbeleda/gameterm/issues/859)
* Improved: DEC Special Graphics mode conformance and complete coverage of the graphics character set. Thanks to [Autumn Lamonte](https://gitlab.com/autumnmeowmeow)! [#891](https://github.com/JulianAbeleda/gameterm/pull/891)
* Fixed: click to focus now focuses the pane under the mouse cursor [#881](https://github.com/JulianAbeleda/gameterm/issues/881)
* Removed: `Parasio Dark` color scheme; it was a duplicate of the correctly named `Paraiso Dark` scheme. Thanks to [@adrian5](https://github.com/adrian5)! [#906](https://github.com/JulianAbeleda/gameterm/pull/906)
* Fixed: key repeat on Wayland now respects the system specified key repeat rate, and doesn't "stick". [#669](https://github.com/JulianAbeleda/gameterm/issues/669)
* Fixed: `force_reverse_video_cursor` wasn't correctly swapping the cursor colors in all cases. [#706](https://github.com/JulianAbeleda/gameterm/issues/706)
...
* Fixed: Mod3 is no longer treater as SUPER on X11 and Wayland [#933](https://github.com/JulianAbeleda/gameterm/issues/933)
* Fixed: paste now respects `scroll_to_bottom_on_input`. [#931](https://github.com/JulianAbeleda/gameterm/issues/931)
...
* Fixed: focus tracking events are now also generated when switching between panes [#941](https://github.com/JulianAbeleda/gameterm/issues/941)
* New: [window_frame](config/lua/config/window_frame.md) option to configure Wayland window decorations [#761](https://github.com/JulianAbeleda/gameterm/issues/761)
* New: [window:get_appearance()](config/lua/window/get_appearance.md) to determine if the window has a dark mode appearance, and adjust color scheme to match [#806](https://github.com/JulianAbeleda/gameterm/issues/806)
* Improved: [improve the new-tab button formatting](config/lua/config/tab_bar_style.md). Thanks to [@sdrik](https://github.com/sdrik)! [#950](https://github.com/JulianAbeleda/gameterm/pull/950)
* Fixed: if a line of text was exactly the width of the terminal it would get marked as wrappable even when followed by a newline, causing text to reflow incorrectly on resize. [#971](https://github.com/JulianAbeleda/gameterm/issues/971)
* Fixed: `gameterm ssh` could loop forever in the background if the connection drops and the window is closed. [#857](https://github.com/JulianAbeleda/gameterm/issues/857)
* Improved: VT102 conformance. Many thanks to [Autumn Lamonte](https://gitlab.com/autumnmeowmeow)! [#904](https://github.com/JulianAbeleda/gameterm/pull/904)
* New: [text_blink_rate](config/lua/config/text_blink_rate.md) and [text_blink_rate_rapid](config/lua/config/text_blink_rate_rapid.md) options to control blinking text. Thanks to [Autumn Lamonte](https://gitlab.com/autumnmeowmeow)! [#904](https://github.com/JulianAbeleda/gameterm/pull/904)
* New: Added support for [Synchronized Rendering](https://gist.github.com/christianparpart/d8a62cc1ab659194337d73e399004036) [#882](https://github.com/JulianAbeleda/gameterm/issues/882)
* New: gameterm now draws its own pixel-perfect versions of more block drawing glyphs.  See [custom_block_glyphs](config/lua/config/custom_block_glyphs.md) for more details. [#584](https://github.com/JulianAbeleda/gameterm/issues/584)
* Fixed: wayland: CursorNotFound error with the whiteglass theme. [#532](https://github.com/JulianAbeleda/gameterm/issues/532)
* Improved: Can now recover from exhausting available texture space by clearing the screen. [#879](https://github.com/JulianAbeleda/gameterm/issues/879)
* Updated bundled `Noto Color Emoji` font to version 2.028 featuring a [design update](https://blog.google/products/android/emoji-day-redesign-easier-sharing/). Thanks to [@4cm4k1](https://github.com/4cm4k1)! [#1003](https://github.com/JulianAbeleda/gameterm/pull/1003)
* Fixed: wayland: putting a window in the Sway scratchpad no longer blocks the gameterm process [#884](https://github.com/JulianAbeleda/gameterm/issues/884)
* Fixed: mouse reporting now correctly reports release events when multiple buttons are pressed and released at the same time. [#973](https://github.com/JulianAbeleda/gameterm/issues/973)
* Fixed: incorrect initial window/pty size when running with some tiling window managers. [#695](https://github.com/JulianAbeleda/gameterm/issues/695)
* New: CTRL-SHIFT-L shows the [debug overlay](config/lua/keyassignment/ShowDebugOverlay.md), which shows the error log and a lua repl. [#641](https://github.com/JulianAbeleda/gameterm/issues/641)
* Fixed: macOS: bright window padding on Intel-based macs [#653](https://github.com/JulianAbeleda/gameterm/issues/653), [#716](https://github.com/JulianAbeleda/gameterm/issues/716) and [#1000](https://github.com/JulianAbeleda/gameterm/issues/1000)
* Improved: gameterm now uses the Dual Source Blending feature of OpenGL to manage subpixel anti-aliasing alpha blending, resulting in improved appearance particularly when using a transparent window over the top of something with a light background. [#932](https://github.com/JulianAbeleda/gameterm/issues/932)
* Fixed: copying really long lines could falsely introduce line breaks on line wrap boundaries [#874](https://github.com/JulianAbeleda/gameterm/issues/874)
* New: [gameterm.add_to_config_reload_watch_list](config/lua/gameterm/add_to_config_reload_watch_list.md) function to aid with automatically reloading the config when you've split your config across multiple files. Thanks to [@AusCyberman](https://github.com/AusCyberman)! [#989](https://github.com/JulianAbeleda/gameterm/pull/989)
* Improved: gameterm now respects default emoji presentation and explicit emoji variation selectors (VS15 and VS16) so that glyphs that have both textual (usually monochrome, single cell width) and emoji (color, double width) presentations can be more faithfully rendered. [#997](https://github.com/JulianAbeleda/gameterm/issues/997)
...
* New: [gameterm.gradient_colors](config/lua/gameterm/gradient_colors.md) function to compute RGB values for gradients for use in your config.
...
* Fixed: red and blue subpixel channels were swapped, leading to excessively blurry text when using `freetype_load_flags="HorizontalLcd"`. [#639](https://github.com/JulianAbeleda/gameterm/issues/639)
* Fixed: the selection wouldn't always clear when the intersecting lines change [#644](https://github.com/JulianAbeleda/gameterm/issues/644)
* Fixed: vertical alignment issue with Iosevka on Windows [#661](https://github.com/JulianAbeleda/gameterm/issues/661)
* Fixed: support for "Variable" fonts such as Cascadia Code and Inconsolata on all platforms [#655](https://github.com/JulianAbeleda/gameterm/issues/655)
* New: [gameterm.font](config/lua/gameterm/font.md) and [gameterm.font_with_fallback](config/lua/gameterm/font_with_fallback.md) *attributes* parameter now allows matching more granular font weights and font stretch. e.g.: `gameterm.font('Iosevka Term', {stretch="Expanded", weight="Regular"})`, as fallback can specify weight/stretch/style for each individual font in the fallback.
...
* Fixed: `gameterm ssh HOST` no longer overrides the `User` config specified by `~/.ssh/config`
* Fixed: X11: detect when gnome DPI scaling changes [#667](https://github.com/JulianAbeleda/gameterm/issues/667)
* Fixed: potential panic when pasting large amounts of multi-byte text [#668](https://github.com/JulianAbeleda/gameterm/issues/668)
* Fixed: X11: non-ascii text could appear mangled in titlebars [#673](https://github.com/JulianAbeleda/gameterm/issues/673)
* Improved font IO performance and memory usage on all platforms
* New [window:toast_notification](config/lua/window/toast_notification.md) method for showing desktop notifications. [#619](https://github.com/JulianAbeleda/gameterm/issues/619)
* Fixed: half-pixel gaps in ligatured/double-wide glyphs depending on the font size [#614](https://github.com/JulianAbeleda/gameterm/issues/614)
* Fixed: Window could vanish if a tab closed while the rightmost tab was active(!) [#690](https://github.com/JulianAbeleda/gameterm/issues/690)
* Fixed: macOS: mouse cursor could get stuck in the hidden state. [#618](https://github.com/JulianAbeleda/gameterm/issues/618)
...
* Fixed: [Hide](config/lua/keyassignment/Hide.md) had no effect on macOS when the titlebar was disabled [#679](https://github.com/JulianAbeleda/gameterm/issues/679)
* Fixed: key and mouse assignments set via [window:set_config_overrides](config/lua/window/set_config_overrides.md) were not respected. [#656](https://github.com/JulianAbeleda/gameterm/issues/656)
* Fixed: potential panic when word selecting off top of viewport [#713](https://github.com/JulianAbeleda/gameterm/issues/713)
* Fixed: really long busy wait when displaying single logical json line of 1.5MB in length [#714](https://github.com/JulianAbeleda/gameterm/issues/714)
* New: [swallow_mouse_click_on_pane_focus](config/lua/config/swallow_mouse_click_on_pane_focus.md) option [#724](https://github.com/JulianAbeleda/gameterm/issues/724)
* New: [pane_focus_follows_mouse](config/lua/config/pane_focus_follows_mouse.md) option [#600](https://github.com/JulianAbeleda/gameterm/issues/600)
* Fixed: splitting a pane while a pane is in the zoomed state would swallow the new pane [#723](https://github.com/JulianAbeleda/gameterm/issues/723)
* Fixed: multi-cell glyphs weren't displayed in tab titles [#711](https://github.com/JulianAbeleda/gameterm/issues/711)
...
* New: [format-tab-title](config/lua/window-events/format-tab-title.md) hook for customizing the text in tab titles. [#647](https://github.com/JulianAbeleda/gameterm/issues/647)
...
* New: [force_reverse_video_cursor](config/lua/config/force_reverse_video_cursor.md) setting to override the cursor color scheme settings. [#706](https://github.com/JulianAbeleda/gameterm/issues/706)
* Fixed: ssh config parsing now expands `~` to your home directory for appropriate options; previously only `%d` and `${HOME}` were substituted. [#729](https://github.com/JulianAbeleda/gameterm/issues/729)
* New: [Quick Select Mode](quickselect.md) for a tmux-fingers/tmux-thumbs style mouse-less select and copy flow [#732](https://github.com/JulianAbeleda/gameterm/issues/732)
* Fixed: tabs would remain hovered after moving the mouse down into the main terminal area [#591](https://github.com/JulianAbeleda/gameterm/issues/591)
* New: [tab_bar_at_bottom](config/lua/config/tab_bar_at_bottom.md) setting to put the tab bar at the bottom of the window [#278](https://github.com/JulianAbeleda/gameterm/issues/278)
* New: [gameterm.column_width](config/lua/gameterm/column_width.md) function for measuring the displayed width of a string
* New: [gameterm.pad_left](config/lua/gameterm/pad_left.md), [wgameterm.pad_right](config/lua/gameterm/pad_right.md), [gameterm.truncate_left](config/lua/gameterm/truncate_left.md) and [gameterm.truncate_right](config/lua/gameterm/truncate_right.md) function for truncating/padding a string based on its displayed width
* Updated bundled `Noto Color Emoji` font to version 2.020 with unicode 13.1 support. Thanks to [@4cm4k1](https://github.com/4cm4k1)! [#742](https://github.com/JulianAbeleda/gameterm/pull/742)
* Fixed: Numpad Enter reported as CTRL-C on macOS [#739](https://github.com/JulianAbeleda/gameterm/issues/739)
* Fixed: mouse reporting button state not cleared when focus is lost, eg: from clicking a link [#744](https://github.com/JulianAbeleda/gameterm/issues/744)
* Improved: better looking curly underline. Thanks to [@ModProg](https://github.com/ModProg)! [#733](https://github.com/JulianAbeleda/gameterm/pull/733)
* Fixed: gameterm now sets argv0 to `-$SHELL` to invoke a login shell, rather than running `$SHELL -l`. [#753](https://github.com/JulianAbeleda/gameterm/issues/753)
* Improved: `ssh_config` parsing now supports `Match` for `Host`, `LocalUser`.
* Improved render performance for wide windows [#740](https://github.com/JulianAbeleda/gameterm/issues/740)
...
* Fixed: bold text got broken as part of fixing #617 :-( [#648](https://github.com/JulianAbeleda/gameterm/issues/648)

### 20210404-112810-b63a949d

* Fixed: 100% CPU due to spurious resize events generated by herbstluftwm. [#557](https://github.com/JulianAbeleda/gameterm/issues/557)
* Fixed: improved conformance with xterm for keys like CTRL-6 and CTRL-/. [#556](https://github.com/JulianAbeleda/gameterm/discussions/556)
* Fixed: detection and handling of fonts such as terminus-bold.otb that contain only bitmap strikes. [#560](https://github.com/JulianAbeleda/gameterm/issues/560)
* Fixed: the pixel size reported by the pty to the kernel wasn't adjusted for font metrics/dpi until the config was reloaded or window resized. [#563](https://github.com/JulianAbeleda/gameterm/issues/563)
* Fixed: greatly reduce memory consumption when system fallback fonts are loaded [#559](https://github.com/JulianAbeleda/gameterm/issues/559)
* Fixed: Windows: `window_background_opacity` was only taking effect when `window_decorations="NONE"` [#553](https://github.com/JulianAbeleda/gameterm/issues/553)
* Fixed: an issue where gameterm could hang if the process spawned by a pane doesn't quit when asked [#558](https://github.com/JulianAbeleda/gameterm/issues/558)
* Fixed: panic when dismissing the tab navigator [#542](https://github.com/JulianAbeleda/gameterm/issues/542)
* Fixed: font fallback on macOS returns unresolvable `.AppleSymbolsFB` rather than `Apple Symbols`, leading to slowdowns when rendering symbols [#506](https://github.com/JulianAbeleda/gameterm/issues/506)
* Fixed: laggy repaints for large windows particularly on Windows, but applicable to all systems.  Tuned and triple-buffered vertex buffer updates. [#546](https://github.com/JulianAbeleda/gameterm/issues/546)
* Changed: [allow_square_glyphs_to_overflow_width](config/lua/config/allow_square_glyphs_to_overflow_width.md) now defaults to `WhenFollowedBySpace` and applies to more symbol glyphs. [#565](https://github.com/JulianAbeleda/gameterm/issues/565)
* Changed: macOS: `CMD-Q` is now bound by default to [QuitApplication](config/lua/keyassignment/QuitApplication.md)
* New: added [skip_close_confirmation_for_processes_named](config/lua/config/skip_close_confirmation_for_processes_named.md) option which specifies a list of processes for which it is considered safe to allow closing a pane/tab/window without a prompt. [#562](https://github.com/JulianAbeleda/gameterm/issues/562)
* Fixed: triggering the search overlay again while the search overlay is active no longer closes the underlying pane [#572](https://github.com/JulianAbeleda/gameterm/issues/572)
...
* Fixed: font fallback now happens asynchronously from painting [#508](https://github.com/JulianAbeleda/gameterm/issues/508)
* New: added [window:get_selection_text_for_pane](config/lua/window/get_selection_text_for_pane.md) method [#575](https://github.com/JulianAbeleda/gameterm/issues/575)
* Fixed: implicit hyperlink rules, word and line selection now operate on logical lines which means that they deal with wrapped lines outside of the viewport. [#408](https://github.com/JulianAbeleda/gameterm/issues/408)
* New: `gameterm ssh` now supports reading `~/.ssh/config` and overriding options via the command line.  `IdentityFile` and `ProxyCommand` are the two main new supported options.  Read more about it in [ssh](ssh.md).
* Fixed: ssh support will now try all available identities from the SSH agent rather than just the first.
* New: splitting panes in `gameterm ssh` now works like spawning new tabs: the new program is started on the remote host with no additional authentication required.
* Fixed: Multiplexer sessions would fail to bootstrap via ssh because the bootstrap process exited too soon. [#507](https://github.com/JulianAbeleda/gameterm/issues/507)
...
* Fixed: macOS: middle mouse button wasn't recognized. Thanks to [@guswynn](https://github.com/guswynn)! [#599](https://github.com/JulianAbeleda/gameterm/pull/599)
* New: added [ActivateLastTab](config/lua/keyassignment/ActivateLastTab.md) key assignment for jumping back to a previously active tab. Thanks to [@alexgartrell](https://github.com/alexgartrell) [#610](https://github.com/JulianAbeleda/gameterm/pull/610)
* Fixed: added missing XTSMGRAPHICS query/response for sixel support [#609](https://github.com/JulianAbeleda/gameterm/issues/609)
* Fixed: avoid showing an error dialog for synthesized `font_rules` when the configuration specifies a font that doesn't have bold/italic variants. [#617](https://github.com/JulianAbeleda/gameterm/issues/617)
* New: mouse cursor hides when keyboard input is sent to a pane, and shows again when the mouse is moved. [#618](https://github.com/JulianAbeleda/gameterm/issues/618)
* Fixed: macOS: CTRL-Tab key combination was not recognized. [#630](https://github.com/JulianAbeleda/gameterm/issues/630)
* Fixed: gameterm-mux-server will now continue running even after all tabs/panes have been closed. [#631](https://github.com/JulianAbeleda/gameterm/issues/631)
* Fixed: macOS: gameterm-gui could linger in the background until the mouse moves after all tabs/panes have closed
* Fixed: when using [line_height](config/lua/config/line_height.md), gameterm now vertically centers the cell rather than padding only the top [#582](https://github.com/JulianAbeleda/gameterm/issues/582)
* Fixed: macOS: in US layouts, `SUPER+SHIFT+[` was incorrectly recognized as `SUPER+SHIFT+{` instead of `SUPER+{` [#601](https://github.com/JulianAbeleda/gameterm/issues/601)
* Fixed: [gameterm.config_dir](config/lua/gameterm/config_dir.md) was returning the config file path instead of the directory!
* New: [gameterm.config_file](config/lua/gameterm/config_file.md) which returns the config file path
...
* New: animated gif and png images displayed via `gameterm imgcat` (the iTerm2 image protocol), or attached to the window background via [window_background_image](config/appearance.md#window-background-image) will now animate while the window has focus.
...
* New: OSC 777 and OSC 9 escapes now generate Toast Notifications. `printf "\e]777;notify;%s;%s\e\\" "title" "body"` and `printf "\e]9;%s\e\\" "hello there"`.  These don't currently pass through multiplexer connections. [#489](https://github.com/JulianAbeleda/gameterm/issues/489).
* New: [exit_behavior](config/lua/config/exit_behavior.md) config option to keep panes open after the program has completed. [#499](https://github.com/JulianAbeleda/gameterm/issues/499)
* New: added `--config name=value` options to `gameterm`, `gameterm-gui` and `gameterm-mux-server`.  The `--front-end`, `--font-locator`, `--font-rasterizer` and `--font-shaper` CLI options have been removed in favor of this new mechanism.
* New: [window:set_config_overrides](config/lua/window/set_config_overrides.md) method that can be used to override GUI related configuration options on a per-window basis. Click through to see examples of dynamically toggling ligatures and window opacity. [#469](https://github.com/JulianAbeleda/gameterm/issues/469) [#329](https://github.com/JulianAbeleda/gameterm/issues/329)
* New: introduced [custom_block_glyphs](config/lua/config/custom_block_glyphs.md) option to ensure that block glyphs don't have gaps. [#433](https://github.com/JulianAbeleda/gameterm/issues/433)
* New: you can now drag the gameterm window via the tab bar
* New: holding SUPER+Drag (or CTRL+SHIFT+Drag) will drag the gameterm window.  Use [StartWindowDrag](config/lua/keyassignment/StartWindowDrag.md) to configure your own binding.
...
* New: [window:set_right_status](config/lua/window/set_right_status.md) allows setting additional status information in the tab bar. [#500](https://github.com/JulianAbeleda/gameterm/issues/500)
* New: Search Mode: Added `CTRL-u` key assignment to clear the current search pattern. Thanks to [@bew](https://github.com/bew)! [#465](https://github.com/JulianAbeleda/gameterm/pull/465)
...
* Fonts: when computing default `font_rules` for bold and italic fonts, strip italic and bold components from the family name. eg: if you set `font = gameterm.font("Source Code Pro Medium")` then the `Medium` text will be stripped from the font name used to locate bold and italic variants so that we don't report an error loading a non-sensical `Source Code Pro Medium Bold`. [#456](https://github.com/JulianAbeleda/gameterm/issues/456)
* Fonts: fix a regression where bright windows behind gameterm could "shine through" on the alpha channel, and adjust the tinting operation to avoid anti-aliased dark fringes [#470](https://github.com/JulianAbeleda/gameterm/issues/470) [#491](https://github.com/JulianAbeleda/gameterm/issues/491)
* Fonts: macOS: fix an issue where gameterm could hang when loading a font located via Core Text [#475](https://github.com/JulianAbeleda/gameterm/issues/475)
* Fonts: Changed the default [font_size](config/lua/config/font_size.md) to 12 points. [#517](https://github.com/JulianAbeleda/gameterm/discussions/517)
* Fonts: Updated bundled JetBrainsMono font to version 2.225
* Added `--config-file` CLI option to specify an alternate config file location. [Read more about config file resolution](config/files.md). Thanks to [@bew](https://github.com/bew)! [#459](https://github.com/JulianAbeleda/gameterm/pull/459)
...
* Fixed: line-based mouse selection (default: triple click) now extends forwards to include wrapped lines. [#466](https://github.com/JulianAbeleda/gameterm/issues/466)
* Fixed: the [RIS](https://vt100.net/docs/vt510-rm/RIS) escape wasn't clearing the scrollback. [#511](https://github.com/JulianAbeleda/gameterm/issues/511)
* Wayland: fixed opengl context creation issues.  Thanks to [@valpackett](https://github.com/valpackett)! [#481](https://github.com/JulianAbeleda/gameterm/pull/481)
...
* Wayland: fixed window decoration and full screen handling [#224](https://github.com/JulianAbeleda/gameterm/issues/224)
...
* Windows: Fixed [ToggleFullScreen](config/lua/keyassignment/ToggleFullScreen.md) so that it once again toggles between full screen and normal placement. [#177](https://github.com/JulianAbeleda/gameterm/issues/177)
* Windows: fix the unexpected default behavior of Ctrl-Alt being converted to AltGr for layouts supporting this key, the previous behavior is still possible by enabling the option [`treat_left_ctrlalt_as_altgr`](config/lua/config/treat_left_ctrlalt_as_altgr.md) (to solve [#392](https://github.com/JulianAbeleda/gameterm/issues/392)). Thanks to [@bew](https://github.com/bew)! [#512](https://github.com/JulianAbeleda/gameterm/pull/512)
* Windows: fixed "Open GameTerm Here" context menu in explorer when used on the root of a drive (eg: `C:\`).  Thanks to [@flyxyz123](https://github.com/flyxyz123)! [#526](https://github.com/JulianAbeleda/gameterm/issues/526) [#451](https://github.com/JulianAbeleda/gameterm/issues/451)
* X11: fix an issue where SHIFT-Enter was not recognized [#516](https://github.com/JulianAbeleda/gameterm/issues/516)
* X11: improved DPI detection for high-DPI displays. [#515](https://github.com/JulianAbeleda/gameterm/issues/515)
* X11: we now load the XCursor themes when possible, which means that the mouse cursor is now generally a bit larger and clearer as well as conforming more with the prevailing style of the desktop environment. [#524](https://github.com/JulianAbeleda/gameterm/issues/524)
* Improved and optimized image processing so that watching videos via [timg - Terminal Image and Video Viewer](http://timg.sh) works better [#537](https://github.com/JulianAbeleda/gameterm/issues/537) [#535](https://github.com/JulianAbeleda/gameterm/issues/535) [#534](https://github.com/JulianAbeleda/gameterm/issues/534)

### 20210203-095643-70a364eb

* Fix cursor position after using iTerm2 image protocol [#317](https://github.com/JulianAbeleda/gameterm/issues/317)
* Fix pixel dimensions after changing the pane size; this was mostly invisible but impacted image scaling when using sixel or iTerm2 image protocols. [#312](https://github.com/JulianAbeleda/gameterm/issues/312)
...
* Added `gameterm start --class CLASSNAME` option to specify the window class name under X11 and Windows, or the `app_id` under Wayland.  See `gameterm start --help` for more information.
...
* Improved font rendering [#320](https://github.com/JulianAbeleda/gameterm/issues/320) [#331](https://github.com/JulianAbeleda/gameterm/issues/331) [#413](https://github.com/JulianAbeleda/gameterm/issues/413) and changed `font_antialias = "Greyscale"` by default.
* Updated internal harfbuzz shaper to 2.7.2
* Fixed ALT-Escape not sending ESC-ESC [#338](https://github.com/JulianAbeleda/gameterm/issues/338)
* Added `allow_square_glyphs_to_overflow_width = "WhenFollowedBySpace"` option to allow square symbol glyphs to deliberately overflow their specified cell width when the next cell is a space.  Can be set to `Always` to allow overflowing regardless of the next cell being a space, or `Never` to strictly respect the cell width.  The default is `Never`. [#342](https://github.com/JulianAbeleda/gameterm/issues/342)
* macOS: Improved key input when Option is pressed.  Fixed dead key processing when `use_ime=true`. [#357](https://github.com/JulianAbeleda/gameterm/issues/357)
* macOS: Adjusted default dpi to 72 to bring point sizes into alignment with other macOS apps. [#332](https://github.com/JulianAbeleda/gameterm/issues/332)
...
* The 256 color cube now uses slightly brighter colors [#348](https://github.com/JulianAbeleda/gameterm/issues/348)
...
* macOS: Fixed an issue where hovering over the split between panes could result in gameterm becoming unresponsive [#391](https://github.com/JulianAbeleda/gameterm/issues/391)
* Closing windows and `QuitApplication` will now prompt for confirmation before proceeding with the close/quit.  Added `window_close_confirmation` to control this; valid values are `AlwaysPrompt` and `NeverPrompt`. [#280](https://github.com/JulianAbeleda/gameterm/issues/280)
* Tidied up logging. Previously ERROR level logging was used to make sure that informational things showed up in the stderr stream. Now we use INFO level logging for this to avoid alarming the user.  You can set `GAMETERM_LOG=trace` in the environment to get more verbose logging for troubleshooting purposes.
* Windows: fix an issue where VNC-server-emulated AltGr was not treated as AltGr [#392](https://github.com/JulianAbeleda/gameterm/issues/392)
* X11: fix an issue where keys that produce unicode characters retained SHIFT as a modifier instead of normalizing it away. [#394](https://github.com/JulianAbeleda/gameterm/issues/394)
* Fixed an issue where a symbol-only font would be seen as 0-width and panic gameterm [#404](https://github.com/JulianAbeleda/gameterm/issues/404)
* Tweaked mouse selection: we now round the x-coordinate to the nearest cell which makes it a bit more forgiving if the mouse cursor is slightly to the left of the intended cell start. [#350](https://github.com/JulianAbeleda/gameterm/issues/350)
* Added `selection_word_boundary` option to control double-click word selection boundaries. The default is <tt> \t\n{}\[\]()\"'\`</tt>. [#405](https://github.com/JulianAbeleda/gameterm/issues/405)
* Added support for Curly, Dotted and Dashed underlines.  See [this documentation](faq.md#how-do-i-enable-undercurl-curly-underlines) on the escape sequences how enable undercurl support in vim and nvim. [#415](https://github.com/JulianAbeleda/gameterm/issues/415)
* Fixed an issue where gameterm would spawn processes with `umask 077` on unix systems, rather than the more commonly expected `umask 022`. [#416](https://github.com/JulianAbeleda/gameterm/issues/416)
* macOS: We now ship a Universal binary containing both Intel and "Apple Silicon" architectures
* Setting a really large or really small font scale (using CTRL +/-) no longer causes a panic [#428](https://github.com/JulianAbeleda/gameterm/issues/428)
* Fixed an issue where the mouse wheel wasn't mapped to cursor up/down when the alternate screen was active [#429](https://github.com/JulianAbeleda/gameterm/issues/429)
* Fixed `ToggleFullScreen` not working on macOS and X11.  It still doesn't function on Windows.  `native_macos_fullscreen_mode = false` uses a fast full-screen window on macOS. Set it to `true` to use the slower macOS native "Spaces" style fullscreen mode. [#177](https://github.com/JulianAbeleda/gameterm/issues/177)
* Windows: fix an issue where the initial window size didn't factor the correct DPI when the system-wide display scaling was not 100%. [#427](https://github.com/JulianAbeleda/gameterm/issues/427)
* New: `adjust_window_size_when_changing_font_size` option to control whether changing the font size adjusts the dimensions of the window (true) or adjusts the number of terminal rows/columns (false).  The default is `true`. [#431](https://github.com/JulianAbeleda/gameterm/issues/431)
...
* macOS: Windows now have drop-shadows when they are opaque. These were disabled due transparency support was added. Thanks to [Rice](https://github.com/fanzeyi)! [#445](https://github.com/JulianAbeleda/gameterm/pull/445)
* Unix: adjust font-config patterns to also match "dual spacing" fonts such as [Iosevka Term](https://typeof.net/Iosevka/). Thanks to [Leiser](https://github.com/leiserfg)! [#446](https://github.com/JulianAbeleda/gameterm/pull/446)
* New: Added [alternate_buffer_wheel_scroll_speed](config/lua/config/alternate_buffer_wheel_scroll_speed.md) option to control how many cursor key presses are generated by the mouse wheel when the alternate screen is active. The new default for this is a faster-than-previous-releases 3 lines per wheel tick. [#432](https://github.com/JulianAbeleda/gameterm/issues/432)
* macOS: Dead Keys are now processed even when `use_ime=false`.  [More details in the docs](config/keyboard-concepts.md#macos-left-and-right-option-key). [#410](https://github.com/JulianAbeleda/gameterm/issues/410).
* X11: attempt to load cursors from the XCursor.theme resource specified on the root window [#524](https://github.com/JulianAbeleda/gameterm/issues/524)
* Added `file://` URL matching to the default list of implicit hyperlink rules [#525](https://github.com/JulianAbeleda/gameterm/issues/525)

### 20201101-103216-403d002d

* Whoops! fixed a crash on macOS when using multiple windows in the new Metal renderer [#316](https://github.com/JulianAbeleda/gameterm/issues/316)
...
* New: expanded lua API allows handling URI clicks and keyboard events
  with lua callbacks.  See [gameterm.on](config/lua/gameterm/on.md) docs.
...
* On Linux and macOS systems, gameterm can now attempt to guess the current
...
* macOS: Fix issue where new windows would open as Cocoa tabs
  when gameterm was maximized.
* macOS: Fix issue where gameterm wouldn't adjust to DPI changes
...
* If EGL/WGL/OpenGL fail to initialize, we now try to fallback
  ...
  has improved visuals compared to gameterm's own basic CPU
...
* The multiplexer server has been moved into its own
  `gameterm-mux-server` executable.  You will need to revise
...
* Windows: we now prefer to use Direct3D11 to render the
  ...
  graphics drivers while you have a stateful gameterm session
  will no longer terminate the gameterm process. Resize
...
* Fixed a crash on Linux/X11 when using `gameterm connect HOST`
...
* Windows: fixed buffering/timing issue where a response to a color query in
  ...
  letter `g`.
* X11: Improved support for non-24bpp display depths.  GameTerm now tries
...
* macos: we now attempt to set a reasonable default LANG environment based
  on the locale settings at the time that gameterm is launched.
...
* Fonts: fixed an issue where specifying italic or bold in the second parameter
  of `gameterm.font` didn't work as intended or documented
...
* Removed old `arg="something"` key binding parameter.  This was a remnant from
  ...
  `action=gameterm.action{ActivateTab=i-1}` to pass the integer argument.
* Windows: now also available with a setup.exe installer.  The installer
  enables "Open GameTerm Here" in the explorer.exe context menu.
...
* AppImage: Support looking for configuration in `GameTerm.AppImage.config` and
  `GameTerm.AppImage.home` to support portable thumbdrive use of gameterm on
  linux systems
* We now check the github releases section for updated stable releases and show
  ...
  call to github.  There is no data collection performed by the gameterm project
...
* GameTerm is now also available to Windows users via [Scoop](https://scoop.sh/)
...
* Windows: we now support mouse reporting on Windows native ptys.  For this to
  work, `conpty.dll` and `OpenConsole.exe` must be present alongside `gameterm.exe`
  when starting gameterm.
...
* Added `HideApplication` key action (defaults to `CMD-H` on macOS only) which
  hides the gameterm application.  This is macOS specific.
...
* Nightly builds are now available for Linux in [AppImage](https://github.com/JulianAbeleda/gameterm/releases/download/nightly/GameTerm-nightly.AppImage) format.
...
* Improved resize behavior; lines that were split due to
  ...
  [Issue 14](https://github.com/JulianAbeleda/gameterm/issues/14)
* Double-click and triple-click and hold followed by a drag now extends
  the selection by word and line respectively.
* The OSC 7 (CurrentWorkingDirectory) escape sequence is now supported; gameterm records the cwd in a tab and that will be used to set the working directory when spawning new tabs in the same domain.  You will need to configure your shell to emit OSC 7 when appropriate.
* [Changed Backspace/Delete handling](https://github.com/JulianAbeleda/gameterm/commit/f0e94084d1df36009b879b06e9cfd2be946168e8)
...
* [The multiplexer protocol is undergoing major changes](https://github.com/JulianAbeleda/gameterm/issues/106).
  The multiplexer will now raise an error if the client and server are incompatible.
* Fixed an issue where gameterm would linger for a few seconds after the last tab was closed
* Fixed an issue where gameterm wouldn't repaint the screen after a tab was closed
...
* Added `$GAMETERM_CONFIG_FILE` to the start of the config file search path
...
* We now respect [DECSCUSR and DECTCEM](https://github.com/JulianAbeleda/gameterm/issues/7) escape sequence to select between hidden, block, underline and bar cursor types, as well as blinking cursors.  New configuration options have been added to control the appearance and blink rate.
...
* New `font_dirs` configuration option to specify a set of dirs to search for fonts. Useful for self-contained gameterm deployments.
...
* Disable Nagle's algorithm for `gameterm ssh` sessions
...
* `gameterm ssh user@host` for ad-hoc SSH sessions. You may also define SSH multiplexer sessions.
* `gameterm serial /dev/ttyUSB0` to connect to your Arduino
* `gameterm imgcat /some/image.png` to display images inline in the terminal using the iTerm2 image protocol
