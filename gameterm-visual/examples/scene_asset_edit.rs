...
Options:
  ...
  --session PATH             Edit session JSON for session-run.
...
fn run_session(args: CliArgs) -> Result<(), String> {
    let session = required_path(args.session.clone(), "--session")?;
...
fn parse_args() -> Result<CliArgs, String> {
    ...
    while let Some(arg) = args.next() {
        ...
        match arg_text.as_ref() {
            ...
            "--session" => parsed.session = Some(next_path(&mut args, "--session")?),
