...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    gameterm_mod.set("nerdfonts", NerdFonts {})?;
    gameterm_mod.set("format", lua.create_function(format)?)?;
    gameterm_mod.set(
        "column_width",
    ...
    gameterm_mod.set("permute_any_mods", lua.create_function(permute_any_mods)?)?;
    gameterm_mod.set(
...
pub fn format_as_escapes(items: Vec<FormatItem>) -> anyhow::Result<String> {
    ...
    let mut renderer = new_gameterm_terminfo_renderer();
...
fn permute_mods<'lua>(
    ...
    use gameterm_input_types::Modifiers;
    ...
    static ref CAPS: Capabilities = {
        ...
        Capabilities::new_with_hints(
            ProbeHints::new_from_env()
                ...
                .colorterm(None)
                .colorterm_bce(None)
                .term_program(Some("GameTerm".into()))
                .term_program_version(Some(config::gameterm_version().into())),
        )
        .expect("cannot fail to make internal Capabilities")
    };
}

pub fn new_gameterm_terminfo_renderer() -> TerminfoRenderer {
