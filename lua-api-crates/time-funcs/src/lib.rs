...
/// by the user via `gameterm.time.call_after`.
...
const SCHEDULED_EVENTS: &str = "gameterm-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    gameterm_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    gameterm_mod.set("strftime", lua.create_function(strftime)?)?;
    gameterm_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}
