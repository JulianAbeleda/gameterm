...
use gameterm_dynamic::{FromDynamic, Value as DynValue};
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    gameterm_mod.set("json_parse", lua.create_function(json_decode)?)?;
    gameterm_mod.set("json_encode", lua.create_function(json_encode)?)?;
...
fn lua_value_to_json_value(value: LuaValue, visited: &mut HashSet<usize>) -> mlua::Result<JValue> {
    ...
    Ok(match value {
        ...
        LuaValue::UserData(ud) => match ud.get_metatable() {
            Ok(mt) => {
                if let Ok(to_dynamic) = mt.get::<mlua::Function>("__gameterm_to_dynamic") {
                    match to_dynamic.call(LuaValue::UserData(ud.clone())) {
                        ...
                        Err(err) => {
                            return Err(mlua::Error::FromLuaConversionError {
                                from: "userdata",
                                to: "gameterm_dynamic::Value",
                                message: Some(format!(
                                    "error calling __gameterm_to_dynamic: {err:#}"
                                )),
                            })
                        }
                    }
                    ...
                    return Err(mlua::Error::FromLuaConversionError {
                        from: "userdata",
                        to: "gameterm_dynamic::Value",
                        message: Some(format!("no __gameterm_to_dynamic metadata")),
                    });
                }
            }
            Err(err) => {
