...
use std::path::Path;
use std::sync::Mutex;
use gameterm_dynamic::{
    FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value as DynValue,
};

pub use mlua;

static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "gameterm-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let sub = gameterm_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            gameterm_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module gameterm.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    gameterm_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// gameterm specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `gameterm` module is registered so that the script can
/// `require "gameterm"` and call into functions provided by
/// gameterm.  The gameterm module contains:
/// * `executable_dir` - the directory containing the gameterm
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the gameterm
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized gameterm).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running gameterm instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `gameterm` module in the script
        let gameterm_mod = get_or_create_module(&lua, "gameterm")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".gameterm"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                gameterm_mod
                    ...
                    .context("set gameterm.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("gameterm_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.gameterm.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        gameterm_mod
            ...
            .context("set gameterm.config_file")?;
        gameterm_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set gameterm.config_dir")?;

        lua.set_named_registry_value("gameterm-watch-paths", Vec::<String>::new())?;
        gameterm_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        gameterm_mod.set("target_triple", crate::gameterm_target_triple())?;
        gameterm_mod.set("version", crate::gameterm_version())?;
        gameterm_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        gameterm_mod.set(
            "running_under_wsl",
        ...
        gameterm_mod.set("font", lua.create_function(font)?)?;
        gameterm_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        gameterm_mod.set("hostname", lua.create_function(hostname)?)?;
        gameterm_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        gameterm_mod.set(
            "has_action",
        ...
        gameterm_mod.set("action_callback", lua.create_function(action_callback)?)?;
        gameterm_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        gameterm_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        gameterm_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        gameterm_mod.set("on", lua.create_function(register_event)?)?;
        gameterm_mod.set("emit", lua.create_async_function(emit_event)?)?;
        gameterm_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        gameterm_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        gameterm_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since gameterm was started
...
/// <https://github.com/JulianAbeleda/gameterm/issues/1736>
/// <https://github.com/JulianAbeleda/gameterm/issues/1786>
...
///
/// `gameterm.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `gameterm.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `gameterm.font` documentation.
...
/// This implements `gameterm.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// gameterm.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// gameterm.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("gameterm-event-{}", name);
...
const IS_EVENT: &str = "gameterm-is-event-emission";
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value("gameterm-watch-paths")?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value("gameterm-watch-paths", watch_paths)?;
    Ok(())
}
...
assert(gameterm.emit('foo', 2) == false)

gameterm.on('bar', function (n, str)
...
assert(gameterm.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
