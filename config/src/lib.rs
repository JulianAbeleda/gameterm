...
use gameterm_dynamic::{FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value};
use gameterm_term::UnicodeVersion;
...
fn xdg_config_home() -> PathBuf {
    match std::env::var_os("XDG_CONFIG_HOME").map(|s| PathBuf::from(s).join("gameterm")) {
        Some(p) => p,
        None => HOME_DIR.join(".config").join("gameterm"),
    }
}
...
fn config_dirs() -> Vec<PathBuf> {
    ...
    if let Some(d) = std::env::var_os("XDG_CONFIG_DIRS") {
        dirs.extend(std::env::split_paths(&d).map(|s| PathBuf::from(s).join("gameterm")));
    }
...
pub fn overridden_config(overrides: &gameterm_dynamic::Value) -> Result<ConfigHandle, Error> {
...
impl ConfigInner {
    ...
    fn accumulate_watch_paths(lua: &Lua, watch_paths: &mut Vec<PathBuf>) {
        if let Ok(mlua::Value::Table(tbl)) = lua.named_registry_value("gameterm-watch-paths") {
    ...
    fn reload(&mut self) {
        ...
        if let Some(path) = file_name {
            ...
            if let Some(parent) = path.parent() {
                ...
                // home dir changes!
                // <https://github.com/JulianAbeleda/gameterm/issues/1895>
                if parent != &*HOME_DIR {
    ...
    fn overridden(&mut self, overrides: &gameterm_dynamic::Value) -> Result<ConfigHandle, Error> {
...
impl Configuration {
    ...
    fn overridden(&self, overrides: &gameterm_dynamic::Value) -> Result<ConfigHandle, Error> {
        let mut inner = self.inner.lock().unwrap();
