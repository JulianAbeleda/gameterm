...
use gameterm_mux_server_impl::update_mux_domains_for_server;
...
pub fn spawn_listener() -> anyhow::Result<()> {
    ...
    for unix_dom in &config.unix_domains {
        std::env::set_var("GAMETERM_UNIX_SOCKET", unix_dom.socket_path());
        let mut listener = gameterm_mux_server_impl::local::LocalListener::with_domain(unix_dom)?;
        thread::spawn(move || {
            listener.run();
        });
    }
