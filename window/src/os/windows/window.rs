...
use std::rc::Rc;
use std::sync::Mutex;
use gameterm_color_types::LinearRgba;
use gameterm_font::FontConfiguration;
use gameterm_input_types::KeyboardLedStatus;
...
impl Window {
    fn create_window(
        ...
        let class = WNDCLASSW {
            ...
            cbWndExtra: 0,
            hInstance: h_inst,
            // FIXME: this resource is specific to the gameterm build and this should
...
unsafe fn update_title_font(hwnd: HWND) {
    ...
    if let Some(lf) = get_title_log_font(hwnd, hdc) {
        *font = gameterm_font::locator::gdi::parse_log_font(&lf, hdc).ok();
    }
                    ...
                    .resolve_dead_key(leader, (modifiers, vk))
                {
                    ...
                    ResolvedDeadKey::InvalidCombination(c) => {
                            ...
                            inner.keyboard_info.is_dead_key_leader(modifiers, vk)
                        {
                            ...
                            // it done.
                            // <https://github.com/JulianAbeleda/gameterm/issues/1729>
                            inner.events.dispatch(WindowEvent::KeyEvent(key.clone()));
                            return Some(0);
                        }
