...
use gameterm_input_types::{is_ascii_control, IntegratedTitleButtonStyle, KeyboardLedStatus};
...
impl WindowInner {
    ...
    fn update_window_shadow(&mut self) {
        ...
        unsafe {
            ...
            // https://github.com/JulianAbeleda/gameterm/issues/310.
            // But allow overriding the shadows independent of opacity as well:
            // <https://github.com/JulianAbeleda/gameterm/issues/2669>
            let shadow = if self
                .config
                .window_decorations
...
const VIEW_CLS_NAME: &str = "GameTermWindowView";
const WINDOW_CLS_NAME: &str = "GameTermWindow";
...
impl WindowView {
    ...
    extern "C" fn gameterm_perform_key_assignment(
        this: &mut Object,
        _sel: Sel,
        menu_item: *mut Object,
    ) {
        let menu_item = MenuItem::with_menu_item(menu_item);
        // Safe because gametermPerformKeyAssignment: is only used with KeyAssignment
        let action = menu_item.get_represented_item();
        log::debug!("gameterm_perform_key_assignment {action:?}",);
        match action {
    ...
    fn key_common(this: &mut Object, nsevent: id, key_is_down: bool) {
        ...
        let (prefer_vkey, unmod) =
            if virtual_key == kVK_Delete && modifiers.contains(Modifiers::ALT) {
                ...
                // https://github.com/JulianAbeleda/gameterm/issues/739
        ...
        // <https://github.com/JulianAbeleda/gameterm/issues/1902>
        ...
        if let Some(key) = key_string_to_key_code(chars).or_else(|| key_string_to_key_code(unmod)) {
                ...
                || (only_right_alt && !send_composed_key_when_right_alt_is_pressed)
            {
                ...
                match (&key, &raw) {
                    ...
                    // <https://github.com/JulianAbeleda/gameterm/issues/1891>
    ...
    extern "C" fn perform_key_equivalent(this: &mut Object, _sel: Sel, nsevent: id) -> BOOL {
            ...
            || (chars == "\x19"/* Shift-Tab: See issue #1902 */)
        {
            ...
            // <https://github.com/JulianAbeleda/gameterm/issues/1867>
    ...
    extern "C" fn did_resize(this: &mut Object, _sel: Sel, _notification: id) {
        ...
        if let Some(this) = Self::get_this(this) {
            ...
            // gameterm-gui/src/termwindow/resize.rs.
            // <https://github.com/JulianAbeleda/gameterm/issues/3503>
    ...
    fn define_class() -> &'static Class {
        ...
        unsafe {
            ...
            cls.add_method(
                sel!(gametermPerformKeyAssignment:),
                Self::gameterm_perform_key_assignment
