...
for shell in bash zsh fish ; do
  target/debug/gameterm shell-completion --shell $shell > assets/shell-completion/$shell
done
...
for mode in copy_mode search_mode ; do
  ...
  echo "\`\`\`lua" > $fname
  target/debug/gameterm -n show-keys --lua --key-table $mode >> $fname
  echo "\`\`\`" >> $fname
done
...
cargo run --example narrow $PWD/target/debug/gameterm --help | ./target/debug/strip-ansi-escapes | trim_file > docs/examples/cmd-synopsis-gameterm--help.txt

for cmd in start ssh serial connect ls-fonts show-keys imgcat set-working-directory record replay  ; do
  fname="docs/examples/cmd-synopsis-gameterm-${cmd}--help.txt"
  cargo run --example narrow $PWD/target/debug/gameterm $cmd --help | ./target/debug/strip-ansi-escapes | trim_file > $fname
done
...
for cmd in \
    ...
    zoom-pane \
    ; do
  fname="docs/examples/cmd-synopsis-gameterm-cli-${cmd}--help.txt"
  cargo run --example narrow $PWD/target/debug/gameterm cli $cmd --help | ./target/debug/strip-ansi-escapes | trim_file > $fname
done
