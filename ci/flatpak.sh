...
#flatpak run --env=G_DEBUG=fatal-criticals org.freedesktop.appstream-glib validate assets/gameterm.appdata.xml
...
if [ "${CI}" != "yes" ] ; then
  flatpak-builder \
    --state-dir /var/tmp/gameterm-flatpak-builder \
    --install /var/tmp/gameterm-flatpak-repo \
    assets/flatpak/org.wezfurlong.gameterm.json \
    --force-clean --user -y
fi
