...
URL="https://github.com/JulianAbeleda/gameterm/releases/download/${TAG_NAME}/gameterm-${TAG_NAME}-src.tar.gz"
...
SHA256=$(sha256sum gameterm*-src.tar.gz | cut -d' ' -f1)

sed -e "s,@URL@,$URL,g" -e "s/@SHA256@/$SHA256/g" < assets/flatpak/org.wezfurlong.gameterm.template.json > flathub/org.wezfurlong.gameterm.json

RELEASE_DATE=$(git -c "core.abbrev=8" show -s "--format=%cd" "--date=format:%Y-%m-%d")
sed -e "s,@TAG_NAME@,$TAG_NAME,g" -e "s/@DATE@/$RELEASE_DATE/g" < assets/flatpak/org.wezfurlong.gameterm.appdata.template.xml > flathub/org.wezfurlong.gameterm.appdata.xml
