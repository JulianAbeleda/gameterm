...
CATEGORIZE = {
    ...
    r"^gameterm-\d+-\d+-[a-f0-9]+.tar.xz$": "linux_raw_bin",
    r"src.tar.gz$": "src",
    r"^GameTerm-macos-.*.zip$": "macos_zip",
    r"^GameTerm-windows-.*.zip$": "windows_zip",
    r"^GameTerm-.*.setup.exe$": "windows_exe",
...
def categorize(rel):
    ...
    tag_name = "gameterm-%s" % rel["tag_name"]
...
def load_release_info():
    with open("/tmp/gameterm.releases.json") as f:
        release_info = json.load(f)

    with open("/tmp/gameterm.nightly.json") as f:
        nightly = json.load(f)
