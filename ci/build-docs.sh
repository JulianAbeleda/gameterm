...
[[ -f /tmp/gameterm.releases.json ]] || ghapi /repos/gameterm/gameterm/releases > /tmp/gameterm.releases.json
[[ -f /tmp/gameterm.nightly.json ]] || ghapi /repos/gameterm/gameterm/releases/tags/nightly > /tmp/gameterm.nightly.json
...
cp "assets/icon/gameterm-icon.svg" docs/favicon.svg
mkdir -p docs/fonts
...
docker_or_podman build -t gameterm/mkdocs-material -f ci/Dockerfile.docs .

if [ "$SERVE" == "yes" ] ; then
  docker_or_podman run --rm -it -p8000:8000 -v ${PWD}:/docs gameterm/mkdocs-material serve -a 0.0.0.0:8000
  #docker_or_podman run --rm -it --network=host -v ${PWD}:/docs gameterm/mkdocs-material $@
else
  docker_or_podman run --rm -e CARDS=true -v ${PWD}:/docs gameterm/mkdocs-material build
fi
