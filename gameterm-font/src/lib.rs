...
use gameterm_term::{CellAttributes, Intensity};
use gameterm_toast_notification::ToastNotification;
    ...
    fn process(self) {
        ...
        if !wanted.is_empty() {
            ...
            if show_warning {
                ...
                let url = "https://gameterm.org/config/fonts.html";
                log::warn!(
...
impl FontConfigInner {
            ...
            if !attr.is_synthetic && !attr.is_fallback && !loaded.contains(attr) {
                ...
                let explanation = if is_primary {
                    ...
                    format!(
                        ...
                        Note that gameterm will synthesize font_rules to select bold \
                ...
                config::show_error(&format!(
                    ...
                    https://gameterm.org/config/fonts.html for more information",
    ...
    pub fn match_style<'a>(
        ...
        let would_bright = match attrs.foreground() {
            gameterm_term::color::ColorAttribute::PaletteIndex(idx) if idx < 8 => {
