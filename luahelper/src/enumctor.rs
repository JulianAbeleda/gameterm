...
/// The primary usage is to enable `gameterm.action` to have the following
/// behaviors for KeyAssignment:
///
/// `gameterm.action{QuickSelectArgs={}}` -> compatibility with prior versions;
/// the table is passed through and from_dynamic -> lua conversion is attempted.
///
/// `gameterm.action.QuickSelectArgs` -> since the `QuickSelectArgs` variant
/// has a payload that impl Default, this is equivalent to the call above.
///
/// `gameterm.action.QuickSelectArgs{}` -> equivalent to the call above, but
/// explicitly calls the constructor with no parameters.
///
/// `gameterm.action.QuickSelectArgs{alphabet="abc"}` -> configures the alphabet.
///
