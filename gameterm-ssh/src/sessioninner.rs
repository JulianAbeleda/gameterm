...
impl SessionInner {
    ...
    fn run_impl(&mut self) -> anyhow::Result<()> {
        ...
        match backend {
            ...
            "ssh2" => anyhow::bail!(
                "invalid gameterm_ssh_backend value: {}, not compiled with `ssh2`",
                backend
            ),
            ...
            #[cfg(not(feature = "libssh-rs"))]
            "libssh" => anyhow::bail!(
                "invalid gameterm_ssh_backend value: {}, not compiled with `libssh`",
                backend
            ),

            _ => anyhow::bail!(
                "invalid gameterm_ssh_backend value: {}, expected either `ssh2` or `libssh`",
                backend
            ),
        }
    }
