...
use url::Url;
use gameterm_term::terminal::Alert;
use gameterm_term::StableRowIndex;
        ...
        match decoded.pdu {
            ...
            Pdu::SetClientId(SetClientId {
                ...
                if is_proxy {
                    ...
                    // on from the `gameterm cli list-clients` information
                ...
                match std::env::current_exe().context("resolving current_exe") {
                    ...
                    Ok(executable_path) => {
                        send_response(Ok(Pdu::GetCodecVersionResponse(GetCodecVersionResponse {
                            ...
                            version_string: config::gameterm_version().to_owned(),
                            executable_path,
                            config_file_path: std::env::var_os("GAMETERM_CONFIG_FILE")
                                .map(Into::into),
                        })))
                    }
                }
