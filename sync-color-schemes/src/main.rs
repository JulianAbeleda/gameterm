...
fn make_cache() -> Cache {
    let file_name = "/tmp/gameterm-sync-color-schemes.sqlite";
...
pub async fn fetch_url(url: &str) -> anyhow::Result<Vec<u8>> {
    ...
    let client = reqwest::Client::builder()
        .user_agent("gameterm-sync-color-schemes/1.0")
        .build()?;
...
",
    ...
    for s in &all {
        if s.data.metadata.gameterm_version.as_deref() == Some("nightly builds only") {
...
impl SchemeSet {
    ...
    pub fn load_existing() -> anyhow::Result<Self> {
        ...
        if let Ok(data) = std::fs::read_to_string(&DATA_FILE_NAME) {
            ...
            struct MetaData {
                name: String,
                aliases: Vec<String>,
                gameterm_version: Option<String>,
            }

            let existing: Vec<Entry> = serde_json::from_str(&data)?;
            for item in existing {
                if let Some(version) = &item.metadata.gameterm_version {
    ...
    pub fn add(&mut self, mut candidate: Scheme) {
        ...
        // Resolve gameterm version information for this scheme
...
async fn main() -> anyhow::Result<()> {
    ...
    schemeses
        .sync_toml("https://github.com/catppuccin/gameterm", "main", "")
        .await?;
    schemeses
    ...
    schemeses
        .sync_toml(
            "https://github.com/Hiroya-W/gameterm-sequoia-theme",
            "main",
            "",
        )
        .await?;
    schemeses
        .sync_toml("https://github.com/dracula/gameterm", "main", "")
        .await?;
    schemeses
        .sync_toml(
    ...
    schemeses
        ...
        .await?;
    schemeses
        .sync_toml("https://codeberg.org/anhsirk0/gameterm-themes", "main", "")
        .await?;
    schemeses
        .sync_toml(
            "https://github.com/hardhackerlabs/theme-gameterm",
            "master",
            "",
        )
        .await?;
    schemeses
        ...
        .await?;
    schemeses
        .sync_toml("https://github.com/eldritch-theme/gameterm", "master", "")
        .await?;
