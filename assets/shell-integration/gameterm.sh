...
# This file hooks up shell integration for gameterm.
# It is suitable for zsh and bash.
#
# Although gameterm is mentioned here, the sequences used are not gameterm
...
#
# GAMETERM_SHELL_SKIP_ALL - disables all
# GAMETERM_SHELL_SKIP_SEMANTIC_ZONES - disables zones
# GAMETERM_SHELL_SKIP_CWD - disables OSC 7 cwd setting
# GAMETERM_SHELL_SKIP_USER_VARS - disable user vars that capture information
...
if [ "${GAMETERM_SHELL_SKIP_ALL-}" = "1" ] ; then
  return 0
fi
...
__gameterm_install_bash_prexec() {
...
} # end of __gameterm_install_bash_prexec
...
# command provided by gameterm if gameterm is installed, but falls
# back to a simple printf command otherwise.
__gameterm_osc7() {
  if hash gameterm 2>/dev/null ; then
    gameterm set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed gameterm
...
__gameterm_semantic_precmd_executing=""
__gameterm_semantic_precmd() {
  local ret="$?"
  if [[ "$__gameterm_semantic_precmd_executing" != "0" ]] ; then
    __gameterm_save_ps1="$PS1"
    __gameterm_save_ps2="$PS2"
    ...
    __gameterm_check_ps1="$PS1"
  fi
  if [[ "$__gameterm_semantic_precmd_executing" != "" ]] ; then
  ...
  __gameterm_semantic_precmd_executing=0
}

function __gameterm_semantic_preexec() {
  # Restore the original PS1/PS2 if set
  if [[ -n "${__gameterm_save_ps1+1}" && "${__gameterm_check_ps1-}" == "${PS1}" ]]; then
	  PS1="$__gameterm_save_ps1"
	  PS2="$__gameterm_save_ps2"
	  unset __gameterm_save_ps1
  fi
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __gameterm_semantic_precmd_executing=1
}

__gameterm_user_vars_precmd() {
  __gameterm_set_user_var "GAMETERM_PROG" ""
  __gameterm_set_user_var "GAMETERM_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX-}" ]] ; then
    __gameterm_set_user_var "GAMETERM_IN_TMUX" "1"
  else
    __gameterm_set_user_var "GAMETERM_IN_TMUX" "0"
  fi

  # You may set GAMETERM_HOSTNAME to a name you want to use instead
  # of calling out to the hostname executable on every prompt print.
if [[ -z "${GAMETERM_HOSTNAME}" ]]; then
  if [[ -r /proc/sys/kernel/hostname ]]; then
    __gameterm_set_user_var "GAMETERM_HOST" "$(cat /proc/sys/kernel/hostname)"
  elif hash hostname 2>/dev/null; then
    __gameterm_set_user_var "GAMETERM_HOST" "$(hostname)"
  elif hash hostnamectl 2>/dev/null; then
    __gameterm_set_user_var "GAMETERM_HOST" "$(hostnamectl hostname)"
  else
    __gameterm_set_user_var "GAMETERM_HOST" "unknown"
  fi
else
  __gameterm_set_user_var "GAMETERM_HOST" "${GAMETERM_HOSTNAME}"
fi

}

__gameterm_user_vars_preexec() {
  # Tell gameterm the full command that is being run
  __gameterm_set_user_var "GAMETERM_PROG" "$1"
}
...
if [[ -z "${GAMETERM_SHELL_SKIP_SEMANTIC_ZONES-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__gameterm_semantic_precmd
    blehook PREEXEC+=__gameterm_semantic_preexec
  else
    precmd_functions+=(__gameterm_semantic_precmd)
    preexec_functions+=(__gameterm_semantic_preexec)
  fi
fi

if [[ -z "${GAMETERM_SHELL_SKIP_USER_VARS-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__gameterm_user_vars_precmd
    blehook PREEXEC+=__gameterm_user_vars_preexec
  else
    precmd_functions+=(__gameterm_user_vars_precmd)
    preexec_functions+=(__gameterm_user_vars_preexec)
  fi
fi

if [[ -z "${GAMETERM_SHELL_SKIP_CWD-}" ]] ; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__gameterm_osc7
  else
    precmd_functions+=(__gameterm_osc7)
  fi
fi
