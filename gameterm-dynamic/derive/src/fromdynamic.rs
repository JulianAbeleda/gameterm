...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(gameterm_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| gameterm_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        gameterm_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
                    other => Err(gameterm_dynamic::Error::NoConversion {
    ...
    let tokens = quote! {
        impl #impl_generics gameterm_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &gameterm_dynamic::Value, options: gameterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, gameterm_dynamic::Error> {
                use gameterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(gameterm_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| gameterm_dynamic::Error::Message(
            ...
            quote!(
                    match value {
                        Value::String(s) => {
                            match s.as_str() {
                                #( #units )*
                                _ => Err(gameterm_dynamic::Error::InvalidVariantForType {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(gameterm_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        impl gameterm_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &gameterm_dynamic::Value, options: gameterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, gameterm_dynamic::Error> {
                use gameterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
