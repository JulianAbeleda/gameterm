...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(gameterm_dynamic::PlaceDynamic);
    let bounded_where_clause = bound::where_clause_with_bound(&input.generics, bound);

    let tokens = match info.into {
        Some(into) => {
            quote!(
            impl #impl_generics gameterm_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> gameterm_dynamic::Value {
        ...
        None => {
            quote!(
            impl #impl_generics gameterm_dynamic::PlaceDynamic for #ident #ty_generics #bounded_where_clause {
                fn place_dynamic(&self, place: &mut gameterm_dynamic::Object) {
                    #(
                        #placements
                    )*
                }
            ...
            impl #impl_generics gameterm_dynamic::ToDynamic for #ident #ty_generics #bounded_where_clause {
                fn to_dynamic(&self) -> gameterm_dynamic::Value {
                use gameterm_dynamic::PlaceDynamic;

                let mut object = gameterm_dynamic::Object::default();
                self.place_dynamic(&mut object);
                gameterm_dynamic::Value::Object(object)
                }
            }
            )
        }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
    ...
    let tokens = match info.into {
        Some(into) => {
            quote! {
                impl gameterm_dynamic::ToDynamic for #ident {
                    fn to_dynamic(&self) -> gameterm_dynamic::Value {
                        let target : #into = self.into();
                        target.to_dynamic()
                    }
                    ...
                    Fields::Named(fields) => {
                        ...
                        Ok(quote!(
                            Self::#ident { #( #var_fields, )* } => {
                                let mut place = gameterm_dynamic::Object::default();

                                #( #placements )*

                                let mut obj = gameterm_dynamic::Object::default();
