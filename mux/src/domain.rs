...
impl LocalDomain {
    ...
    async fn fixup_command(&self, cmd: &mut CommandBuilder) -> anyhow::Result<()> {
        if let Some(wsl) = self.resolve_wsl_domain() {
            ...
            *cmd.get_argv_mut() = argv;
            ...
            // Note: GAMETERM_UNIX_SOCKET, GAMETERM_CONFIG_(FILE|DIR) and other env
    ...
    async fn build_command(
        ...
        if let Ok(sock) = std::env::var("GAMETERM_UNIX_SOCKET") {
...
impl Domain for LocalDomain {
    async fn spawn_pane(
        ...
        let mut terminal = gameterm_term::Terminal::new(
            size,
    ...
    async fn domain_label(&self) -> String {
        if let Some(ed) = self.resolve_exec_domain() {
            match &ed.label {
                Some(ValueOrFunc::Value(gameterm_dynamic::Value::String(s))) => s.to_string(),
