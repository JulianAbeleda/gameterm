...
pub fn ssh_connect_with_ui(
    ssh_config: gameterm_ssh::ConfigMap,
    ui: &mut ConnectionUI,
...
pub fn ssh_domain_to_ssh_config(ssh_dom: &SshDomain) -> anyhow::Result<ConfigMap> {
    ...
    ssh_config.insert(
        "gameterm_ssh_backend".to_string(),
        match ssh_dom
            .ssh_backend
    ...
    if let Some("true") = ssh_config.get("gameterm_ssh_verbose").map(|s| s.as_str()) {
...
impl RemoteSshDomain {
    ...
    fn build_command(
        ...
        env.insert("GAMETERM_REMOTE_PANE".to_string(), pane_id.to_string());
...
impl Domain for RemoteSshDomain {
    async fn spawn_pane(
        ...
        let StartNewSessionResult { pty, child, writer } = if let Some(session) = session.take() {
                ...
                .context("request ssh pty")
            {
                ...
                Err(err) => {
                    if err
                        .root_cause()
                        .downcast_ref::<gameterm_ssh::DeadSession>()
                        .is_some()
                    {
        ...
        let terminal = gameterm_term::Terminal::new(
            size,
...
impl std::io::Write for PtyWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        // the next byte.
        // <https://github.com/JulianAbeleda/gameterm/issues/771>
