    ...
    fn log(&self, record: &Record) {
        if self.filter.matches(record) {
...

            {
                ...
                // <https://github.com/JulianAbeleda/gameterm/issues/1839>
...
fn setup_pretty() -> (LevelFilter, Logger) {
    let base_name = std::env::current_exe()
        ...
        .unwrap_or_else(|| "gameterm".to_string());

    if base_name.contains("gui") {
        // Only tidy up logs when the gui process is starting.
        // rationale: `gameterm cli` commands should have as low startup
    ...
    if let Ok(s) = std::env::var("GAMETERM_LOG") {
        filters.parse(&s);
    } else {
