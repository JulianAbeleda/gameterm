...
pub fn set_gameterm_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("GAMETERM_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("GAMETERM_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `GameTerm.AppImage foo`, which is super confusing for everyone!
        ...
        // GAMETERM_EXECUTABLE_DIR is set by `set_gameterm_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("GAMETERM_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via gameterm.
        /// However, if we are using the system gameterm to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_gameterm_config_env() {
...
pub fn bootstrap() {
    config::assign_version_info(
        gameterm_version::gameterm_version(),
        gameterm_version::gameterm_target_triple(),
    );
    setup_logger();
    register_panic_hook();

    set_gameterm_executable();
    ...
    // gameterm never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // gameterm or the mux server from inside gnome terminal.
    // <https://github.com/JulianAbeleda/gameterm/issues/2237>
