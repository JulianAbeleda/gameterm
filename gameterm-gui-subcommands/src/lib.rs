...
pub const DEFAULT_WINDOW_CLASS: &str = "org.wezfurlong.gameterm";
...
pub struct StartCommand {
    ...
    /// in your gameterm configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing gameterm GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of gameterm so that you can wait for the command
    /// to complete by waiting for this gameterm process to finish.
    ...
    /// The default is "org.wezfurlong.gameterm".
    ...
    /// instance of gameterm, including error, update and ssh
    ...
    /// gameterm will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, gameterm will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `gameterm start -- bash -l` will spawn bash
...
pub struct SshCommand {
    ...
    /// `gameterm ssh` is able to parse some (but not all!) options
    ...
    /// For example:
    ///
    /// `gameterm ssh -oIdentityFile=/secret/id_ed25519 some-host`
    #[arg(
        long = "ssh-option",
        short = 'o',
        name = "name=value",
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `gameterm ssh user@host -- bash -l` will spawn bash
