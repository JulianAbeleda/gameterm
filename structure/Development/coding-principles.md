...
## Reducing Code The Right Way (the tiny principle)
...
### Cut what you don't own and don't use (fork corollary)
In a fork, the fastest way to shrink the maintenance surface is not refactoring —
it is **deleting whole inherited subsystems the product does not use**. Carrying
an upstream subsystem you never exercise costs build time, test flake, audit
attention, and security surface for zero product value. Before optimizing code
inside a subsystem, ask whether the product scope (`cache/module-notes/product-scope.md`)
needs that subsystem at all.
...
The allowed prefixes are project-specific. For this repo they live below in **Project Adaptation (GameTerm)**; keep them in that local override section when this scaffold is reused across projects.

Non-functional changes must be marked as NFC:

```text
[repo] NFC - split generated workflow update
```
...
GameTerm enforces this with:

```sh
ci/check-commit-message.sh origin/main..HEAD
```

For local hook use, install the same script as `.git/hooks/commit-msg`.
...
## Project Adaptation (GameTerm)

This section is the GameTerm-specific override layer. It fills in the project
facts the scaffold above intentionally leaves generic. When this scaffold is
reused for another repo, replace this whole section.

GameTerm starts as a fork of WezTerm. The default development rule is to
...
- Prefer small commits that separate upstream syncs, rebranding, feature work, and build/release changes.
- Avoid broad rename sweeps until there is a concrete rename plan for binaries, packages, docs, assets, and config compatibility.
- Keep upstream attribution and licensing intact.
- Add fork-specific documentation in `structure/` before changing upstream docs.

### Rust Workspace

- Treat the root `Cargo.toml` as the source of workspace membership.
- Inspect the relevant crate before changing shared types or APIs.
- Prefer existing crate boundaries and helper APIs over introducing new cross-cutting abstractions.
- Use upstream patterns for error handling, async, config, Lua API exposure, mux behavior, and platform-specific code.
...
For GUI, terminal rendering, mux, config, or platform integration changes, identify the upstream-supported build/test path before claiming the change is verified.

### Subsystem Prefixes

These are the project-specific commit prefixes referenced by **Commit Discipline** above.

- `[identity]` - product naming, app identity, crate names, binary names, branding, and compatibility shims
- `[repo]` - repository structure, workspace metadata, local scaffolding, ignored cache rules, and mechanical maintenance
- `[build]` - Cargo, Makefile, dependency, toolchain, and build-system changes
- `[ci]` - GitHub Actions, generated workflows, release automation, and packaging pipelines
- `[release]` - installers, app bundles, package metadata, changelog, versioning, and distribution assets
- `[docs]` - documentation-only changes
- `[config]` - config model, Lua loading, generated config docs, and user-facing configuration behavior
- `[lua]` - Lua API modules and bindings
- `[mux]` - mux, domains, panes, tabs, sessions, and client/server coordination
- `[term]` - terminal state, escape parsing, terminal protocols, and terminal compatibility
- `[render]` - surfaces, cells, fonts, shaping, GUI rendering, and visual state representation
- `[visual]` - GameTerm Scene Mode, visual scene runtime, patch schema, authoring helpers, and visual-state product surface
- `[window]` - platform windowing, input events, menus, clipboard, and OS integration
- `[pty]` - PTY, process spawning, process inspection, and local shell integration
- `[ssh]` - SSH, remote domains, transport, and remote execution behavior
- `[test]` - test harnesses, fixtures, and verification-only changes

Examples:

```text
[identity] hard rename WezTerm fork to GameTerm
[render] add visual state tile prototype
[config] expose gameterm Lua module alias
[repo] NFC - refresh local audit cache
```

### Local Structure

The `structure/` folder is the local human-facing layer. Keep it compact, factual, and free of secrets or transient logs.
