...
`ci/deploy.sh` is a script that is used to build packages in gameterm's CI.
...
GameTerms version number is derived from the date of the commit from which it
was released:
...
If you are not building gameterm from its git repo, gameterm will read a file named
`.tag` in the root of its source tree to determine the version.

Please package using the `gameterm-YYYYMMDD-HHMMSS-HASH-src.tar.gz` release
...
If you must decorate gameterm's version, then it is recommend that you append
your supplemental extra version information on the end of the gameterm's
version.
...
* `gameterm-mux-server` - multiplexer server. No gui required. Likely want this
  in a separate package from `gameterm-gui` so it can run on a headless system.
* `gameterm-gui` - the GUI portion of the terminal
* `gameterm` - the CLI and frontend that knows how to launch the GUI. It is
...
* `assets/shell-integration`, `assets/shell-completion`: should be deployed along with the `gameterm` executable
* `assets/gameterm.desktop`, `assets/gameterm.appdata.xml`, `assets/gameterm-nautilus.py`: should be deployed along with `gameterm-gui`

## Building gameterm

It is recommended that you enable the `distro-defaults` rust feature
when building for a distro (`cargo build --release -p gameterm-gui --features distro-defaults`).
...
By default, gameterm will compile in a handful of fonts in order to provide a
...
Note that gameterm requires at least the following fonts to be available, either
...
to maintain your gameterm package.

### Building without wayland support

If your distro doesn't include any support for Wayland, you will need to
disable that feature when you build gameterm:

```
cargo build --release -p gameterm-gui --no-default-features --features distro-defaults,vendored-fonts
```
